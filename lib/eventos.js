...
export async function emitirEvento({ empresaId, tipoEvento, payload, origen }) {
  const { data, error } = await supabase
    ...
    .insert({
      ...
      payload: payload || {},
      // origenALS gana sobre el `origen` explícito del caller: funciones
      // compartidas como crearPedidoParaCliente/emitirFactura/anularFactura
      // pasan un origen fijo (ej. 'crearPedidoParaCliente') que describe
      // QUÉ función disparó el evento, sin importar si se llamó desde el
      // panel, un cron o el asistente. Para el checklist §6 (voz vs. a
      // mano) lo que importa es precisamente si vino envuelta en
      // conOrigenAsistenteVoz() — así que ese contexto tiene que pisar el
      // literal del caller, no al revés. Fuera de ese contexto (origenALS
      // vacío) el comportamiento no cambia: se usa el origen del caller.
      origen: origenALS.getStore() || origen || null,
    })
    .select()
    .single();
