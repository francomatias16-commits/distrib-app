...
const TOOLS = [
  ...
  },
  {
    name: 'agregar_item',
    description:
      'Agrega uno o varios productos (con cantidad) al borrador de pedido en curso, o suma cantidad si ya estaban agregados. Requiere haber encontrado cada producto_id con buscar_productos primero — nunca inventar un producto_id. Si el cliente pide varios productos en el mismo mensaje (ej: "2 aceites y 3 harinas"), mandalos TODOS juntos en un solo llamado a esta tool (un elemento del array "items" por producto) en vez de llamarla una vez por producto.',
    parameters: {
      type: 'object',
      properties: {
        items: {
          type: 'array',
          description: 'Uno o más productos a agregar en este mismo llamado.',
          items: {
            type: 'object',
            properties: {
    ...
    async execute({ conversacionId, args }) {
      // FIX (2026-08-30, batch): antes esta tool solo aceptaba UN producto
      // por llamado — un pedido de N productos distintos gastaba N vueltas
      // de ida y vuelta al modelo (ver TOOL_CALLS_MAX en
      // asistente-providers.js), cada una con su propio round-trip de red
      // y su historial/tools reenviados de nuevo. Ahora acepta un array y
      // aplica todos los ítems sobre el MISMO borrador, leído una sola vez,
      // en una sola tool call.
      // Defensivo: si el modelo (sobre todo los gratuitos de Groq/
      // OpenRouter, menos disciplinados con schemas nuevos) todavía manda
      // el shape viejo (producto_id/nombre/cantidad/precio sueltos, sin
      // envolver en items), se lo toma igual como un array de un solo
      // elemento en vez de fallar la tool call entera.
      const itemsPedidos = Array.isArray(args.items) ? args.items : (args.producto_id ? [args] : []);
      if (!itemsPedidos.length) {
        throw new Error('agregar_item: no se recibió ningún producto en items');
      }

      const borrador = await obtenerBorrador(conversacionId);
      const items = borrador.items || [];
      for (const itemPedido of itemsPedidos) {
        const existente = items.find((i) => i.producto_id === itemPedido.producto_id);
        if (existente) {
          existente.cantidad += Number(itemPedido.cantidad) || 0;
        } else {
          items.push({
            producto_id: itemPedido.producto_id,
            nombre: itemPedido.nombre,
            cantidad: Number(itemPedido.cantidad) || 0,
            precio: Number(itemPedido.precio) || 0,
          });
        }
      }
  ...
  },
  {
    ...
    async execute({ conversacionId, args }) {
      const borrador = await obtenerBorrador(conversacionId);
      const itemsActuales = borrador.items || [];
      // FIX (2026-09-08, hallazgo de scripts/audit-borrador-whatsapp.js Capa 4):
      // antes esto era un filter() ciego — si el producto_id no estaba en el
      // borrador (típicamente porque el modelo alucina o repite un id de un
      // turno anterior), el filter no sacaba nada, guardaba el mismo borrador
      // de vuelta, y el bot igual le confirmaba al cliente "listo, lo saqué"
      // sin que fuera cierto (silent no-op reportado como éxito). Mismo
      // criterio de validación explícita que ya usa modificar_cantidad acá
      // abajo.
      const existente = itemsActuales.find((i) => i.producto_id === args.producto_id);
      if (!existente) {
        throw new Error('quitar_item: ese producto no está en el borrador actual');
      }
      const items = itemsActuales.filter((i) => i.producto_id !== args.producto_id);
      const nuevoBorrador = { ...borrador, items };
      await guardarBorrador(conversacionId, nuevoBorrador);
      return nuevoBorrador;
    },
  },
  {
    ...
    description:
      'Cambia la cantidad de un producto que YA está en el borrador a un valor exacto (a diferencia de agregar_item, que suma). ' +
      'Usar cuando el cliente pide dejar una cantidad puntual (ej: "dejalo en 3", "che, poné 2 nomás") o cuando se le ofreció ' +
      'reducir la cantidad por falta de stock y aceptó — no hace falta encadenar quitar_item + agregar_item para esto. ' +
      'Si la cantidad nueva es 0 o menor, el ítem se quita del borrador (mismo resultado que quitar_item).',
    parameters: {
      type: 'object',
      properties: {
        producto_id: { type: 'string', description: 'ID del producto a modificar, tal como aparece en el borrador actual.' },
        cantidad:    { type: 'number', description: 'Cantidad final que debe quedar para ese producto (no se suma a lo existente).' },
      },
    ...
    async execute({ conversacionId, args }) {
      ...
      if (!existente) {
        throw new Error('modificar_cantidad: ese producto no está en el borrador actual');
      }
      const cantidadNueva = Number(args.cantidad) || 0;
      const itemsFinales = cantidadNueva > 0
        ? items.map((i) => (i.producto_id === args.producto_id ? { ...i, cantidad: cantidadNueva } : i))
        : items.filter((i) => i.producto_id !== args.producto_id);
      const nuevoBorrador = { ...borrador, items: itemsFinales };
      await guardarBorrador(conversacionId, nuevoBorrador);
      return nuevoBorrador;
    },
  },
  ...
  },
  {
    name: 'derivar_humano',
    description:
      // FIX (2026-08-30): se saca el criterio de "N idas y vueltas sin
      // identificar qué quiere" (pasó de "2-3" a "5-6" y ahora se elimina
      // del todo) — cualquier número fijo terminaba derivando clientes que
      // responden ambiguo pero de buena fe (ej: "Si", "Seguro" sin un
      // número concreto) antes de agotar intentos razonables de
      // reformular la pregunta. Ahora derivar_humano solo se usa por
      // MOTIVO (pedido explícito de hablar con alguien, o algo fuera de
      // un pedido simple), nunca por cantidad de mensajes — ante
      // ambigüedad, el modelo debe seguir reformulando o pidiendo un dato
      // concreto en vez de rendirse. El corte duro por costo
      // (MAX_TURNOS_SIN_CONFIRMAR = 20 mensajes por ronda, notif.js) sigue
      // intacto como única red de seguridad contra loops largos — no
      // depende de este criterio y no se tocó.
      'Deriva la conversación a un vendedor humano y deja de responder automáticamente. Usar SOLO cuando el cliente pide hablar con una persona, o cuando el pedido es demasiado ambiguo para resolverlo por chat (ej: precios especiales, reclamos, algo fuera de un pedido simple que estas herramientas no pueden resolver). NO derives solo porque llevás varios mensajes sin resolver algo: ante una respuesta ambigua del cliente (ej: "Si", "Seguro", "dale" sin especificar qué), reformulá la pregunta de otra manera o pedí explícitamente el dato concreto que falta (ej: "¿cuántas unidades querés dejar?") las veces que haga falta, en vez de derivar por cansancio.',
    parameters: {
      type: 'object',
      properties: {
