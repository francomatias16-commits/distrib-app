...
export async function emitirFactura(origen, usuarioId = null) {
  ...
  try {
    ...
    if (!resultado.ok) {
      // WSFEv1 puede haber obtenido CAE pero no persistido el resultado local.
      // Se propaga el estado y los datos fiscales para reconciliación; nunca se
      // transforma en un error_afip reintentable que pueda emitir otro CAE.
      return {
        ok: false,
        codigo: resultado.codigo || null,
        estado: resultado.estado || null,
        facturaId: resultado.facturaId || factura.id,
        cae: resultado.cae || null,
        caeVto: resultado.caeVto || null,
        numero: resultado.numeroFormateado || resultado.numero || null,
    ...
    //
    // forma_pago (portal cliente): mismo criterio para pedidos con
    // forma_pago === 'pago_inmediato' — el cliente eligió pagar por afuera
    // de la cuenta corriente (transferencia/efectivo a coordinar con el
    // vendedor), así que este pedido no debe generar deuda.
    const esPedidoPagoInmediato = !pedido.__venta_pos_id && pedido.forma_pago === 'pago_inmediato';
    const montoADebitar = pedido.__venta_pos_id
      ? (pedido.__monto_cta_cte_pos ?? factura.total)
      : (esPedidoPagoInmediato ? 0 : factura.total);
    ...
    if (asientoErr || !asiento?.ok) {
      const asientoMensaje = asientoErr?.message || asiento?.error || 'La RPC no confirmó el asiento';
      console.error(
      ...
      const { error: colaError } = await FacturasRepo.encolarConciliacionFinanciera({
        empresa_id: factura.empresa_id,
        tipo: 'asiento_factura',
        referencia_id: factura.id,
        payload: { factura_id: factura.id, cliente_id: factura.cliente_id, monto: montoADebitar },
        error_msg: asientoMensaje,
      });
      if (colaError) console.error('[facturas] No se pudo encolar asiento pendiente:', colaError.message);
    }
    ...
    if (pedido.__venta_pos_id) {
      const { error: vinculoError } = await FacturasRepo.vincularFacturaAVentaPos(
        pedido.__venta_pos_id,
        factura.id,
        factura.empresa_id,
      );
      if (vinculoError) {
        const vinculoMensaje = vinculoError.message || 'No se pudo vincular la venta POS con la factura';
        const { error: colaError } = await FacturasRepo.encolarConciliacionFinanciera({
          empresa_id: factura.empresa_id,
          tipo: 'vinculo_venta_factura',
          referencia_id: pedido.__venta_pos_id,
          payload: { venta_pos_id: pedido.__venta_pos_id, factura_id: factura.id },
          error_msg: vinculoMensaje,
        });
        if (colaError) console.error('[facturas] No se pudo encolar vínculo venta-factura pendiente:', colaError.message);
      }
    }
...
async function crearFacturaPendiente(pedido, facturacionConfig) {
  ...
  // Para pedidos del portal con forma_pago 'pago_inmediato', mismo criterio:
  // el cliente eligió pagar por afuera de la cuenta corriente, así que la
  // factura nace sin saldo pendiente. Para pedidos a cuenta corriente
  // (default) queda en 0 como siempre.
  const totalYaCobrado = pedido.__venta_pos_id
    ? Math.max(0, pedido.total - (pedido.__monto_cta_cte_pos ?? pedido.total))
    : (pedido.forma_pago === 'pago_inmediato' ? pedido.total : 0);
