...
class CircuitBreaker {
   ...
   * @param {number} [timeoutMsOverride] Ms máx para ESTA llamada puntual,
   *   en vez de this.timeoutMs. Pensado para callers que reparten un
   *   presupuesto de tiempo total entre varias llamadas encadenadas (ver
   *   responderConFallback en asistente-providers.js): cada llamada puede
   *   necesitar un timeout más corto que el configurado en el breaker si
   *   ya se gastó tiempo en intentos anteriores. Si no se pasa, se usa
   *   this.timeoutMs (comportamiento de siempre, sin cambios).
   * @throws {CircuitBreakerOpenError} si el circuito está abierto
   */
  async exec(fn, timeoutMsOverride) {
    ...
    try {
      const resultado = await Promise.race([
        fn(),
        this._timeout(timeoutMsOverride),
      ]);
  ...
  _timeout(timeoutMsOverride) {
    const timeoutMs = timeoutMsOverride ?? this.timeoutMs;
    return new Promise((_, reject) =>
      setTimeout(() => reject(new Error(`[CircuitBreaker:${this.name}] Timeout ${timeoutMs}ms`)), timeoutMs)
    );
  }
