...
async function llamarGemini(params) {
  ...
  for (const apiKey of apiKeys) {
    ...
    for (let intento = 0; intento <= delaysMs.length; intento++) {
      try {
        return await llamarGeminiConKey({ ...params, apiKey });
      } catch (error) {
        ultimoError = error;
    ...
    // Error no reintentable y ajeno a cuota (ej. 400/401): rotar de key no
    // va a cambiar nada, cortamos directo en vez de agotar las restantes.
    if (!ultimoError.cuotaAgotada && !RETRYABLE.includes(ultimoError.status)) throw ultimoError;
  }
