...
async function llamarGeminiConKey({ apiKey, systemPrompt, historial, mensaje, tools, imagen, maxTokens }) {
  ...
  while (true) {
    ...
    if (!funcionesPedidas.length) {
      ...
      if (!texto) {
        const error = new Error('Gemini devolvió una respuesta vacía o inesperada');
...
async function llamarGemini(params) {
  ...
  throw ultimoError || new Error('Sin GEMINI_API_KEYS configuradas.');
}
...
async function llamarGroq({ systemPrompt, historial, mensaje, tools, imagen, maxTokens }) {
