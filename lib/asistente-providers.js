...
async function fetchConTimeout(url, options, timeoutMs) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      // SEC-04 (Auditoría Integral 2026): la URL de Gemini lleva la API key
      // como query param (?key=...). Antes este mensaje incluía `url` tal
      // cual, así que cualquier error HTTP de Gemini terminaba con la key
      // en texto plano en el mensaje del Error — y ese mensaje se loguea
      // más arriba en la cadena (ver responderConFallback y el catch de
      // lib/handlers/asistente.js). Se loguea solo origen+path, sin query
      // string, para los 3 proveedores por igual (Groq/OpenRouter no
      // llevan secretos en la URL, pero se aplica parejo por las dudas).
      let urlSinQuery = url;
      try {
        const u = new URL(url);
...
async function llamarGemini({ systemPrompt, historial, mensaje, tools, imagen, maxTokens }) {
