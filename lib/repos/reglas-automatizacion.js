 ...
 * límite explícito.
 *
 * usuarioId (migración 593, Fase 2 de PLAN_CLIENTES_EN_FUGA.md): además
 * de las tareas por rol, un usuario ve las que se le dirigieron a él
 * puntualmente (ej. "Llamar a [cliente]" asignada a su
 * vendedor_id_default) — filtrar solo por rol no alcanza para esas.
 * Parámetro opcional para no romper otros llamadores existentes que
 * todavía pasen solo (empresa_id, rol).
 */
export async function listarTareasAutomatizacion(empresa_id, rol, usuarioId) {
  let query = db
    .from('tareas_automatizacion')
    .select('id, empresa_id, regla_id, evento_disparador, titulo, descripcion, roles, estado, completada_por, completada_en, created_at, usuario_id')
    .eq('empresa_id', empresa_id)
    .eq('estado', 'pendiente');

  query = usuarioId
    ? query.or(`roles.cs.{${rol}},usuario_id.eq.${usuarioId}`)
    : query.contains('roles', [rol]);
