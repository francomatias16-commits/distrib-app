...
export async function listarSesionesPorEmpresa(empresa_id) {
  return await db
    .from('migracion_sesiones')
    .select('id, entidad, nombre_archivo_original, estado, total_filas, filas_validas, filas_con_error, created_at')
    .eq('empresa_id', empresa_id)
    .order('created_at', { ascending: false })
    .limit(20);
}
