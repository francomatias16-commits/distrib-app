...
export async function listarHistorialRecepciones(empresa_id, { orden_id, offset, limit }) {
  let q = db
    ...
    .select('id, orden_id, estado, foto_url, created_at, confirmada_at, discrepancias, items_conciliados, notas, usuarios(nombre), ordenes_compra(numero)', { count: 'exact' })
    .eq('empresa_id', empresa_id)
...
export async function listarHistorialRecepcionesConOcr(empresa_id, { orden_id, offset, limit }) {
  let q = db
    .from('recepciones_mercaderia')
    .select('id, orden_id, estado, foto_url, created_at, confirmada_at, discrepancias, datos_ocr, items_conciliados, notas, usuarios(nombre), ordenes_compra(numero)', { count: 'exact' })
    .eq('empresa_id', empresa_id)
    .order('created_at', { ascending: false })
    .range(offset, offset + limit - 1);
...
/** URL pública del remito subido. Síncrono (igual que el SDK de Supabase). */
export function obtenerUrlPublicaRemito(path) {
  const { data: { publicUrl } } = db.storage.from('remitos').getPublicUrl(path);
  return publicUrl;
}
