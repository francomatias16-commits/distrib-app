...
/**
 * Estado de suspensión/baja de la empresa, para el portal público de
 * proveedores. Bug de UX: este portal usa `db` con service_role (bypassea
 * RLS) y nunca chequeaba esto — un proveedor de una empresa suspendida
 * veía el portal "vacío" (0 órdenes, 0 facturas) sin ninguna explicación,
 * en vez de un mensaje claro. Silenciosa (puede ser null).
 */
export async function obtenerEstadoEmpresaPortal(empresa_id) {
  const { data } = await db
    .from('empresas')
    .select('activa, saas_suspendida')
    .eq('id', empresa_id)
    .single();
  return data || null;
}
...
/**
 * OFFLINE-05 (auditoría Etapa 5): lookup de idempotencia separado de
 * insertarFacturaProveedorPortal() para que el handler pueda llamarlo ANTES
 * de subir el archivo a Storage — no solo antes del INSERT. Antes, un
 * reintento del outbox (proveedor-offline.js) de una acción `subir_factura`
 * que el servidor YA había aplicado con éxito (la respuesta se perdió antes
 * de llegar al cliente) volvía a subir el archivo entero al bucket
 * `facturas-proveedor` en cada intento, porque el chequeo de idempotencia
 * solo vivía inmediatamente antes del INSERT — el upload, que pasa antes en
 * el handler, se ejecutaba siempre sin importar si la factura ya existía.
 */
export async function buscarFacturaProveedorPorOfflineLocalId(empresa_id, offline_local_id) {
  if (!offline_local_id) return null;
  const { data } = await db
    .from('facturas_proveedor')
 ...
 *
 * OFFLINE-05: este chequeo queda como defensa en profundidad (ej. carrera
 * entre dos requests casi simultáneos) — el handler ya hace el mismo lookup
 * arriba, ANTES de subir el archivo, para el caso común de un reintento.
 */
export async function insertarFacturaProveedorPortal(campos) {
  if (campos.offline_local_id) {
    const existente = await buscarFacturaProveedorPorOfflineLocalId(campos.empresa_id, campos.offline_local_id);
