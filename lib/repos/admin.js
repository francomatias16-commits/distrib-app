...
/** Desglose de ventas por canal (pedidos.canal + POS) — migración 478. */
export async function obtenerVentasPorCanalRpc(params) {
  return db.rpc('obtener_ventas_por_canal', params);
}

/** Resumen de compras/deuda a proveedores del período — migración 478. */
export async function obtenerResumenComprasProveedorRpc(params) {
  return db.rpc('obtener_resumen_compras_proveedor', params);
}

/** Resumen de gastos generales del período (total + por categoría) — migración 479. */
export async function obtenerResumenGastosGeneralesRpc(params) {
  return db.rpc('obtener_resumen_gastos_generales', params);
}

/** Estado financiero integral: ingresos por canal + egresos por categoría +
 *  serie de resultado (día/mes/año) + patrimonio neto aproximado — migración 564. */
export async function obtenerEstadoFinancieroIntegralRpc(params) {
  return db.rpc('obtener_estado_financiero_integral', params);
}
...
// FIX (perf, load test 2026-08-29): antes esto traía TODAS las filas de
// stock×productos de la empresa y agrupaba/filtraba por umbral en JS
// (handleStockBajo). Bajo carga (30 conexiones concurrentes) eso movía
// payloads grandes y consumía CPU del lambda en cada request. Ahora el
// agrupado y el filtro de umbral corren en SQL (462_perf_stock_agregado_en_sql.sql)
// y esto devuelve ya el JSON armado — ver handleStockBajo más abajo.
export async function obtenerStockBajo(empresa_id, limit) {
  return db.rpc('obtener_stock_bajo', { p_empresa_id: empresa_id, p_limit: limit });
}

// Igual criterio que obtenerStockBajo, pero para los 4 totales que usa
// resumen-arranque (unidades/valorizado/con_stock/critico) — ya no hace
// falta traer el detalle por depósito a JS.
export async function obtenerStockResumenArranque(empresa_id) {
  return db.rpc('obtener_stock_resumen_arranque', { p_empresa_id: empresa_id });
}
