...
export async function registrarMensajeWhatsapp({
  conversacion_id, direccion, wa_message_id, texto, tipo, metadata,
  tools_usadas, proveedor_usado,
}) {
  const payload = {
    conversacion_id,
    direccion,
  ...
  // Migración 604: opcionales a propósito — solo el mensaje saliente del
  // bot generado por procesarConAsistente() los tiene (ver notif.js,
  // responderYRegistrar()). El resto de los ~9 call sites (mensajes
  // entrantes, avisos fijos, derivación a humano) no manda estos campos y
  // se insertan como antes, con los DEFAULT de la migración ('[]'/NULL).
  if (tools_usadas !== undefined) payload.tools_usadas = tools_usadas;
  if (proveedor_usado !== undefined) payload.proveedor_usado = proveedor_usado;

  const { error } = await db.from('whatsapp_mensajes').insert(payload);
  return { error };
}
