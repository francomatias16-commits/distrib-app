...
export async function listarChequesPorVencer(desde, hasta) {
  const { data, error } = await db
    .from('cheques')
    .select('id, numero, monto, vencimiento, banco, empresa_id, clientes(id, razon_social, telefono)')
    // FIX: 'pendiente' nunca se usa en la práctica — el estado real para
    // "cheque en cartera, todavía sin cobrar" es 'en_cartera' (mismo
    // criterio que obtenerChequesVencidos en lib/repos/admin.js). Con
    // 'pendiente' este filtro no traía nunca resultados y el aviso de
    // cheques por vencer no disparaba nunca.
