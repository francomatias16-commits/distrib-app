...
/**
 * Inserta una fila en `notificaciones_push` — la tabla que lee de verdad la
 * campanita del topbar (AdminRepo.obtenerNotificacionesRecientes), distinta
 * de `notif_log` (auditoría de envío por canal). Silenciosa a propósito:
 * la llaman notifAuto() y pushInternoHandler() con `.catch(() => {})`, así
 * que un error acá no debe tumbar el flujo de push/email que sí importa.
 */
export async function insertarNotificacionPush({ empresa_id, titulo, cuerpo, tipo, datos_json = null }) {
  try {
    await db.from('notificaciones_push').insert({
      empresa_id, tipo, titulo, cuerpo, datos_json, leida: false,
    });
  } catch (err) {
    console.error('[NotifRepo.insertarNotificacionPush] Error silencioso:', err.message);
  }
}
