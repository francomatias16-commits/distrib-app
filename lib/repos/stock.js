...
/**
 * Ajuste de stock vía RPC `ajustar_stock` — envoltorio compartido para no
 * duplicar la forma de los parámetros `p_*` en cada caller (hasta v1089
 * solo la vivía `lib/handlers/stock.js`; el endpoint de alta rápida desde
 * el POS, punto 1 del audit, la reusa tal cual).
 */
export async function ajustarStockRpc({ producto_id, deposito_id, delta, tipo, motivo, notas, usuario_id }) {
  return db.rpc('ajustar_stock', {
