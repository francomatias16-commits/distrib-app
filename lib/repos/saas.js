...
export async function listarEventosNegocio({ empresaId, tipoEvento, estado, offset, limite }) {
  // Fase 5 — fix retención: lee de la vista eventos_negocio_completo (migración
  // 589) para no perder acceso a eventos archivados por el cron de retención
  // (>180 días, ver lib/handlers/retencion.js). La vista no tiene FK real hacia
  // empresas (es un UNION ALL), así que PostgREST no puede resolver el embed
  // `empresas(nombre)` que se usaba contra la tabla viva — la vista ya trae
  // `empresa_nombre` resuelto por LEFT JOIN, y acá lo reempaquetamos como
  // `empresas: { nombre }` para no tener que tocar el frontend (saas-billing.html
  // sigue leyendo `ev.empresas?.nombre` tal cual).
  let query = db
    .from('eventos_negocio_completo')
    .select('id, empresa_id, tipo_evento, payload, origen, estado, creado_en, procesado_en, empresa_nombre', { count: 'exact' })
  ...
  const dataConEmpresas = (data || []).map(({ empresa_nombre, ...ev }) => ({
    ...ev,
    empresas: { nombre: empresa_nombre },
  }));
  return { data: dataConEmpresas, error: null, count };
}
