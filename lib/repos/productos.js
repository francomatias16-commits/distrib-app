...
export async function buscarProductosPos(empresa_id, { codigo, vendidoPorPeso, textoLibre, limit } = {}) {
  let query = db
    .from('productos')
    .select('id, codigo, nombre, precio_base, iva, unidad, vendido_por_peso')
    .eq('empresa_id', empresa_id)
    .eq('activo', true);

  if (vendidoPorPeso) query = query.eq('vendido_por_peso', true);
  if (codigo) query = query.eq('codigo', codigo);
  if (textoLibre) query = query.or(`codigo.ilike.${textoLibre},nombre.ilike.${textoLibre}`);
