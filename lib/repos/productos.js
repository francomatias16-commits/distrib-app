...
export async function buscarIdsProductos(empresa_id, term) {
  const { data } = await db
    ...
    .eq('empresa_id', empresa_id)
    .eq('activo', true)
    .or(`nombre.ilike.%${escaparFiltroPostgrest(term)}%,codigo.ilike.%${escaparFiltroPostgrest(term)}%`);
  return data || [];
}
...
export async function buscarProductosParaRemito(empresa_id, { busqueda } = {}) {
  ...
  if (busqueda) {
    const b = escaparFiltroPostgrest(busqueda);
    query = query.or(`nombre.ilike.%${b}%,codigo.ilike.%${b}%`);
  }
...
export async function buscarProductosPos(empresa_id, { codigo, vendidoPorPeso, textoLibre, limit } = {}) {
  // v631: se suma permite_negativo. La columna existe desde la migración
  // 001 y el trigger de la 438 la respeta, pero nunca había salido de la
  // base hacia el POS — sin ella el frontend no puede distinguir "sin
  // stock, no se puede vender" de "sin stock, pero este producto está
  // autorizado a quedar en negativo".
  const baseSelect = 'id, codigo, nombre, precio_base, iva, unidad, vendido_por_peso, permite_negativo';
  ...
  if (textoLibre) {
    ...
    // FIX (v1089, punto 3 del audit del POS): `term` se interpolaba crudo
    // en los tres `.or()` de abajo. La coma y los paréntesis son sintaxis
    // dentro de un filtro `.or()` de PostgREST, así que un producto como
    // "SECADOR GOMA SIMPLE HACENDOSA 40 CM (SIN CABO)" no se encontraba
    // aunque el cajero escribiera el nombre exacto — el paréntesis partía
    // la condición al medio. Sobre el catálogo real de la empresa piloto
    // eran 104 de 1458 productos activos (7%). Se escapa una sola vez acá
    // y se reusa en los tres niveles.
    const term  = escaparFiltroPostgrest(textoLibre.replace(/^%|%$/g, '')); // texto sin los % que arma el caller
    const tope  = limit || 20;
...
export async function obtenerProductosParaVentaPos(empresa_id, ids) {
  const { data } = await db
    .from('productos')
    .select('id, nombre, precio_base, iva, activo, permite_negativo')
    .eq('empresa_id', empresa_id)
    .in('id', ids);
  return data || [];
}
