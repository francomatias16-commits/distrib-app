...
export async function obtenerUsuarioParaGestionFactura(user_id) {
  const { data } = await db.from('usuarios').select('id, empresa_id, rol').eq('id', user_id).single();
  return data;
}
...
/** Deja la referencia factura_id en la venta POS de origen. */
export async function vincularFacturaAVentaPos(venta_pos_id, factura_id) {
...
export async function obtenerNotaCreditoParaEmitir(id, empresa_id) {
  const { data, error } = await db
    .from('notas_credito')
    .select(`*, clientes(*), facturas(numero, tipo, cae), notas_credito_items(*)`)
    .eq('id', id)
    .eq('empresa_id', empresa_id)
    .single();
  return { data, error };
