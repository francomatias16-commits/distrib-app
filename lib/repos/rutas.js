...
/**
 * Genera (o regenera) el reporte de eficiencia de una ruta ya completada.
 * Extraído del handler `POST /api/rutas-live?accion=cerrar-reporte` para
 * poder invocarlo también automáticamente desde `sincronizarEstadoRuta`
 * (lib/handlers/pedidos/_helpers.js) apenas la última entrega de la ruta
 * queda en estado terminal — antes esto dependía de un botón/acción manual
 * que ninguna pantalla del frontend llegó a implementar, así que
 * `reportes_ruta` quedaba siempre vacía aunque hubiera rutas completadas.
 */
export async function generarReporteEficienciaRuta(ruta_id) {
  const ruta = await obtenerRutaParaCerrarReporte(ruta_id);
  if (!ruta) return;

  const entregas = await listarEntregasParaReporte(ruta_id);
  const total      = entregas?.length || 0;
  const entregadas = entregas?.filter(e => e.estado === 'entregado').length || 0;

  await upsertReporteRuta({
    ruta_id,
