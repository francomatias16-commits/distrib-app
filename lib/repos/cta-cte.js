...
/**
 * Saldo de deuda actual de un cliente.
 *
 * HALLAZGO CTACTE-AUDIT-01 (corregido acá): esta función leía la columna
 * por-fila `cta_cte.saldo` (fila más reciente por `fecha`), asumiendo un
 * modelo de "saldo corrido" donde cada movimiento trae el acumulado a esa
 * fecha. Ese modelo está roto en la práctica: ninguno de los flujos reales
 * que insertan en `cta_cte` completa esa columna —
 * `registrar_venta_pos` (mig. 468), `registrar_cobro_completo` (mig. 486)
 * y `aplicar_nota_credito_cta_cte` (mig. 315) insertan sin `saldo`, que
 * queda NULL. Solo `insertarEnCtaCte()` (más abajo, usado por cierre.js)
 * lo completaba a mano. Como POS/cobros/NC son mucho más frecuentes que el
 * cierre de pedidos sin facturación electrónica, en la práctica la fila
 * más reciente de casi cualquier cliente tiene `saldo = NULL`, y
 * `data?.saldo || 0` devolvía 0 — deuda cero aunque el cliente debiera.
 *
 * Impacto real: `procesarNotifVencimiento()` en cierre.js usa este valor
 * para decidir si manda el recordatorio de vencimiento (`if (deuda <= 0)
 * return`) — con el bug, esa notificación se salteaba en silencio para la
 * mayoría de los clientes con deuda real, cada vez que su movimiento más
 * reciente en cta_cte no fuera uno de los pocos insertados por cierre.js.
 * No afectaba el límite de crédito en POS/pedidos ni el saldo mostrado en
 * el panel admin o portal cliente, porque esos leen `clientes.saldo_deuda`
 * — una columna aparte, mantenida por el trigger `sync_saldo_deuda_cliente`
 * (mig. 078/409) que SUMA todos los movimientos por tipo en cada
 * insert/update/delete de `cta_cte`, sin depender de la columna `saldo`.
 *
 * Fix: leer directamente `clientes.saldo_deuda`, la fuente ya correcta y
 * siempre sincronizada, en vez de reconstruir un "saldo corrido" que
 * ningún flujo mantiene consistentemente.
 */
