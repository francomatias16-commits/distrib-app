...
// Frente 2 de PLAN_OPTIMIZACION_ASISTENTE_2026.md (migración 602):
// análoga a buscarArticulosAsistenteRpc, pero sin p_rol — el filtro por
// rol de las tools lo hace seleccionarToolsRelevantes() en JS (ver
// lib/asistente-tools/index.js), no la RPC.
export async function buscarToolsAsistenteRpc({ query_embedding, match_count, match_threshold }) {
  return db.rpc('buscar_tools_asistente_rpc', {
    query_embedding,
    match_count,
    match_threshold,
  });
...
// Frente 3 de PLAN_OPTIMIZACION_ASISTENTE_2026.md (migración 601): los 3
// campos nuevos son opcionales a propósito (undefined si el caller no los
// pasa) — mismo criterio fail-open que el resto de este archivo, para no
// romper ningún call site existente que no sepa de ellos todavía.
// metodo_seleccion_tools se suma en el Frente 2 (migración 602), mismo
// criterio: opcional, undefined si no se pasa.
export async function insertarUsoAsistente({
  usuario_id, empresa_id, pregunta, proveedor_usado, articulos_encontrados, latencia_ms,
  cayo_en_nucleo_fallback, cantidad_tools_con_match, tool_finalmente_usada, metodo_seleccion_tools,
}) {
  return db.from('asistente_uso').insert({
    ...
    latencia_ms,
    cayo_en_nucleo_fallback,
    cantidad_tools_con_match,
    tool_finalmente_usada,
