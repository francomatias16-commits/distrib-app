...
/**
 * Etapa 1 auditoría de dinero (2026-09): `presupuestos.estado='vencido'`
 * no lo pone nunca ni un cron ni nada — se descubrió al auditar el punto
 * abierto del plan. Mismo patrón ya usado para `lotes.estado`
 * (actualizar_estado_lotes, 442): autocorregir en cada lectura en vez de
 * depender de un cron. No crítico si falla: seguimos con la lectura
 * igual (mismo criterio que el de lotes — ver lib/repos/stock.js).
 */
async function autocorregirPresupuestosVencidos(empresa_id) {
  try {
    await db.rpc('actualizar_estado_presupuestos_vencidos', { p_empresa_id: empresa_id });
  } catch (_err) {
...
export async function obtenerPresupuestoConDetalle(empresa_id, id) {
  await autocorregirPresupuestosVencidos(empresa_id);
