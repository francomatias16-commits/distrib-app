...
export async function obtenerClienteCredito(cliente_id) {
  const { data } = await db
    .from('clientes')
    .select('limite_credito, saldo_deuda')
    .eq('id', cliente_id)
    .single();
  return data;
}
 ...
 */
export async function buscarEntregaPorOfflineLocalId(offline_local_id) {
  const { data } = await db
    .from('entregas')
 ...
 */
export async function buscarDevolucionPorOfflineLocalId(offline_local_id) {
  const { data } = await db
    .from('devoluciones')
...
/** Ítems de una devolución a reponer en stock — `items_reponer` acota a una selección parcial (null = todos). */
export async function listarItemsDevolucionParaReponer(devolucion_id, items_reponer = null) {
  let query = db.from('devolucion_items').select('id, producto_id, cantidad').eq('devolucion_id', devolucion_id);
...
/** Marca el pedido como cancelado (fire-and-forget, sin select — mismo criterio que el original). */
export async function marcarPedidoCancelado(empresa_id, id) {
...
export async function anularFacturaPendiente(id) {
  await db.from('facturas').update({ estado: 'anulada' }).eq('id', id);
}
...
export async function obtenerClientePorIdParaConfirmar(empresa_id, cliente_id) {
  const { data, error } = await db
    .from('clientes')
    .select('id, razon_social, telefono, limite_credito, dias_credito, activo, saldo_deuda')
    .eq('id', cliente_id)
    .eq('empresa_id', empresa_id)
    .maybeSingle();
  return { data, error };
