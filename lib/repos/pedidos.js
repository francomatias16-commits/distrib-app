...
export async function listarPresupuestos(empresa_id, { estado, clienteId } = {}) {
  let query = db
    ...
    `)
    .eq('empresa_id', empresa_id)
    // Regla "ítem modificado sube al tope" (2026-09, segundo lote): antes
    // ordenaba por creación, así que aceptar/rechazar/editar un
    // presupuesto viejo no lo traía a la vista.
...
export async function listarDevolucionesFiltradas({
  ...
  let q = db
    ...
    .eq('empresa_id', empresa_id)
    // Regla "ítem modificado sube al tope" (2026-09, segundo lote): antes
    // ordenaba por creación, así que aprobar/rechazar una devolución
    // vieja no la traía a la vista.
    .order('updated_at', { ascending: false });
