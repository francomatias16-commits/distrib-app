...
export async function obtenerDatosEditables(empresa_id) {
  const { data, error } = await db
    .from('empresas')
    .select('nombre, cuit, domicilio, telefono, email, logo_url, config, slug')
    .eq('id', empresa_id)
    .single();
...
/**
 * Actualiza el slug del link del catálogo público (501). Propaga error.code
 * para que el handler distinga el 23505 (slug ya tomado por otra empresa)
 * del resto — mismo criterio que actualizarDatosEmpresa con el CUIT.
 */
export async function actualizarSlug(empresa_id, slug) {
  const { data, error } = await db
    .from('empresas')
    .update({ slug })
    .eq('id', empresa_id)
  ...
  if (error) {
    const e = new Error(`[EmpresaRepo.actualizarSlug] ${error.message}`);
    e.code = error.code;
    throw e;
  }
  return data;
