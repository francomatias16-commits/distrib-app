...
export async function asignarDepositoACaja(caja_id, deposito_id, empresa_id = null) {
  let query = db.from('cajas_pos').update({ deposito_id }).eq('id', caja_id);
  if (empresa_id) query = query.eq('empresa_id', empresa_id);
  const { data, error } = await query.select('id, deposito_id').maybeSingle();
  return { data, error };
}
...
export async function listarFavoritosPos(empresa_id) {
  const { data, error } = await db
    .from('pos_favoritos')
    .select('id, producto_id, etiqueta, color, orden, productos(nombre, codigo, precio_base, iva, unidad, permite_negativo)')
    .eq('empresa_id', empresa_id)
    .order('orden')
    .order('created_at');
  return { data, error };
...
export async function obtenerEmpresaParaHardware(empresa_id) {
  const { data, error } = await db
    .from('empresas')
    .select('nombre, cuit, domicilio, telefono, logo_url, config')
    .eq('id', empresa_id)
    .single();
  return { data, error };
}
...
// ── config-hardware POR CAJA (migración 583 / AUDITORÍA 584) ─────────────
// Antes la config de impresora/terminal vivía en `empresas.config->
// pos_hardware` (una sola fila por empresa). Ahora es `cajas_pos.
// hardware_config`, una fila por caja — ver 583_hardware_config_por_caja.sql
// para el porqué (varias cajas físicas en simultáneo, cada una con su
// propia terminal Prisma/MP Point/Getnet).

/** Config de hardware guardada de una caja puntual (scoped por empresa_id). */
export async function obtenerCajaHardwareConfig(caja_id, empresa_id) {
  const { data, error } = await db
    .from('cajas_pos')
    .select('id, nombre, hardware_config')
    .eq('id', caja_id)
    .eq('empresa_id', empresa_id)
    .maybeSingle();
  return { data, error };
...
/** Guarda el `hardware_config` completo de una caja puntual (usado tras mergear impresora/terminal). */
export async function actualizarCajaHardwareConfig(caja_id, empresa_id, hardware_config) {
  const { data, error } = await db
    .from('cajas_pos')
...
export async function togglePromocion(id, empresa_id, activaNueva) {
  const { error } = await db
    .from('promociones')
    .update({ activa: activaNueva })
    .eq('id', id)
    .eq('empresa_id', empresa_id);
  return { error };
}
...
export async function obtenerTurnoConEmpresa(turno_id) {
  const { data } = await db
    .from('turnos_caja')
    .select('id, usuario_id, cajas_pos!inner(empresa_id)')
    .eq('id', turno_id)
    .maybeSingle();
  return data;
}
...
export async function obtenerTurnoConEstadoYEmpresa(turno_id) {
  const { data } = await db
    .from('turnos_caja')
    .select('id, usuario_id, estado, cajas_pos!inner(empresa_id)')
    .eq('id', turno_id)
    .maybeSingle();
  return data;
}
...
export async function listarDevolucionesDeVenta(venta_id, empresa_id) {
  const { data, error } = await db
    ...
    .select(`
      id, motivo, monto_total, created_at, aviso_compensacion, monto_acreditado_cta_cte,
      usuarios(nombre),
