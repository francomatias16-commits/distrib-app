...
import { insertarNotificacionPush } from '../repos/notif.js';
...
export async function notifAuto(empresa_id, { tipo, titulo, cuerpo, link = '/admin/automatizacion' }) {
  try {
    ...
    // FIX ("campanita no avisa hasta 24hs"): mismo hallazgo que en
    // pushInternoHandler (lib/handlers/notif.js) — notifAuto nunca insertaba
    // en `notificaciones_push` (la tabla real de la campanita), solo en
    // notif_log (auditoría de envío). Se inserta acá, ya pasado el chequeo
    // de preferencia deshabilitada, e independiente de si hay tokens o el
    // envío del push en sí falla — para que la campanita muestre el aviso
    // de inmediato aunque nadie tenga el permiso de notificaciones activado.
    insertarNotificacionPush({ empresa_id, titulo, cuerpo, tipo, datos_json: { link } }).catch(() => {});
