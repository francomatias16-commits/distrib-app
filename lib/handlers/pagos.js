...
async function crearPreferencia(req, res) {
  try {
    ...
    // FIX (bug reportado: cliente eligió "cuenta corriente" al confirmar el
    // pedido y sin embargo veía "Pagar online"): un pedido con
    // forma_pago === 'cuenta_corriente' ya va a debitarse de la cta_cte del
    // cliente (ver confirmar-pedido.js) — generarle además un link de MP
    // permitiría pagarlo una segunda vez por otro medio. Se corta acá,
    // server-side, para no depender de que el frontend nunca muestre el
    // botón por error.
    if (pedido.forma_pago === 'cuenta_corriente') {
      return res.status(400).json({
        error: 'Este pedido se paga por cuenta corriente, no requiere pago online.',
      });
    }
...
export { manejarWebhook as webhook, procesarEventoMP, verificarPago };
