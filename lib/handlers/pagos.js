...
async function prismaCobrarHandler(req, res) {
  try {
    ...
    let orden;
    try {
      orden = await prismaBreaker.exec(() =>
        withRetry(() => fetchPrisma(
...
async function manejarWebhook(req, res) {
  try {
    ...
    // CHANGELOG_v760_qr_mercadopago_pos.md) — esto solo resuelve la
    // sincronización de la UI, la venta la sigue registrando el frontend
    // (POST /api/pos) una vez que se entera de que se pagó.
    if (type === 'order') {
      const orderId = data?.id;
      console.log('[MP-WEBHOOK] Notificación de orden QR recibida:', orderId, 'action:', req.body?.action);
      if (!orderId) {
        return res.status(200).json({ received: true, order_id: null });
      }
    ...
    if (type === 'payment') {
      ...
      if (!mpUserId) {
        ...
        return res.status(200).json({ received: true, error: 'user_id ausente en la notificación' });
      }
      ...
      if (!pedido_id) {
        ...
        return res.status(200).json({ received: true, sin_pedido_asociado: true });
      }
      ...
      if (!tx) {
        ...
        return res.status(200).json({ received: true, error: 'transacción no encontrada para el pedido en esta empresa' });
      }
      ...
      if (!errorUpdate && (!filasActualizadas || filasActualizadas.length === 0)) {
        ...
        return res.status(200).json({ received: true, message: 'Payment already processed (race)' });
      }
