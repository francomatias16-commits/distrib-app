...
async function handleVision(req, res) {
  ...
  try {
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        messages: [{ role: 'user', content: [
          { type: 'image', source: { type: 'base64', media_type: mime_type, data: await preprocessImage(imagen_base64, mime_type) } },
    ...
    catch { return res.status(502).json({ error: 'Respuesta de Vision no es JSON válido', raw: texto }); }
...
- No incluyas subtotales, totales ni encabezados como productos.`;
  ...
  try {
    ...
    catch { return res.status(502).json({ error: 'Respuesta OCR no es JSON válido', raw: texto }); }
