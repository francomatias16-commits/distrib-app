...
import {
  ...
  obtenerWabaIdYTokenWhatsapp,
  borrarCredencialesWhatsapp,
  ...
  cerrarConversacionPorExpiracion,
...
// Fase 2 de PLAN_CLIENTES_EN_FUGA.md — mismo criterio que el resto de los
// imports de repos de este archivo: livianos, sin ciclo (clientes-fuga.js
// y piloto.js solo importan repos/_db.js). El listener
// (eventos-listeners/cliente_en_riesgo_fuga.js) es el que import dinámico
// del dispatcher evitaría un ciclo — acá no hace falta, porque
// clientesEnFugaRpc/listarEmpresasActivas no importan nada de este archivo.
import { clientesEnFugaRpc, resolverUmbralClienteGrande, ultimoAvisoFuga } from '../repos/clientes-fuga.js';
import { listarEmpresasActivas } from '../repos/piloto.js';
import { obtenerSaldoPuntos } from '../repos/fidelizacion.js';
import { waEndpointConfigurado, leerRespuestaWa } from '../wa-endpoint-http.js';
...
export default async function handler(req, res) {
  ...
  if (_svc === 'fuga-cron')          return handleFugaCron(req, res);
  ...
  if (_svc === 'webhooks-reprocesar-cron') return handleWebhooksReprocesarCron(req, res);
  ...
  // Desconexión manual desde el panel (botón "Desconectar" en Configuración
  // → Conectar WhatsApp) — auth de usuario vía Bearer, ver
  // whatsappDesconectarHandler.
  if (_svc === 'whatsapp-desconectar') return whatsappDesconectarHandler(req, res);
...
// Circuit breaker (Etapa 5, PLAN_ROBUSTEZ_ESCALABILIDAD_PROFESIONAL_2026.md):
// mismo patrón que mercado-pago/prisma-paystore (lib/handlers/pagos.js) y
// supabase-auth (lib/handlers/auth.js). Un solo breaker compartido por
// whatsappHandler() y enviarTextoWhatsApp() porque ambos pegan al mismo
// servicio externo (Graph API de Meta) — igual criterio que "un breaker
// por servicio externo", no por endpoint/función.
// Exportado para que otros callers directos de la Graph API de Meta (ej.
// lib/handlers/piloto.js, envío masivo del Piloto Automático) compartan el
// mismo breaker en vez de abrir uno nuevo por archivo — sigue habiendo un
// solo breaker por servicio externo.
export const waBreaker = new CircuitBreaker({
  name:               'whatsapp-meta',
  umbralFallas:       5,
...
const TEMPLATES = {
  ...
  // Fase 2 de PLAN_CLIENTES_EN_FUGA.md — recuperación automática para
  // clientes chico/mediano que rompieron su patrón de compra (ver
  // enviarRecuperacionFuga más abajo). Igual que pedido_por_llegar y
  // codigo_recuperacion_password: falta darlo de alta y aprobarlo en Meta
  // Business Manager con este mismo nombre/idioma/variables antes de que
  // el envío deje de fallar con "template inexistente" — el resto del
  // flujo (notif_log, cooldown) ya queda funcionando igual.
  recuperacion_cliente: {
    name: 'recuperacion_cliente', language: 'es_AR',
    components: (p) => [{ type: 'body', parameters: [
      { type: 'text', text: p.nombre_cliente },
      { type: 'text', text: p.producto_principal },
      { type: 'text', text: String(p.puntos_disponibles) },
    ]}],
  },
};
...
// Exportada (antes interna) para poder testear directamente el control de
// acceso — Etapa 8, ver tests/handlers/whatsapp-notif-permisos.test.js.
export async function whatsappHandler(req, res) {
  ...
  // HALLAZGO (auditoría v960, sesión notif/whatsapp): este endpoint no
  // tenía NINGÚN check de auth — a diferencia de todos los demás _svc de
  // este mismo archivo (reintentar-email, whatsapp-conversacion-accion,
  // whatsapp-embedded-signup), que exigen verificarToken()+puede(). Sin
  // empresa_id en el body (como lo llamaban pedidos.js/rutas.js) caía en
  // el fallback de resolverCredencialesWhatsapp() y disparaba un envío
  // real contra el número compartido de la plataforma si
  // WA_NOTIF_SALIENTES_HABILITADAS estaba en true — cualquiera con la URL
  // podía mandar cualquiera de los templates aprobados a cualquier
  // teléfono, sin login, con costo real por Meta y superficie de
  // phishing/spam usando plantillas de "tu pedido está en camino".
  // Con empresa_id explícito en el body (nunca usado por el frontend
  // legítimo, pero aceptado igual) además permitía elegir de qué empresa
  // se descontaba el envío. Fix: mismo patrón que el resto del archivo —
  // Bearer obligatorio, empresa_id SIEMPRE de la sesión (perfil), nunca
  // del body.
  //
  // FIX AUTOMATIZACION-003 (auditoría etapa 6, esta sesión): el fix de
  // v960 de arriba era correcto contra un cliente HTTP externo, pero dejó
  // sin forma de pasar NINGÚN caller server-to-server legítimo del propio
  // repo — y hay 6: `notificarEstado`/`notificarPedidoCreado`
  // (lib/handlers/pedidos/notificaciones.js, confirmación y despacho de
  // pedido), el recordatorio de deuda de cierre.js, el código de
  // recuperación de contraseña por WhatsApp de auth.js, la oferta de plan
  // de pago a clientes en riesgo de score.js, y la acción 'enviar_whatsapp'
  // de las reglas de automatización personalizadas
  // (lib/reglas-automatizacion.js) — ninguno manda Authorization, todos
  // corren en background sin sesión de usuario. Desde v960 los 6 pegan
  // contra este endpoint, reciben 401 y la notificación se pierde en
  // silencio (o queda logueada como error, según el caller) — confirmación
  // de pedido, aviso de despacho, recordatorio de vencimiento, recuperar
  // contraseña, oferta de plan de pago y automatizaciones custom de
  // WhatsApp dejaron de mandarse de verdad desde entonces. Se agrega un
  // segundo camino de auth, server-to-server, con el mismo secreto que ya
  // usan los crons de este archivo (CRON_SECRET) — fail-closed si no está
  // configurada, igual que el resto. En ese camino no hay `perfil`:
  // `empresa_id` viene del body, puesto por el propio código interno
  // (nunca alcanzable por un cliente externo, que no conoce CRON_SECRET).
  // Los 6 callers se actualizan para mandar este header — ver sus propios
  // comentarios de fix.
  const CRON_SECRET_WA = process.env.CRON_SECRET;
  const esLlamadaInternaWa = !!CRON_SECRET_WA && req.headers.authorization === `Bearer ${CRON_SECRET_WA}`;

  let empresa_id;
  if (esLlamadaInternaWa) {
    empresa_id = req.body?.empresa_id;
    if (!empresa_id) return res.status(400).json({ error: 'empresa_id requerido (llamada interna)' });
  } else {
    const perfil = await verificarToken(req, supabase);
    if (!perfil) return res.status(401).json({ error: 'No autorizado' });
    if (!puede(perfil, 'enviar', 'whatsapp_template')) {
      return res.status(403).json({ error: 'Rol insuficiente para enviar WhatsApp' });
    }
    empresa_id = perfil.empresa_id;
  }

  const { template, telefono, params } = req.body;
  ...
  // ── Corte de plan trial — tope de mensajes salientes (Etapa 8, migración
  // 576) ──────────────────────────────────────────────────────────────
  // Trial permite un puñado de mensajes de WhatsApp de prueba (10, cuenta
  // todo el historial vía chequear_limite_plan('whatsapp_mensajes'), no
  // por mes) antes de exigir upgrade. Mismo criterio de "bloqueo
  // silencioso" que el corte de enviosHabilitados de más abajo: no es un
  // error del caller, es una política comercial — 200 con bloqueado:true
  // en vez de 403, para no romper al frontend que ya maneja esa forma de
  // respuesta.
  try {
    await exigirLimitePlan(supabase, empresa_id, 'whatsapp_mensajes');
  } catch (err) {
    if (err instanceof LimitePlanError) {
      const { message_id } = whatsappSimulado();
      return res.status(200).json({ ok: true, bloqueado: true, message_id, template, telefono: telefonoLimpio, limite_plan: true });
    }
    throw err;
  }
  ...
  try {
    ...
    // Circuit breaker (Etapa 5, PLAN_ROBUSTEZ_ESCALABILIDAD_PROFESIONAL_2026.md):
    // envuelve toda la secuencia de intentos (131030 + reintentos transitorios)
    // como una sola unidad — mismo criterio que mpBreaker.exec(() => withRetry(...))
    // en pagos.js. Si Meta viene con fallas consecutivas, corta de entrada en
    // vez de agotar los reintentos de cada request nueva.
    const { resp, data } = await waBreaker.exec(async () => {
      ...
      // Un !resp.ok que llega hasta acá (agotó los reintentos transitorios,
      // o fue un 4xx no transitorio) SÍ debe contar como falla para el
      // breaker — mismo criterio que fetchMP() en pagos.js, que lanza en
      // cualquier !response.ok. No se lanza en el 502 de negocio (token
      // vencido, plantilla rechazada, etc.) más abajo porque eso ya salió
      // de este bloque como éxito de la llamada HTTP en sí.
      if (!resp.ok) {
        const err = new Error(data?.error?.message || 'Error desconocido de Meta API');
    ...
    if (propia && empresa_id) {
      marcarEstadoTokenWhatsapp(empresa_id, false)
        .catch(e => console.error('[NOTIF] Error limpiando flag de reconexión WhatsApp:', e.message));
    }
    return res.status(200).json({ ok: true, message_id: data.messages?.[0]?.id, template, telefono: telefonoLimpio });
  } catch (err) {
    if (err.name === 'CircuitBreakerOpenError') {
      return errorSeguro(res, err, 503, 'WhatsApp no está disponible en este momento.', { retryAfter: err.retryAfterSeconds });
    }
    ...
    if (err.data?.error?.code === 190) {
      await alertarTokenWhatsAppVencido(err.data.error, empresa_id, propia)
        .catch(e => console.error('[NOTIF] Error alertando token WhatsApp vencido:', e.message));
    }
    if (err.status) {
      return res.status(502).json({ error: err.message, code: err.data?.error?.code });
    }
...
const MAX_TURNOS_SIN_CONFIRMAR = 50; // corte defensivo: evita loops largos de tokens de IA por conversación (subido de 8 a 20 el 2026-08-04, y de 20 a 50 el 2026-08-30 — con el criterio de derivar_humano por ambigüedad eliminado, este es ahora el único freno del flujo, así que necesita más margen)

// FIX (2026-08-30): responderConFallback ahora acepta un `maxTokens`
// opcional (ver CONFIG.gemini/groq/openrouter en asistente-providers.js),
// pero acá no se lo estaba pasando — el bot seguía usando el límite de
// 2048/800 tokens pensado para el asistente de AYUDA del admin
// (diagnósticos largos), a pesar de que el propio systemPromptConTools le
// pide respuestas "breves, es un chat, no un email". Este es el límite que
// se manda desde acá: bajo a propósito, porque una respuesta típica del
// bot es un resumen de 2-3 líneas o un pedido de confirmación, nunca un
// texto largo.
const MAX_TOKENS_RESPUESTA_WHATSAPP = 300;

// FIX (2026-08-30, atajo por palabra clave para pedir un humano): hasta acá,
// la única forma de derivar era que el LLM interpretara el mensaje y llamara
// a la tool derivar_humano (ver lib/whatsapp-pedido-tools.js). Eso tiene un
// punto flojo: si el proveedor de turno cae al fallback sin tools
// (systemPromptSinTools, ver procesarConAsistente más abajo) o directamente
// los tres proveedores están caídos, el texto de respuesta le dice al
// cliente "te estoy pasando con un vendedor" pero la tool nunca se ejecuta —
// la conversación no queda en 'derivada_humano' y no se manda ningún push.
// El cliente queda con la sensación de que lo derivaron, pero nadie se
// entera hasta que alguien lee el chat a mano desde el panel.
// Este regex es un atajo determinístico: si el mensaje pide explícitamente
// hablar con una persona, se deriva ACÁ, antes de tocar el LLM, así el
// resultado no depende de qué proveedor esté respondiendo ni de si tiene
// tools disponibles. No reemplaza a derivar_humano (que sigue cubriendo los
// casos ambiguos que el modelo detecta pero que no matchean ninguna palabra
// clave fija) — es una red de seguridad para el caso más común y explícito.
const REGEX_HABLAR_HUMANO = /\b(hablar|comunicarme|conectarme|pasame|pásame)\s+con\s+(un[a]?\s+)?(persona|alguien|vendedor|operador|humano|encargado)\b/i;
...
// libere desde el panel admin", pero esa acción no existe en el panel real
// (tomar/liberar solo asignan `tomada_por` entre vendedores, nunca tocan
// `estado`) — así que una conversación derivada y nunca tomada quedaba
// bloqueando cualquier mensaje futuro de ese cliente para siempre, sin
// importar cuántos días pasaran ni de qué se tratara el mensaje nuevo (caso
// real: un cliente derivado un día antes escribió "Quiero hacer un pedido"
// al día siguiente y recibió el mismo mensaje enlatado de espera).
// Si la conversación derivada lleva más de este umbral sin que nadie la
// haya tomado (`tomada_por IS NULL`), un mensaje nuevo la cierra y arranca
// una conversación fresca en vez de reusarla — ver
// resolverConversacionWhatsapp. Si SÍ la tomó un vendedor, nunca se
// auto-cierra por acá, para no pisarle la charla a alguien que la está
// atendiendo a mano.
const UMBRAL_CONVERSACION_DERIVADA_EXPIRA_HORAS = 12;
...
async function whatsappWebhookHandler(req, res) {
  ...
  // Motor de Integraciones (577_webhooks_recibidos.sql): log genérico +
  // dedupe entre integraciones. Un solo POST de Meta puede traer varios
  // entries/changes (no hay un único wa_message_id a este nivel), así que
  // el evento se identifica por hash del body crudo — si Meta reintenta el
  // mismo POST byte a byte, el segundo insert choca y se corta acá. La
  // idempotencia fina por mensaje individual (UNIQUE wa_message_id) sigue
  // viviendo más abajo, sin cambios.
  let _webhookLogId = null;
  {
    const rawParaHash = req.rawBody instanceof Buffer ? req.rawBody : Buffer.from(JSON.stringify(req.body || {}));
    const eventoExternoId = crypto.createHash('sha1').update(rawParaHash).digest('hex');
    const { id: logId, yaProcesado } = await registrarWebhookEntrante({
      integracion: 'whatsapp',
      eventoExternoId,
      tipo: req.body?.entry?.[0]?.changes?.[0]?.field || (req.body?.history ? 'history_declined' : null),
      payload: req.body,
      headers: { 'x-hub-signature-256': req.headers['x-hub-signature-256'] },
    });
    if (yaProcesado) {
      console.log('[whatsapp-webhook] Evento duplicado (reintento de Meta), ya registrado');
  ...
  try {
    ...
    if (_webhookLogId) await marcarWebhookError(_webhookLogId, err.message);
...
export async function resolverEmpresaCliente(telefono, phoneNumberIdReceptor) {
  ...
  if (phoneNumberIdReceptor) {
    ...
    if (empresaWa) {
      ...
      if (abiertaEnEmpresa) {
        if (abiertaEnEmpresa.cliente_id) {
          return { empresaId: abiertaEnEmpresa.empresa_id, clienteId: abiertaEnEmpresa.cliente_id };
        }
        // FIX (v965): antes, una conversación ya abierta sin cliente_id
        // (típico de un chat importado por el historial/eco de Coexistencia
        // antes de que ese contacto existiera como cliente) quedaba pegada
        // a `null` para siempre — nunca se volvía a intentar el match
        // aunque el cliente se diera de alta después, y el bot no
        // contestaba nunca más ese teléfono. Acá la empresa ya es
        // inequívoca (vino de phoneNumberIdReceptor), así que reintentar
        // el match es seguro (no hay ambigüedad entre empresas como en el
        // camino global de abajo, que sí respeta la decisión #1 de la
        // migración 246 a propósito).
        const clienteRematch = await buscarClientePorTelefonoEnEmpresa(empresaWa.empresa_id, telefono);
        if (clienteRematch) {
          await vincularClienteAConversacion(abiertaEnEmpresa.id, clienteRematch.id);
          return { empresaId: abiertaEnEmpresa.empresa_id, clienteId: clienteRematch.id };
        }
        return { empresaId: abiertaEnEmpresa.empresa_id, clienteId: null };
      }
...
async function resolverConversacionWhatsapp({ telefono, empresaId, clienteId }) {
  ...
  if (existente) {
    // FIX (2026-08-30): ver comentario de UMBRAL_CONVERSACION_DERIVADA_EXPIRA_HORAS.
    // Una conversación derivada, vieja y que nadie tomó no debe seguir
    // bloqueando mensajes nuevos del cliente — se cierra y se arranca una
    // conversación limpia. Si alguien la tomó (tomada_por), se respeta esa
    // asignación y se reusa igual que antes (nunca se le pisa la charla a
    // un vendedor que la está atendiendo a mano).
    const horasDerivada = existente.ultima_interaccion
      ? (Date.now() - new Date(existente.ultima_interaccion).getTime()) / 3_600_000
      : 0;
    const expiro = existente.estado === 'derivada_humano'
      && !existente.tomada_por
      && horasDerivada > UMBRAL_CONVERSACION_DERIVADA_EXPIRA_HORAS;

    if (expiro) {
      await cerrarConversacionPorExpiracion(existente.id);
...
async function registrarMensaje({ conversacionId, direccion, waMessageId, texto, tipo, metadata, toolsUsadas, proveedor }) {
  const { error } = await registrarMensajeWhatsapp({
    ...
    tipo,
    metadata,
    // Migración 604 — opcionales, solo los manda responderYRegistrar()
    // cuando viene de procesarConAsistente() (ver ahí).
    ...(toolsUsadas !== undefined ? { tools_usadas: toolsUsadas } : {}),
    ...(proveedor !== undefined ? { proveedor_usado: proveedor } : {}),
  });
...
export async function procesarMensajeTexto({ telefono: telefonoCrudo, texto, waMessageId, phoneNumberIdReceptor }) {
  ...
  // FIX (v1054 → v1055, Bloque 4, hallazgo del asistente): desde acá hasta
  // el final de la función se lee y potencialmente se escribe
  // pedido_borrador (directo, ej. reiniciarBorradorConversacion, o indirecto
  // vía las tools del asistente dentro de procesarConAsistente). Dos
  // mensajes del mismo cliente llegando en invocaciones del webhook lo
  // bastante seguidas como para solaparse corrían esta sección en paralelo
  // sin ninguna sincronización — la que terminara después podía pisar el
  // borrador que había armado la primera. adquirirLockConversacion (claim
  // atómico sobre whatsapp_conversaciones.procesando_desde, migración 577)
  // serializa esto: si otra invocación ya está adentro, esta espera un poco
  // a que termine antes de leer el borrador, en vez de leer un estado viejo.
  const tomoLock = await adquirirLockConversacion(conversacionId);
  if (!tomoLock) {
    // Se agotó la espera (LOCK_CONVERSACION_TIMEOUT_MS) y el lock seguía
    // tomado: la otra invocación sigue procesando el mensaje anterior. No
    // forzamos una segunda pasada sobre el mismo borrador — el mensaje ya
    // quedó registrado arriba (no se pierde) y el cliente puede seguir en
    // cuanto el otro termine.
    await responderYRegistrar(conversacionId, telefono, empresaId,
      'Un segundo, todavía estoy procesando tu mensaje anterior 🙏');
    return;
  }
  ...
  try {
    ...
    // Atajo por palabra clave (ver comentario de REGEX_HABLAR_HUMANO más
    // arriba): se chequea ANTES que esperando_confirmacion a propósito — si el
    // cliente pide un humano en medio de una confirmación pendiente, gana el
    // pedido de derivación, no el borrador que estaba por confirmarse.
    if (REGEX_HABLAR_HUMANO.test(texto)) {
      await marcarDerivada(conversacionId, 'El cliente pidió hablar con una persona');
      await responderYRegistrar(conversacionId, telefono, empresaId,
        'Dale, ya te paso con alguien de nuestro equipo 🙂');
      return;
    }
    ...
    // Se libera siempre, tanto en un return normal como si algo de arriba
    // tira una excepción (el catch general del webhook la sigue manejando
    // más arriba) — un lock que quedara tomado bloquearía el próximo
    // mensaje de este cliente hasta que expire solo por TTL (2 minutos).
    await liberarLockConversacion(conversacionId);
  }
}

// Extraída de procesarConAsistente() (antes inline) para que
// scripts/eval-asistente-whatsapp.js (Capa 2, PLAN_QA_ASISTENTE_WHATSAPP.md)
// pueda reusar EXACTAMENTE el mismo prompt que producción en vez de
// mantener una copia pegada a mano que se desincroniza en el primer
// cambio de wording — mismo criterio que armarSystemPrompt() del lado
// del asistente de ayuda (handlers/asistente.js), que el eval de esa
// capa ya reusa.
export function armarSystemPromptWhatsApp() {
  ...
  const systemPromptConTools =
    ...
    // FIX (2026-09-08, hallazgo real de scripts/eval-asistente-whatsapp.js,
    // caso buscar-productos-directo-01): antes, ante productos sin stock,
    // el modelo directamente listaba todas las variantes con precio en el
    // mismo mensaje — 6 líneas de "sin stock" con precio que el cliente no
    // pidió ver. Ahora se le pide primero avisar la falta de stock y
    // preguntar si igual quiere ver precios/alternativas, en vez de
    // volcar la lista completa de una.
    'Si buscar_productos devuelve resultados pero NINGUNO tiene stock, no listes todas las variantes ' +
    'con precio de una: avisale primero al cliente que no hay stock de eso y preguntale si igual ' +
    'quiere que le pases precios o alternativas — recién ahí, si dice que sí, mostraselos. Si HAY ' +
    'stock de al menos una variante, mostrala normalmente (podés mencionar las sin stock aparte, sin ' +
    'precio, solo para que sepa que existen). ' +
    'Si el cliente pide varios productos en el mismo mensaje (ej: "2 aceites y 3 harinas"), primero ' +
    'buscá cada uno con buscar_productos y después agregalos TODOS juntos en un solo llamado a ' +
    'agregar_item (un elemento por producto), en vez de llamarla una vez por producto. ' +
    // FIX (2026-09-08, hallazgo real del mismo eval, caso
    // agregar-item-simple-01): antes, tras llamar agregar_item, el modelo
    // podía responder solo "Listo, anotado" sin decir qué producto ni
    // cuántas unidades — si el modelo había entendido mal a qué producto
    // se refería "esos" (ej. tras varios turnos), el cliente no tenía
    // forma de darse cuenta antes de la confirmación final.
    'Después de llamar a agregar_item (o modificar_cantidad), tu respuesta en texto SIEMPRE tiene que ' +
    'nombrar el producto y la cantidad que quedó (ej: "Listo, agregué 2x Harina Leudante 000 1kg. ' +
    '¿Algo más?") — nunca confirmes solo con un "listo"/"anotado" genérico sin decir qué se agregó. ' +
    'Si no se pudo confirmar un pedido por falta de stock de algún producto y el cliente quiere ' +
    'seguir con una cantidad menor (ej: "dejalo en 3", "poné 2 nomás"), usá modificar_cantidad con ' +
    'esa cantidad exacta — no hace falta quitar el producto y volver a agregarlo. Si el cliente no ' +
    'aclaró una cantidad concreta, preguntale explícitamente cuántas unidades quiere dejar antes de ' +
    'llamar a ninguna tool. ' +
  ...
  return { systemPromptConTools, systemPromptSinTools };
}
...
async function procesarConAsistente({ conversacionId, empresaId, clienteId, telefono, texto }) {
  ...
  let texto_respuesta;
  let usoAsistente; // { toolsUsadas, proveedor } — migración 604, ver responderYRegistrar()
  try {
    const resultado = await responderConFallback({
      systemPromptConTools, systemPromptSinTools, historial, mensaje: texto, tools,
      maxTokens: MAX_TOKENS_RESPUESTA_WHATSAPP,
    });
    texto_respuesta = resultado.texto;
    // FIX (2026-08-30, observabilidad): responderConFallback ya venía
    // devolviendo `proveedor` y `tokens` (consumo acumulado de la
    // pregunta, incluidas las vueltas de tools) sin que nadie los mirara.
    // Se loguea acá para poder grepear logs de Vercel si hace falta
    // ajustar MAX_TOKENS_RESPUESTA_WHATSAPP o diagnosticar consumo alto en
    // una empresa puntual.
    console.log(
      `[whatsapp-webhook] respuesta de ${resultado.proveedor} — tokens: ` +
      `prompt=${resultado.tokens?.prompt ?? '?'} completion=${resultado.tokens?.completion ?? '?'} ` +
      `total=${resultado.tokens?.total ?? '?'} (conversacion=${conversacionId})`
    );
    // Migración 604 (Capa 3, PLAN_QA_ASISTENTE_WHATSAPP.md): antes esto se
    // descartaba después del console.log de arriba. Se persiste junto con
    // el mensaje saliente para que detectar-fallas-asistente-whatsapp.js
    // pueda cruzar mensaje de cliente ↔ tools llamadas ↔ proveedor.
  ...
  await responderYRegistrar(conversacionId, telefono, empresaId, texto_respuesta, usoAsistente);
}
...
// `usoAsistente` (5º parámetro, opcional) — { toolsUsadas, proveedor } —
// migración 604. Solo lo manda procesarConAsistente(); los ~9 call sites
// restantes (avisos fijos, derivación a humano, confirmación de pedido)
// siguen llamando con 4 argumentos sin cambios, `usoAsistente` queda
// `undefined` y registrarMensaje() no manda esos campos (DEFAULT de la
// migración: '[]'/NULL).
async function responderYRegistrar(conversacionId, telefono, empresaId, texto, usoAsistente) {
  const envio = await enviarTextoWhatsApp(telefono, texto, empresaId);
  const metadata = envio
    ? { estado_envio: 'enviado' }
    : { estado_envio: 'pendiente', intentos: 0 };
  await registrarMensaje({
    conversacionId, direccion: 'out', waMessageId: envio?.message_id, texto, tipo: 'text', metadata,
    toolsUsadas: usoAsistente?.toolsUsadas, proveedor: usoAsistente?.proveedor,
  });
}
...
// FIX (2026-08-30, UUID crudo al cliente): la RPC crear_pedido_cliente hace
// un chequeo de stock ATÓMICO justo antes de reservar — red de seguridad
// contra la condición de carrera de que el stock haya cambiado entre el
// chequeo previo en JS (más arriba en crearPedidoDesdeItemsWhatsapp, que sí
// resuelve nombres) y el momento real de la reserva. Ese chequeo de la RPC
// es deliberadamente bare-bones (`'Stock insuficiente para producto ' ||
// producto_id`, sin nombre — ver las migraciones de crear_pedido_cliente),
// porque en el resto de la app (portal/admin) esa rama casi nunca se
// alcanza gracias al pre-chequeo en JS. Acá SÍ se estaba alcanzando en
// producción (reportado por un cliente real vía WhatsApp) y el UUID crudo
// llegaba tal cual al chat del cliente. Se resuelve acá, reemplazando
// "producto <uuid>" por el nombre que ya tenemos en `items` (vino del
// borrador armado por agregar_item, que siempre guarda `nombre`) — mismo
// estilo de mensaje que el pre-chequeo de más arriba (`stock insuficiente
// para "<nombre>"`).
const REGEX_PRODUCTO_UUID_EN_ERROR = /producto\s+([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})/i;

function resolverNombreEnErrorStock(mensajeCrudo, items) {
  if (!mensajeCrudo) return mensajeCrudo;
  return mensajeCrudo.replace(REGEX_PRODUCTO_UUID_EN_ERROR, (match, productoId) => {
    const nombre = items.find((i) => i.producto_id === productoId)?.nombre;
    return nombre ? `"${nombre}"` : match; // si no lo encontramos en items, se deja el texto original (nunca peor que antes)
  });
}
...
export async function crearPedidoDesdeItemsWhatsapp({ empresaId, clienteId, items }) {
  ...
  // Multi-depósito (550): mismo criterio que el admin y el portal —
  // sucursal fija del cliente (clientes.deposito_id) > depósito principal.
  const depositoId = await resolverDepositoParaPedido({
    empresaId,
    clienteDepositoId: clienteRow.deposito_id,
  });
  if (!depositoId) return { ok: false, error: 'la empresa no tiene ningún depósito activo configurado' };
  ...
  if (!rpcResult?.ok) {
    return { ok: false, error: resolverNombreEnErrorStock(rpcResult?.error, items) || 'stock insuficiente' };
  }
...
async function enviarTextoWhatsApp(telefono, texto, empresaId) {
  ...
  // Mismo tope de plan que whatsappHandler (ver comentario ahí) — el
  // texto libre del bot cuenta contra el mismo cupo de 10 mensajes de
  // trial, porque chequear_limite_plan('whatsapp_mensajes') cuenta TODAS
  // las filas direccion='out' de whatsapp_mensajes, sin distinguir
  // template vs. texto libre.
  try {
    await exigirLimitePlan(supabase, empresaId, 'whatsapp_mensajes');
  } catch (err) {
    if (err instanceof LimitePlanError) return null;
    throw err;
  }
  ...
  try {
    // Mismo breaker compartido que whatsappHandler() — ver comentario junto
    // a la declaración de waBreaker más arriba.
    const { data } = await waBreaker.exec(async () => {
    ...
    if (propia && empresaId) {
      marcarEstadoTokenWhatsapp(empresaId, false)
        .catch(e => console.error('[NOTIF] Error limpiando flag de reconexión WhatsApp:', e.message));
    }
    return { message_id: data.messages?.[0]?.id };
  } catch (err) {
    if (err.name === 'CircuitBreakerOpenError') {
      console.warn(`[whatsapp-webhook] ${err.message} — no se reintenta.`);
      return null;
    }
    if (err.data?.error?.code === 190) {
      await alertarTokenWhatsAppVencido(err.data.error, empresaId, propia)
        .catch(e => console.error('[NOTIF] Error alertando token WhatsApp vencido:', e.message));
    }
...
export async function whatsappEmbeddedSignupHandler(req, res) {
  ...
  const { code, waba_id, business_id } = req.body || {};
  ...
  try {
    ...
    // business_id (migración 544): solo se agrega al payload si esta vez
    // Meta lo mandó (pasó por la pantalla de negocio) — si vino null porque
    // el frontend ya lo prefilleó y Meta saltó esa pantalla, NO se pisa el
    // que ya está guardado en la fila (el upsert de Supabase solo toca las
    ...
    if (business_id) credenciales.business_id = business_id;

    const { error: dbError } = await guardarCredencialesWhatsapp(credenciales);
...
// Desconexión manual desde el panel — botón "Desconectar" en
// /admin/whatsapp-onboarding, visible solo cuando la empresa ya tiene un
// número propio conectado (ver whatsapp-onboarding.js). A diferencia del
// offboarding que Meta avisa por webhook (ACCOUNT_OFFBOARDED, ver
// actualizarEstadoConexionWhatsapp en whatsapp-bot.js), esto es una
// decisión explícita del dueño/admin de dejar de usar Embedded Signup en
// distrib — el número de WhatsApp Business en sí NO se toca, solo se le
// avisa a Meta que esta app deja de recibir sus webhooks y se borra la
// fila local. La empresa vuelve a usar el número de prueba global hasta
// que reconecte.
export async function whatsappDesconectarHandler(req, res) {
  if (req.method === 'OPTIONS') {
    res.setHeader('Access-Control-Allow-Origin', '*');
    res.setHeader('Access-Control-Allow-Methods', 'POST, OPTIONS');
    res.setHeader('Access-Control-Allow-Headers', 'Content-Type, Authorization');
  ...
  const perfil = await verificarToken(req, supabase);
  if (!perfil) return res.status(401).json({ error: 'No autorizado' });
  if (!puede(perfil, 'desconectar', 'whatsapp_onboarding')) {
    return res.status(403).json({ error: 'Solo el dueño o un admin puede desconectar el WhatsApp de la empresa' });
  }
  if (!perfil.empresa_id) return res.status(400).json({ error: 'Tu usuario no tiene una empresa asociada' });

  try {
    const { data: cred, error: credError } = await obtenerWabaIdYTokenWhatsapp(perfil.empresa_id);
    if (credError) return errorSeguro(res, credError, 500, 'No se pudo leer la conexión actual.');
    if (!cred) {
      // Ya no había nada conectado (ej. dos tabs, o el usuario tocó el
      // botón dos veces) — no es un error, el resultado final es el mismo.
      return res.status(200).json({ ok: true, ya_estaba_desconectado: true });
    }

    // Avisarle a Meta que esta app deja de estar suscripta a los webhooks
    // de esa cuenta. No es fatal si falla (ej. el token ya venció, o el
    // dueño ya había desconectado el número desde el celular): igual
    // borramos la fila local abajo, que es lo que controla el panel.
    try {
      const accessToken = descifrar(cred.access_token);
      const unsubscribeResp = await fetch(`${META_BASE_URL}/${cred.waba_id}/subscribed_apps`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${accessToken}` },
      });
      const unsubscribeData = await unsubscribeResp.json();
      if (!unsubscribeResp.ok) {
        console.error('[whatsapp-desconectar] No se pudo desuscribir de los webhooks en Meta (se borra igual la fila local):', unsubscribeData?.error);
      }
    } catch (err) {
      console.error('[whatsapp-desconectar] Error avisando a Meta (se borra igual la fila local):', err.message);
    }

    const { error: dbError } = await borrarCredencialesWhatsapp(perfil.empresa_id);
    if (dbError) return errorSeguro(res, dbError, 500, 'No se pudo desconectar el WhatsApp. Probá de nuevo.');

    return res.status(200).json({ ok: true });
  } catch (err) {
    console.error('[whatsapp-desconectar] Error inesperado:', err.message);
    return res.status(500).json({ error: 'No se pudo desconectar el WhatsApp' });
  }
}
...
async function enviarNotifPedido(pedido, evento, motivo, etaMinutos) {
  ...
  // FIX (retomando PLAN_ROBUSTEZ...2026, sep 2026): ver comentario en
  // enviarAvisoDeudaVencida — mismo criterio que enviarAvisoChequesPorVencer.
  if (!waEndpointConfigurado()) {
    return { ok: false, razon: 'wa_endpoint_no_configurado', cliente: cliente.razon_social };
  }
  try {
    ...
    const respuesta = await leerRespuestaWa(waResp);
    if (!respuesta.esJson) return { ok: false, razon: 'error_wa', detalle: respuesta.motivo, cliente: cliente.razon_social };
    const waData = respuesta.data;
...
// 548/549: mapea `tipo` (push interno vía trigger SQL) → columna de
// notif_prefs_auto que lo habilita/deshabilita. A diferencia de notifAuto()
// (_auto-push.js), este handler no consultaba notif_prefs_auto en absoluto
// — los tipos acá arriba (stock_critico, nuevo_pedido, whatsapp_estancado)
// disparaban siempre, sin que el dueño pudiera apagarlos desde
// /admin/automatizacion. La 548 cerró stock_critico; la 549 cierra
// nuevo_pedido y whatsapp_estancado con el mismo patrón — ahora los 3
// tipos de este handler están cubiertos.
const PREF_POR_TIPO_PUSH_INTERNO = {
  stock_critico: 'stock_critico_bajo',
  nuevo_pedido: 'pedido_nuevo_recibido',
  whatsapp_estancado: 'whatsapp_conversacion_estancada',
};
...
async function pushInternoHandler(req, res) {
  ...
  try {
    // 548: si este tipo tiene una preferencia asociada y la empresa la tiene
    // apagada, cortar acá — mismo criterio que notifAuto() para los tipos
    // del motor JS. `pref` viene null si la empresa nunca tocó el toggle
    // (fila con default TRUE en la columna), así que solo cortamos con
    // === false explícito.
    const columnaPref = PREF_POR_TIPO_PUSH_INTERNO[tipo];
    if (columnaPref) {
      const pref = await obtenerPrefsAuto(empresa_id, columnaPref);
      if (pref && pref[columnaPref] === false) {
        return res.status(200).json({ ok: true, enviadas: 0, motivo: 'tipo_deshabilitado' });
      }
    }
    ...
    // FIX (loop de timeouts/AbortError en push-interno): antes este for-loop
    // esperaba a enviarPush() de a un usuario por vez — cada uno con su
    // propio round-trip a Postgres (rate-limit interno + lookup de tokens)
    // y a Firebase. Con varios destinatarios, la suma de esas esperas
    // secuenciales se comía el timeout de 10s del fetch de Supabase
    // (lib/supabase-fetch-timeout.js) y el request se abortaba a mitad de
    // camino (AbortError), justo lo que apareció en los logs de prod bajo
    // ráfagas de triggers (stock/pedidos cambiando varias filas de golpe).
    // Ahora se dispara todo en paralelo con allSettled: mismo trabajo total,
    // pero el tiempo de respuesta pasa de "N × latencia" a "~1 × latencia".
    const resultados = await Promise.allSettled(
      usuarios.map((usuario) =>
        enviarPush(usuario.id, titulo, cuerpo, datos || {}, { empresa_id, tipo })
      )
    );
    let totalEnviadas = 0;
    for (const r of resultados) {
      if (r.status === 'fulfilled') totalEnviadas += r.value?.enviadas || 0;
      else console.error('[PUSH-INTERNO] Fallo enviando a un destinatario:', r.reason?.message || r.reason);
    }
...
// ══════════════════════════════════════════════════════════════════════════
// ── Reproceso de webhooks entrantes con error CRON (Motor de Integraciones,
//    577_webhooks_recibidos.sql) ────────────────────────────────────────
// ══════════════════════════════════════════════════════════════════════════
// Mismo esquema de auth y mismo criterio de "1 corrida diaria" que los tres
// crons de arriba (plan Hobby de Vercel no permite crons más frecuentes).
//
// Cubre las dos integraciones con webhook entrante (WhatsApp y Mercado
// Pago). Para MP usa procesarEventoMP (lib/handlers/pagos.js), la lógica
// de negocio extraída de manejarWebhook — separada de verificarFirmaMP a
// propósito, porque acá no hay una firma HMAC nueva que validar: el evento
// ya se registró con firma_valida=true en la recepción original, y
// reprocesar es correr la MISMA lógica sobre el MISMO payload guardado.
// Import dinámico (no estático) para no crear una dependencia circular a
// nivel de módulo entre notif.js y pagos.js — mismo criterio que ya usa
// este archivo con eventos-dispatcher.js más arriba.
async function handleWebhooksReprocesarCron(req, res) {
  if (!['GET', 'POST'].includes(req.method)) return res.status(405).json({ error: 'Método no permitido' });
  ...
  try {
    const { listarWebhooksParaReintentar, marcarWebhookError } = await import('../repos/webhooks.js');
    const { db } = await import('../repos/_db.js');
    const { procesarEventoMP } = await import('./pagos.js');

    const resultado = { whatsapp: { evaluados: 0, reprocesados: 0, con_error: 0 }, mercadopago: { evaluados: 0, reprocesados: 0, con_error: 0 } };

    const marcarOk = (id) => db.from('webhooks_recibidos').update({ estado: 'procesado', actualizado_at: new Date().toISOString() }).eq('id', id);

    // ── WhatsApp ──
    const pendientesWa = await listarWebhooksParaReintentar({ integracion: 'whatsapp', maxIntentos: 5, limite: 50 });
    resultado.whatsapp.evaluados = pendientesWa.length;
    for (const evento of pendientesWa) {
      try {
        if (evento.payload?.entry) {
          for (const entry of evento.payload.entry) {
            for (const change of (entry?.changes || [])) {
              await procesarCambioWebhookWhatsapp(entry, change);
            }
          }
        } else if (evento.payload?.history) {
          await procesarHistorialDeclinado(evento.payload);
        }
        await marcarOk(evento.id);
        resultado.whatsapp.reprocesados++;
      } catch (err) {
        await marcarWebhookError(evento.id, `Reintento del cron falló: ${err.message}`);
        resultado.whatsapp.con_error++;
      }
    }

    // ── Mercado Pago ──
    const pendientesMp = await listarWebhooksParaReintentar({ integracion: 'mercadopago', maxIntentos: 5, limite: 50 });
    resultado.mercadopago.evaluados = pendientesMp.length;
    for (const evento of pendientesMp) {
      try {
        const respuesta = await procesarEventoMP(evento.payload);
        // status >= 500 (o 502, el caso "no se pudo verificar el pago" con
        // el circuit breaker/MP caído) sigue siendo un fallo real — no lo
        // marcamos 'procesado' aunque procesarEventoMP no haya tirado
        // excepción, porque esa función devuelve el status en vez de
        // lanzar para la mayoría de sus casos de error.
        if (respuesta.status >= 500) {
          await marcarWebhookError(evento.id, `Reintento del cron: status ${respuesta.status} — ${JSON.stringify(respuesta.body)}`);
          resultado.mercadopago.con_error++;
        } else {
          await marcarOk(evento.id);
          resultado.mercadopago.reprocesados++;
        }
      } catch (err) {
        await marcarWebhookError(evento.id, `Reintento del cron falló: ${err.message}`);
        resultado.mercadopago.con_error++;
    ...
    return errorSeguro(res, err, 500, 'No se pudo completar el reproceso de webhooks.');
  }
}
...
export async function enviarAvisoDeudaVencida({ clienteId, empresaId, telefono, razonSocial, saldoVencido }) {
  // FIX (retomando PLAN_ROBUSTEZ...2026, sep 2026): mismo criterio que ya
  // usaba enviarAvisoChequesPorVencer — sin WA_ENDPOINT configurada, no
  // tiene sentido pegarle al fallback de localhost en producción.
  if (!waEndpointConfigurado()) {
    return { ok: false, motivo: 'WA_ENDPOINT no configurado' };
  }
  try {
...
// ── Fase 2 de PLAN_CLIENTES_EN_FUGA.md, evento `cliente_en_riesgo_fuga` ──
//
// A diferencia de cliente_en_mora/cheques_por_vencer (Fase 3/4 del plan
// ERP, que migraban un camino directo ya existente y necesitaban el
// expand-contract del flag fase3_despachador_eventos), este es un evento
// nuevo desde el día uno — no hay ningún camino directo previo que
// preservar. handleFugaCron solo detecta, aplica cooldown y emite; toda la
// decisión de "WhatsApp automático vs tarea para el vendedor vs tarea de
// cobro" vive en el listener (lib/eventos-listeners/cliente_en_riesgo_fuga.js),
// que es quien llama a enviarRecuperacionFuga de acá.

const FUGA_WA_ENDPOINT = process.env.WA_ENDPOINT || 'http://localhost:3000/api/notif/whatsapp';
const DIAS_COOLDOWN_FUGA = 15; // "no mandar el mismo aviso todos los días" — nota 1.3/2.3 del plan.

// enviarRecuperacionFuga(): mismo criterio que enviarAvisoDeudaVencida —
// función reusable, exportada para que la use el listener del despachador
// sin duplicar la llamada a Meta + el registro en notif_log. Solo se
// invoca para clientes chico/mediano con motivo_probable
// 'posible_fuga_a_competencia' — el listener es quien filtra eso antes de
// llamar acá.
export async function enviarRecuperacionFuga({ clienteId, empresaId, telefono, razonSocial, productoPrincipal }) {
  // FIX (retomando PLAN_ROBUSTEZ...2026, sep 2026): ver comentario en
  // enviarAvisoDeudaVencida — mismo criterio que enviarAvisoChequesPorVencer.
  if (!waEndpointConfigurado()) {
    return { ok: false, motivo: 'WA_ENDPOINT no configurado' };
  ...
  try {
    ...
    try {
      const { data: saldo } = await obtenerSaldoPuntos(empresaId, clienteId);
      puntosDisponibles = Math.trunc(saldo?.puntos_disponibles || 0);
    } catch (err) {
      // Sin fidelización activa para esta empresa/cliente no corta el
      // envío — el mensaje simplemente sale con 0 puntos.
      console.error('[fuga] Error leyendo saldo de puntos (no crítico):', err.message);
    }

    const waResp = await fetch(FUGA_WA_ENDPOINT, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        template: 'recuperacion_cliente',
        telefono,
        params: {
          nombre_cliente: _primerNombreDeuda(razonSocial),
          producto_principal: productoPrincipal || 'tu pedido de siempre',
    ...
    await registrarLog({
      cliente_id: clienteId, empresa_id: empresaId, tipo: 'cliente_en_riesgo_fuga',
      canal: 'whatsapp', telefono, message_id: waData.message_id || null,
      payload: { producto_principal: productoPrincipal, puntos_disponibles: puntosDisponibles },
    });
    return { ok: true };
  } catch (waErr) {
    return { ok: false, motivo: `error red — ${waErr.message}` };
...
async function handleFugaCron(req, res) {
  if (!['GET', 'POST'].includes(req.method)) return res.status(405).json({ error: 'Método no permitido' });
  ...
  const resultados = { empresas_procesadas: 0, eventos_emitidos: 0, omitidos: 0, errores: 0, detalle: [] };
  ...
  try {
    ...
    for (const { id: empresaId } of empresas) {
      resultados.empresas_procesadas++;
      ...
      try {
        ...
        resultados.errores++;
        resultados.detalle.push({ empresa: empresaId, resultado: `error fn_clientes_en_fuga — ${err.message}` });
        continue;
      }

      const clientes = deteccion?.clientes || [];
      if (!clientes.length) continue;

      for (const c of clientes) {
        // ultimoAvisoFuga (no ultimoEnvioPorCliente): el cooldown tiene que
        // cubrir también las tareas de vendedor de los caminos 1/2 del
        // listener (deuda / cliente grande), que no dejan rastro en
        // notif_log — ver comentario en repos/clientes-fuga.js.
        const ultimoAviso = await ultimoAvisoFuga(c.cliente_id);
        if (ultimoAviso) {
          const haceHoras = (hoy - new Date(ultimoAviso)) / 1000 / 3600;
          if (haceHoras < DIAS_COOLDOWN_FUGA * 24) {
            resultados.omitidos++;
            continue;
          }
        }
        ...
        try {
          await emitirEvento({
            empresaId,
            tipoEvento: 'cliente_en_riesgo_fuga',
            payload: {
              cliente_id: c.cliente_id,
              dias_atraso: c.dias_atraso,
              producto_principal: c.producto_principal,
              valor_anual_estimado: c.valor_anual_estimado,
              motivo_probable: c.motivo_probable,
              vendedor_id_default: c.vendedor_id_default,
            },
            origen: 'handleFugaCron',
          });
          resultados.eventos_emitidos++;
        } catch (err) {
          resultados.errores++;
          resultados.detalle.push({ cliente: c.razon_social, resultado: `error emitiendo evento — ${err.message}` });
        }
      }

      // Evento nuevo desde el día uno: siempre vía despachador, sin el
      // expand-contract de fase3_despachador_eventos que sí necesitan
      // cliente_en_mora/cheques_por_vencer (esos migraban un camino
      // directo preexistente; acá no hay nada previo que preservar).
      const { despacharPendientes } = await import('../eventos-dispatcher.js');
      const resultadoDespacho = await despacharPendientes({ empresaId });
      if (!resultadoDespacho.ok) {
        resultados.errores++;
        resultados.detalle.push({ empresa: empresaId, resultado: 'error en el despachador de eventos (ver eventos_negocio)' });
      }
    }
