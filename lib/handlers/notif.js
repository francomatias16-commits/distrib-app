...
async function resolverEmpresaCliente(telefono, phoneNumberIdReceptor) {
  // Si ya hay una conversación abierta con este teléfono, se respeta esa
  // empresa/cliente (evita que un match nuevo "salte" de empresa a mitad
  // de una conversación en curso — ver decisión #1 de la migración 246).
  ...
  // Etapa 7: si el mensaje llegó a un número que una empresa conectó por
  // Embedded Signup, la empresa queda resuelta sin ambigüedad — no hace
  // falta ni conviene pasar por el matching genérico de abajo (que podría
  // devolver otra empresa si el mismo teléfono de cliente existe en más
  // de una). El cliente sí se sigue resolviendo por teléfono, pero
  // acotado a ESA empresa.
...
async function resolverConversacionWhatsapp({ telefono, empresaId, clienteId }) {
  const existente = await buscarConversacionAbiertaId(telefono);
...
export async function crearPedidoDesdeItemsWhatsapp({ empresaId, clienteId, items }) {
  ...
  const stockData = await obtenerStockParaPedidoWhatsapp(productoIds);
  const stockMap = {};
  ...
  for (const item of items) {
    if (item.cantidad > (stockMap[item.producto_id] ?? 0)) {
  ...
  const { subtotal, iva_total, total, itemsParaRpc } = calcularTotalesPedido(items, {
    resolverPrecio: (item) => precioMap[item.producto_id] ?? prodsData.find((p) => p.id === item.producto_id)?.precio_base ?? 0,
    ivaMap,
  });
...
const REINTENTOS_TRANSITORIO = 2;
...
async function enviarTextoWhatsApp(telefono, texto, empresaId) {
  ...
  try {
    ...
    for (let intento = 1; !ok && esTransitorio(status) && intento <= REINTENTOS_TRANSITORIO; intento++) {
      console.warn(`[whatsapp-webhook] Falla transitoria (status ${status}) enviando a ${telefono} — reintento ${intento}/${REINTENTOS_TRANSITORIO}`);
...
async function pushInternoHandler(req, res) {
  ...
  //
  // Estrategia de transición segura (fail-closed siempre):
  //   - Si INTERNAL_PUSH_SECRET está configurada en Vercel → se exige ese
  //     valor en el header x-push-secret. El header x-trigger:supabase ya
  //     no alcanza.
  //   - Si INTERNAL_PUSH_SECRET NO está configurada todavía → se acepta el
  //     header legacy x-trigger:supabase como fallback transitorio, pero se
  //     loguea una advertencia de seguridad para que aparezca en los logs de
  //     Vercel y el operador pueda actuar.
  //   - Si ninguno de los dos pasa → 401 siempre (fail-closed).
  //
  // Para activar el secreto real:
  ...
  //      que actualiza el trigger de Supabase para enviar ese valor en
  //      x-push-secret en vez del header x-trigger:supabase.
  //   4. Una vez confirmado que los triggers pasan el nuevo header, podés
  //      eliminar el bloque de fallback legacy de abajo.
  const pushSecret = process.env.INTERNAL_PUSH_SECRET;

  if (pushSecret) {
    // Modo seguro: exigir el secreto real
    if (req.headers['x-push-secret'] !== pushSecret) {
      return res.status(401).json({ error: 'No autorizado' });
    }
  } else {
    // Modo transitorio: aceptar header legacy mientras no se configura el secreto
    console.warn('[SECURITY] pushInternoHandler: INTERNAL_PUSH_SECRET no configurada — usando fallback legacy x-trigger:supabase. Configurá la variable y corré la migración SQL para activar el secreto real.');
    if (req.headers['x-trigger'] !== 'supabase') {
      return res.status(401).json({ error: 'No autorizado' });
    }
  }
