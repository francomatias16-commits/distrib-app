...
//
// Extraída como función pura (2026-09-08, hallazgos de
// scripts/eval-asistente-whatsapp.js contra Gemini, ver
// PLAN_QA_ASISTENTE_WHATSAPP.md sección 8) para poder testear el texto del
// prompt sin mockear nada — mismo criterio que armarSystemPrompt() en
// handlers/asistente.js.
export function armarSystemPromptWhatsApp() {
  const systemPromptConTools =
    ...
    'nombrar el producto y la cantidad que quedó (ej: "Sumé 3 aceites de girasol al pedido") — ' +
    'nunca confirmes solo con un "listo"/"anotado" genérico, así el cliente puede corregirte si ' +
    'entendiste mal a qué producto se refería. ' +
...
async function procesarConAsistente({ conversacionId, empresaId, clienteId, telefono, texto }) {
  ...
  try {
    ...
    // Se loguea acá — no hay todavía una tabla/dashboard para esto, así
    // que por ahora es solo para poder grepear logs de Vercel si hace
    // falta ajustar MAX_TOKENS_RESPUESTA_WHATSAPP o diagnosticar consumo
    // alto en una empresa puntual.
    console.log(
...
async function pushInternoHandler(req, res) {
  ...
  try {
    ...
    // FIX ("campanita no avisa hasta 24hs"): pushInternoHandler solo dejaba
    // rastro en notif_log (auditoría de envío por canal), nunca en
    // `notificaciones_push` — la tabla que lee de verdad la campanita del
    // topbar (AdminRepo.obtenerNotificacionesRecientes). Por eso un pedido
    // nuevo recién aparecía en la campanita vía la categoría "pedido sin
    // despachar" (obtenerPedidosDemorados), que solo mira pedidos de más de
    // 24hs — no había ningún camino para verlo apenas entra. Se inserta acá,
    // en paralelo al envío de push (no depende de que existan tokens ni de
    // que el usuario haya aceptado el permiso del navegador), para que
    // nuevo_pedido/stock_critico/whatsapp_estancado aparezcan de inmediato.
    insertarNotificacionPush({ empresa_id, titulo, cuerpo, tipo, datos_json: datos || null }).catch(() => {});
...
async function pushChoferHandler(req, res) {
  ...
  const perfil = await verificarToken(req, supabase);
  if (!perfil) return res.status(401).json({ error: 'No autorizado' });
  // Migrado a PermisosService (B3 del plan de cierre) — ver
  // lib/permisos-service.js (recurso `notif_push_chofer`). Antes era un
  // array `['dueno','admin']` inline sin nombre, sin cambio de comportamiento.
  if (!puede(perfil, 'enviar', 'notif_push_chofer')) return res.status(403).json({ error: 'Sin permisos' });
...
async function handleReintentarEmail(req, res) {
  ...
  // Migrado a PermisosService (B3 del plan de cierre) — ver
  // lib/permisos-service.js (recurso `notif_reintentar_email`). Antes era
  // un array `['dueno','admin']` inline sin nombre, sin cambio de comportamiento.
  if (!puede(perfil, 'reintentar', 'notif_reintentar_email')) {
