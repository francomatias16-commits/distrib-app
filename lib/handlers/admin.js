...
// FIX (2026-07-12, incidente dashboard colgado) — actualizado v862:
// supabase.auth.getUser() podía colgarse sin tirar error, comiéndose los
// 60s completos del límite de Vercel. En su momento se parchó ACÁ, puntual,
// con este Promise.race de 8s. v862 generalizó el fix de raíz: el `fetch`
// interno de TODOS los clients de Supabase del proyecto (Auth y PostgREST
// por igual) ahora tiene timeout propio (10s, ver
// lib/supabase-fetch-timeout.js), así que esto ya no es la única red de
// contención. Se deja como capa extra (mensaje 503 más claro que un error
// de red genérico, y corta 2s antes que el timeout del fetch) — no hace
// falta tocarlo, pero ya no es indispensable como lo era antes.
