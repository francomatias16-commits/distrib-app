...
// FIX (2026-07-12, incidente dashboard colgado): supabase.auth.getUser() es
// la ÚNICA llamada de red en todo /api/admin/* que va contra el servicio de
// Auth de Supabase (GoTrue) en vez de PostgREST — piloto.js/score.js no la
// usan (verifican un JWT local, sin red). Confirmado en producción: esta
// llamada puede colgarse sin tirar error, comiéndose los 60s completos del
// límite de Vercel para TODO /api/admin/*, mientras el resto de los módulos
// (que no llaman a Auth) responden normal en la misma lambda ya caliente.
// No podemos hacer fail-open acá (dejar pasar sin autenticar es un agujero
// de seguridad) pero sí fail-FAST: si Auth no contesta en 8s, se corta con
// un 503 claro en vez de colgar hasta que Vercel mate la función a los 60s.
async function getUserConTimeout(token, timeoutMs = 8000) {
  const timeout = new Promise((resolve) =>
    setTimeout(() => resolve({ timedOut: true }), timeoutMs)
  );
  const consulta = supabase.auth.getUser(token).then(r => ({ timedOut: false, ...r }));
...
async function autenticar(req) {
  ...
  const resultado = await getUserConTimeout(token);
  if (resultado.timedOut) {
    console.error('[admin] Timeout consultando Supabase Auth (auth.getUser), se corta rápido en vez de colgar');
    return { error: 'El servicio de autenticación tardó demasiado en responder. Reintentá en unos segundos.', status: 503 };
  }

  const { data: { user }, error: authErr } = resultado;
...
async function handleKPIs(req, res, empresa_id) {
  ...
  if (error) {
    ...
    if (v2.error) {
      ...
      if (fallback.error) return errorSeguro(res, error, 500, 'No se pudo completar la operación.', { error: fallback.error.message });
      kpis = fallback.data;
    } else {
      kpis = v2.data;
    }
...
async function handleStockBajo(req, res, empresa_id) {
  ...
  // Obtener depósitos de la empresa
  const depositos = await AdminRepo.obtenerDepositosIds(empresa_id);

  const depIds = (depositos || []).map(d => d.id);
  if (depIds.length === 0) return res.json({ items: [] });

  // Stock agrupado por producto
  const { data: stocks, error } = await AdminRepo.obtenerStockConProductos(depIds);

  if (error) return errorSeguro(res, error, 500, 'No se pudo completar la operación.');

  // Agrupar por producto y sumar cantidades de todos los depósitos
  const mapa = new Map();
  for (const s of stocks || []) {
    const p = s.productos;
    if (!p || !p.activo) continue;
    const existing = mapa.get(s.producto_id) || {
    ...
    existing.cantidad_total     += (+s.cantidad || 0);
    existing.cantidad_reservada += (+s.cantidad_reservada || 0);
    mapa.set(s.producto_id, existing);
  }
  ...
  const items = [...mapa.values()]
    .map(p => ({
      ...p,
      cantidad_disponible: Math.max(0, p.cantidad_total - p.cantidad_reservada),
    }))
    .filter(p => {
      const umbral = p.stock_minimo > 0 ? p.stock_minimo : umbralDefault;
      return p.cantidad_disponible < umbral;
    })
...
async function handleResumenArranque(req, res, empresa_id) {
  ...
  const [depositosRes, rutasRes, bloqueosRes, scoreCriticoRes] = await Promise.allSettled([
    AdminRepo.obtenerDepositosIds(empresa_id),
  ...
  if (depositosRes.status === 'fulfilled') {
    const depIds = (depositosRes.value || []).map(d => d.id);
    if (depIds.length > 0) {
      const stocks = await AdminRepo.obtenerStockValorizado(depIds);
      ...
      for (const s of stocks || []) {
        ...
        const disp = Math.max(0, (+s.cantidad || 0) - (+s.cantidad_reservada || 0));
        existing.disponible += disp;
        existing.valorizado += disp * (+s.costo_promedio || 0);
        mapa.set(s.producto_id, existing);
      }

      const umbralDefault = 5;
      let unidades = 0, valorizado = 0, conStock = 0, critico = 0;
      for (const item of mapa.values()) {
        unidades   += item.disponible;
        valorizado += item.valorizado;
        if (item.disponible > 0) conStock++;
        const umbral = item.stock_minimo > 0 ? item.stock_minimo : umbralDefault;
        if (item.disponible < umbral) critico++;
      }
      stockOut = {
        unidades_disponibles:   Math.round(unidades),
        valorizado_disponible:  Math.round(valorizado * 100) / 100,
        productos_con_stock:    conStock,
        stock_critico_count:    critico,
      };
    }
  } else {
    console.error('[Dashboard] resumen-arranque depositos:', depositosRes.reason);
  }
...
async function handleAlertas(req, res, empresa_id) {
  ...
  for (const s of sesionesConError || []) {
    alertas.push({
      ...
      link:       `/admin/migracion.html?sesion_id=${s.id}`,
    });
  }
  ...
  for (const t of turnosConDiferencia || []) {
    ...
    alertas.push({
      ...
      leido:      false,
      link:       `/admin/cajas.html?turno_dif=${t.id}`,
    });
  }
  ...
  for (const e of entregasConCobroParcial || []) {
    ...
    alertas.push({
      ...
      leido:      false,
      link:       `/admin/rutas.html?entrega_dif=${e.id}&ruta_id=${e.ruta_id || ''}&fecha=${e.rutas?.fecha || ''}`,
    });
  }
...
async function handleDashboardEjecutivo(req, res, empresa_id) {
  const { desde, hasta, dias } = periodoAFechas(req.query.periodo || '30d');
  const desdeAnterior = new Date(desde);
  desdeAnterior.setDate(desdeAnterior.getDate() - dias);
  ...
  if (kpisRes.error)    return errorSeguro(res, error, 500, 'No se pudo completar la operación.', { error: kpisRes.error.message });
  if (resumenRes.error) return errorSeguro(res, error, 500, 'No se pudo completar la operación.', { error: resumenRes.error.message });
