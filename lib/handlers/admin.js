...
async function handleAlertas(req, res, empresa_id) {
  ...
  // Perf (Etapa 6 — p99 alto bajo tráfico real, 2.8s-4.5s vs 300ms-1.7s del
  // resto del dashboard): las 9 categorías de este panel son lecturas de
  // solo lectura, cada una con su propio filtro (empresa_id + fechas ya
  // calculadas arriba) — NINGUNA depende del resultado de otra. Antes se
  // pedían en secuencia, una `await` atrás de la otra (~11 round-trips a
  // Supabase antes de poder armar la respuesta). Se piden todas juntas acá
  // con Promise.all — mismos datos, mismo orden de armado de `alertas` más
  // abajo, solo cambia CUÁNDO se disparan las consultas.
  // Excepción: turnos-con-diferencia (cat. 7) y entregas-cobro-parcial
  // (cat. 8) necesitan primero saber cuántas anomalías ya fueron
  // "marcadas como resueltas" (su propio limit de consulta depende de ese
  // conteo) — esas dos quedan con su segundo paso en un Promise.all propio,
  // más abajo, pero el primer paso de ambas entra en este mismo grupo.
  const [
    notifs,
    pedidosDemorados,
    sesionesConError,
    ...
    resumenCheques,
    clientesCriticos,
    facturasConDiferencias,
    turnosRevisados,
    entregasRevisadas,
    eventosEnError,
  ] = await Promise.all([
    AdminRepo.obtenerNotificacionesRecientes(empresa_id, 20),
    AdminRepo.obtenerPedidosDemorados(empresa_id, hace24h.toISOString(), limitePorCategoria),
    AdminRepo.obtenerSesionesMigracionConError(empresa_id, limitePorCategoria),
    AdminRepo.obtenerChequesVencidos(empresa_id, hoyISO, limitePorCategoria),
    AdminRepo.obtenerResumenChequesVencidos(empresa_id, hoyISO),
    AdminRepo.obtenerClientesScoreCritico(empresa_id, limitePorCategoria),
    AdminRepo.obtenerFacturasProveedorConDiferencias(empresa_id, limitePorCategoria),
    AdminRepo.obtenerAnomaliasRevisadas(empresa_id, 'diferencia_caja'),
    AdminRepo.obtenerAnomaliasRevisadas(empresa_id, 'entrega_cobro_parcial'),
    ObservabilidadRepo.obtenerEventosEnErrorProlongado(
  ...
  // Perf (Etapa 6): segundo paso de las categorías 7 y 8 — dependen del
  // conteo de anomalías ya revisadas (arriba, ya resuelto en el
  // Promise.all grande) pero son independientes ENTRE ELLAS, así que
  // también se piden juntas en vez de una atrás de la otra.
  const [turnosConDiferencia, entregasConCobroParcial] = await Promise.all([
    AdminRepo.obtenerTurnosConDiferencia(
      empresa_id, hace30dias.toISOString(), limitePorCategoria + idsRevisados.size
    ),
    AdminRepo.obtenerEntregasConCobroParcial(empresa_id, hace30dias.toISOString(), 30),
  ]);
