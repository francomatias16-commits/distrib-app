...
export default async function handler(req, res) {
  ...
  // ── Cron: recalcular ciclos de compra (v1063) ─────────────────────────────
  // Corre antes que 'generar': calcula/actualiza ciclos_compra a partir del
  // historial real de pedidos (RPC calcular_ciclos_cliente, migración 032)
  // para que 'generar' tenga de dónde leer. Hallazgo v1063: esta función
  // nunca se invocaba desde ningún cron ni handler — ver comentario en
  // lib/repos/piloto.js. Se corre para TODAS las empresas activas (incluida
  // demo) porque "Clientes en fuga" también depende de ciclos_compra.
  if (accion === 'recalcular-ciclos' && esCron) {
    const empresas = await listarEmpresasActivas();
    let procesadas = 0, conError = 0;
    for (const emp of empresas) {
      const { error } = await calcularCiclosClienteRpc(emp.id);
      if (error) conError++;
      else procesadas++;
    }
    return res.json({ ok: true, procesadas, con_error: conError });
  }
