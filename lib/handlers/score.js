...
async function ofrecerPlanDePago(empresaId, cliente, opciones = {}) {
  ...
  // usaba enviarAvisoChequesPorVencer en notif.js — sin WA_ENDPOINT
  // configurada, no tiene sentido pegarle al fallback de localhost en
  // producción (y acá, a diferencia de notif.js, ni había try/catch
  // envolviendo el fetch — un error de red quedaba sin capturar).
  if (!waEndpointConfigurado()) {
    return { ok: false, motivo: 'whatsapp_error', error: 'WA_ENDPOINT no configurado' };
  }

  let waResp;
  try {
    // FIX AUTOMATIZACION-003 — ver comentario en lib/handlers/notif.js.
    waResp = await fetch(WA_ENDPOINT, {
      method: 'POST',
      headers: {
    ...
    console.error(`[SCORE] Error de red enviando oferta_plan_pago para ${cliente.id}:`, err.message);
    return { ok: false, motivo: 'whatsapp_error', error: `error red — ${err.message}` };
  }
  const respuesta = await leerRespuestaWa(waResp);
  if (!respuesta.esJson) {
    console.error(`[SCORE] WhatsApp oferta_plan_pago falló para ${cliente.id}:`, respuesta.motivo);
    return { ok: false, motivo: 'whatsapp_error', error: respuesta.motivo };
  }
...
export default async function handler(req, res) {
  ...
  // FIX (auditoría v960): sin chequeo de rol — mismo hallazgo ya corregido acá
  // abajo para 'alertas'/'resolver-alerta'/'reglas' ("cualquier usuario
  // autenticado, incluido un cliente del portal"), pero no se había aplicado
  // a esta acción. Un cliente con acceso portal (rol 'cliente') podía pasar
  // el `cliente_id` de OTRO cliente de la misma empresa y ver su score,
  // límite/días de crédito y si recibió una oferta de plan de pago por
  // deuda — información financiera de un tercero, no la propia. Los repos
  // (`historialScore`/`obtenerScoreCliente`/`ultimoEnvio`) solo scopean por
  // `empresa_id`, no por el cliente_id del que llama, así que el gate tiene
  // que estar acá. Mismo set de roles que el resto del archivo.
