...
export default async function handler(req, res) {
  if (req.method !== 'OPTIONS' && _limiterStock(req, res)) return;
...
async function handleLotes(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    const { data: lote, error } = await crearLote({
      empresa_id,
      producto_id,
    ...
    if (error) return errorSeguro(res, error, 500, 'No se pudo completar la operación.');
    return res.status(201).json({ ok: true, id: lote.id });
  }
  ...
  if (req.method === 'PATCH') {
    ...
    const { id, cantidad, estado, fecha_vencimiento, costo_unitario, numero_lote } = req.body;
    if (!id) return res.status(400).json({ error: 'id requerido' });
    ...
    if (cantidad !== undefined)        patch.cantidad          = parseFloat(cantidad);
...
async function handleClienteCategorias(req, res) {
  ...
  const { data, error } = await listarCategoriasConProductos(empresa_id);
...
async function handleClienteProductos(req, res) {
  ...
  const { data: filas, error } = await db.rpc('cliente_productos_disponibles', {
