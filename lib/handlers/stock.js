...
async function handleLotes(req, res) {
  ...
  if (req.method === 'PATCH') {
    ...
    // Etapa 1 auditoría (2026-09): lotes_estado_check solo permite
    // activo/agotado/vencido y este PATCH hace un UPDATE directo (no pasa
    // por ninguna RPC) con el valor crudo de `estado` del body. A
    // diferencia del caso de facturas_proveedor/pagos_proveedor, acá el
    // error de Postgres si pasa por `errorSeguro` así que no hay fuga de
    // detalle interno — el gap es puramente de robustez/UX (un valor
    // fuera de whitelist rompía como 500 genérico en vez de un 400 claro).
    // Mismo criterio de whitelist en JS que ya se aplica en el resto de
    // esta etapa.
    const ESTADOS_LOTE_VALIDOS = new Set(['activo', 'agotado', 'vencido']);
    if (estado !== undefined && estado !== null && !ESTADOS_LOTE_VALIDOS.has(estado))
      return res.status(400).json({ error: 'estado debe ser activo, agotado o vencido' });
