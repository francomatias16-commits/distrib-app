...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST' && accion === 'cerrar-reporte') {
    ...
    // Queda disponible para regenerar el reporte a mano (ej. si cambió algo
    // en las entregas después de que se generó automáticamente), pero desde
    // el fix del 15/09/2026 el caso normal ya no depende de este endpoint:
    // `sincronizarEstadoRuta` lo dispara solo apenas la ruta pasa a
    // 'completada'.
    await RutasRepo.generarReporteEficienciaRuta(ruta_id);
