...
async function getUsuarioAutenticado(req) {
  ...
  const { data: { user }, error } = await getUserSeguro(supabaseAdmin, token);
...
async function getSuperAdmin(req) {
  ...
  //
  // FIX (v-401-saas-billing): el match por nombre exacto ('MF Web Solutions')
  // nunca calzaba contra el nombre real en prod, 'MF Web Solutions (Superadmin)'
  // — todo dueño de la empresa raíz recibía 401 en /api/saas. Se hardcodea el
  // empresa_id conocido (fijo desde que se creó la empresa raíz,
  // 00000000-0000-0000-0000-000000000099) como fallback SIEMPRE activo.
  //
  // FIX (incidente 2026-09-14, "no cargan las empresas en saas-billing"):
  // el dueño de la empresa raíz seguía recibiendo 401 con el fix de arriba
  // ya desplegado. Causa: acá abajo `empresaRaizId` usaba
  // `process.env.SUPERADMIN_EMPRESA_ID || EMPRESA_RAIZ_ID_DEFAULT` — si esa
  // env var estaba seteada en Vercel con un valor distinto (typo, ID viejo,
  // etc.) PISABA el fallback hardcodeado y bloqueaba al dueño real, sin
  // ningún log que lo explicara (el `if (!perfil)` y el `return null` de
  // acá abajo fallan en silencio). Ahora el ID hardcodeado se chequea
  // SIEMPRE, además de — no en reemplazo de — un posible override por env
  // var: una env var mal seteada ya no puede romper el acceso del dueño
  // real de la empresa raíz.
  const EMPRESA_RAIZ_ID_DEFAULT = '00000000-0000-0000-0000-000000000099';
  const NOMBRE_EMPRESA_RAIZ = 'MF Web Solutions (Superadmin)';
  const empresaRaizIdOverride = process.env.SUPERADMIN_EMPRESA_ID || null;

  // Nota: el enum `rol_usuario` de la base (dueno/admin/vendedor/depositero/
  // chofer/contador/cliente) no tiene ni tuvo nunca el valor 'superadmin' —
  // ese chequeo era código muerto que nunca podía dar true. El único camino
  // real de acceso es ser 'dueno' de la empresa raíz.
  const esDuenoDeEmpresaRaiz = perfil.rol === 'dueno' && (
    perfil.empresas?.nombre === NOMBRE_EMPRESA_RAIZ
    || perfil.empresa_id === EMPRESA_RAIZ_ID_DEFAULT
    || (empresaRaizIdOverride && perfil.empresa_id === empresaRaizIdOverride)
  );

  if (!esDuenoDeEmpresaRaiz) {
    console.error(
      '[saas] Acceso denegado a panel superadmin — perfil no matchea empresa raíz.',
      { user_id: user.id, rol: perfil.rol, empresa_id: perfil.empresa_id, empresa_nombre: perfil.empresas?.nombre },
    );
    return null;
  }
