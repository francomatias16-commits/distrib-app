...
import {
  ...
  revocarSesionesRefreshTokens,
} from '../repos/usuarios.js';

const limiter = rateLimit({ max: 60, windowMs: 60_000 });
// Migrado a PermisosService (B3 del plan de cierre): sigue exportado con
// el mismo nombre/valor para lib/asistente-tools.js, ahora vía rolesDe()
// en vez de array suelto — ver lib/permisos-service.js. ROLES_ASIGNABLES
// y ROLES_PRIVILEGIADOS NO se migran a propósito: no son gates de acceso,
// son validación de valores y jerarquía actor-vs-objetivo (ver nota en
// permisos-service.js).
export const ROLES_GESTION = rolesDe('usuarios', 'acceder');
...
export default async function handler(req, res) {
  ...
  if (!perfil || !puede(perfil, 'acceder', 'usuarios')) {
  ...
  if (req.method === 'POST') {
    ...
    const errorPwFiltradaAlta = await chequearPasswordONull(password);
    if (errorPwFiltradaAlta) return res.status(400).json(errorPwFiltradaAlta);
  ...
  if (req.method === 'PATCH') {
    ...
    if (password) {
      const errorPwFiltradaEdicion = await chequearPasswordONull(password);
      if (errorPwFiltradaEdicion) return res.status(400).json(errorPwFiltradaEdicion);
    }
    ...
    if (hayPassword) {
      ...
      await revocarSesionesRefreshTokens(id);
    }
