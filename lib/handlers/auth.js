...
async function handleRefresh(req, res) {
  ...
  const { data: tokenGuardado } = await supabaseAdmin
    .from('refresh_tokens').select('id, usuario_id, revocado').eq('token_hash', tokenHash).single();

  if (!tokenGuardado || tokenGuardado.revocado) {
    if (tokenGuardado?.usuario_id) {
      await supabaseAdmin.from('refresh_tokens').update({ revocado: true }).eq('usuario_id', tokenGuardado.usuario_id);
    }
    limpiarCookies(res);
    return res.status(401).json({ error: 'Sesión inválida. Iniciá sesión nuevamente.' });
  }

  await supabaseAdmin.from('refresh_tokens').update({ revocado: true }).eq('id', tokenGuardado.id);

  const nuevoPayload = { sub: payload.sub, rol: payload.rol };
  ...
  await supabaseAdmin.from('refresh_tokens').insert({
    usuario_id: tokenGuardado.usuario_id, token_hash: nuevoHash,
...
async function handleMe(req, res) {
  ...
  return res.status(200).json({
    usuario: { id: perfil.id, nombre: perfil.nombre, rol: perfil.rol, email: perfil.email },
    empresa,
  });
}
