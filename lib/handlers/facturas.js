...
async function guardarConfigARCA(req, res, empresa_id) {
  ...
  const certFinal = cert_pem?.trim() ? cifrar(cert_pem.trim()) : (existente?.cert_pem || null);
  const keyFinal  = key_pem?.trim()  ? cifrar(key_pem.trim())  : (existente?.key_pem  || null);
  ...
  const upsertData = {
    ...
    // modo_certificado se deriva de si hay certificado propio cargado (nuevo
    // o ya existente) — antes este upsert nunca tocaba la columna, así que
    // quedaba en 'delegado' (el DEFAULT) para siempre sin importar lo que
    // se subiera acá, y el formulario "usar mi propio certificado ARCA" no
    // hacía lo que decía. Con cert+key presentes pasa a 'propio'; si no, a
    // 'delegado' (empresas ya migradas en 'propio' que solo actualizan
    // datos sin re-subir cert conservan certFinal/keyFinal existentes, así
    // que no se les cae el modo).
    modo_certificado: (certFinal && keyFinal) ? 'propio' : 'delegado',
    cert_pem:      certFinal,
    key_pem:       keyFinal,
  };
...
async function handleNotasCredito(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    // v200d fix: notas_credito_tipo_check solo permite A/B/C y el RPC
    ...
    //
    // Decisión 2.1 (2026-09, confirmada por CLAY): se sacó 'M' del rango
    // válido. Nunca hubo soporte real para Factura/NC tipo M en wsfev1.js
    // (sin código de comprobante ARCA, sin la lógica de umbral RG 3337 ni
    // la percepción del 3% que exige) — la opción estaba en el combo pero
    // era un callejón sin salida: una NC "M" vinculada a una factura real
    // terminaba emitiéndose con la letra de esa factura (A/B), no como M,
    // y una NC "M" sin factura vinculada fallaba directo al intentar
    // emitirla. Si en el futuro un cliente de CLAY factura montos que
    // realmente disparen el régimen de Factura M, esto se reevalúa como
    // una implementación completa, no reactivando este valor sin más.
    const TIPOS_NC_VALIDOS = ['A', 'B', 'C'];
    if (!tipo || !TIPOS_NC_VALIDOS.includes(tipo))
      return res.status(400).json({ error: 'tipo debe ser A, B o C' });
