...
/**
 * NOTA (migración 630, 15/09/2026): desde la migración 630 esta
 * sincronización está garantizada por el trigger `trg_sync_entregas_desde_pedido`
 * sobre `pedidos` (que a su vez llama a `fn_sync_estado_ruta`, espejo SQL
 * exacto de esta función). Se mantiene esta versión JS porque los flujos del
 * chofer escriben en `entregas` sin pasar por `pedidos` (ej. "no entregar",
 * que deja el pedido en 'confirmado'), y ahí el trigger no aplica. Las dos
 * son idempotentes: si corren las dos, la segunda no encuentra nada que
 * cambiar. NO replicar esta lógica en ningún call site nuevo.
 */
