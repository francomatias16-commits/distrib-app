...
import { sincronizarEstadoRuta } from './_helpers.js';
...
export default async function handler(req, res) {
  ...
  if (req.method === 'PATCH') {
    ...
    if (estado === 'despachado') {
      ...
      // FIX (repartos/armar ruta desincronizado): un admin puede confirmar
      // la entrega acá sin que el chofer la haya confirmado nunca desde su
      // portal (chofer.js). Antes esto solo tocaba `pedidos.estado`, y
      // `entregas.estado` quedaba en 'pendiente'/'en_camino' — por eso la
      // sección de repartos/armar ruta seguía mostrando el pedido como
      // pendiente de entrega. Replica el mismo paso de sincronización que
      // hace el flujo del chofer al confirmar (ver chofer.js, PATCH
      // /api/chofer/remitos → entregar).
      if (!error) {
        const { data: entregaActualizada, error: entregaUpdateError } = await marcarEntregaCompletada(id, {
          estado:             'entregado',
          fecha_confirmacion: new Date().toISOString(),
          notas_entrega:      motivo.trim() || null,
        });

        if (entregaUpdateError) {
          console.error('[pedidos PATCH admin] Error sincronizando entregas.estado:', entregaUpdateError?.message || entregaUpdateError);
        } else {
          // Best-effort: si falla, no debe bloquear la confirmación ya guardada.
          const ruta_id_entrega = entregaActualizada?.[0]?.ruta_id;
          if (ruta_id_entrega) {
            sincronizarEstadoRuta(ruta_id_entrega).catch(e =>
              console.error('[pedidos PATCH admin] Error sincronizando rutas.estado:', e?.message || e));
          }
        }
      }
    } else {
