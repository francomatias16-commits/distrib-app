...
import { firmarUrlStorage, firmarCampoUrl, firmarCampoUrlEnLista } from '../utils/storage-urls.js';
...
async function handleCompras(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    if (accion === 'historial-recepciones') {
      ...
      if (error) return errorSeguro(res, error, 500, 'No se pudo completar la operación.');
      const recepciones = await firmarCampoUrlEnLista(supabase, 'remitos', data || [], 'foto_url');
      return res.json({ recepciones, total: count || 0 });
    }
  ...
  if (req.method === 'POST') {
    ...
    if (req.query.accion === 'upload-remito') {
      ...
      // BUG-04: antes no había ningún allowlist acá — mime_type llegaba del
      // cliente y se usaba tal cual para elegir la extensión y el
      // Content-Type de Storage, sin validar ni el tipo declarado ni el
      ...
      const ext  = mimeReal === 'application/pdf' ? 'pdf' : mimeReal.split('/')[1];
      const path = `${empresa_id}/${recepcion_id}.${ext}`;

      const { error: uploadErr } = await ComprasRepo.subirRemitoStorage(path, buffer, mimeReal);

      if (uploadErr)
        return errorSeguro(res, uploadErr, 500, 'Error al subir imagen.');

      // SEC-05: bucket 'remitos' privado. En DB se guarda el path, no una
      // URL pública. Para la miniatura inmediata del panel se firma una URL
      // de corta duración que NO se persiste.
      await ComprasRepo.actualizarFotoRecepcion(recepcion_id, path);

      const foto_url = await firmarUrlStorage(supabase, 'remitos', path);
    ...
    if (req.query.accion === 'notificar-proveedor') {
      ...
      // El link va en un email externo que puede abrirse días después de
      // enviado: acá NO sirve la expiración corta por defecto (10 min).
      const SIETE_DIAS_SEG = 60 * 60 * 24 * 7;
      const recepcionParaEmail = await firmarCampoUrl(supabase, 'remitos', recepcion, 'foto_url', SIETE_DIAS_SEG);
