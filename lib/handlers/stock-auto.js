...
export async function analizarYGenerarOrdenes(empresa_id) {
  ...
  const necesitanReponer = (analisis || []).filter(a => a.necesita_reponer);

  // Fix 460: productos que necesitan reponerse pero para los que el motor
  // no pudo calcular una cantidad sugerida (> 0) — típicamente productos
  // recién migrados/creados sin historial de ventas (velocidad = 0) y sin
  // stock_objetivo cargado, donde la fórmula da 0 unidades sugeridas.
  // Antes: se descartaban en silencio junto con `necesita_reponer = false`,
  // así que un producto en 0 stock nunca aparecía en "Reposición sugerida"
  // hasta que tuviera algo de historial de ventas. Ahora: se deja constancia
  // en alertas_stock (sin orden, porque no hay base para sugerir cantidad)
  // para que al menos sea visible y el dueño pueda cargar un stock_minimo/
  // stock_objetivo o revisar manualmente — mismo criterio que ya existía
  // para "sin_proveedor".
  const sinCantidadCalculable = necesitanReponer.filter(a => !(a.cantidad_sugerida > 0));
  if (sinCantidadCalculable.length) await alertarSinHistorial(empresa_id, sinCantidadCalculable);

  const criticos = necesitanReponer.filter(a => a.cantidad_sugerida > 0);
...
// Fix 460: alerta de visibilidad para productos que necesitan reponerse
// pero sin cantidad sugerida calculable (sin historial de ventas y sin
// stock_objetivo cargado). No genera orden de compra — no hay base
// numérica confiable para decidir cuánto pedir, así que se prioriza no
// adivinar antes que generar una orden con una cantidad arbitraria.
async function alertarSinHistorial(empresa_id, items) {
  const filasAlertas = items.map(item => ({
    empresa_id,
    producto_id:    item.producto_id,
    tipo:           'sin_historial',
  ...
  const enQuiebre = items.filter(i => i.stock_actual <= 0).length;
  notifAuto(empresa_id, {
    tipo:   'stock_sin_historial',
    titulo: '⚠ Reposición sin historial de ventas',
    cuerpo: `${items.length} producto${items.length > 1 ? 's' : ''} necesita${items.length > 1 ? 'n' : ''} reponerse pero no hay historial de ventas ni stock objetivo para calcular cuánto pedir — cargá un stock mínimo/objetivo o revisalo manualmente` +
            (enQuiebre ? ` (${enQuiebre} ya sin stock)` : ''),
    link:   '/admin/stock',
  }).catch(() => {});
}
