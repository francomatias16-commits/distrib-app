...
export async function enviarPush(usuarioId, titulo, cuerpo, datos = {}, logMeta = null) {
  const firebaseApp = asegurarFirebase();
  try {
    ...
    // 3. Enviar a cada dispositivo, capturar el primer message_id de FCM
    let enviadas    = 0;
    let primerMsgId = null;

    for (const dispositivo of dispositivos) {
      try {
        const msgId = await getMessaging(firebaseApp).send({
          ...message,
          token: dispositivo.token_push,
        });
        if (!primerMsgId) primerMsgId = msgId;
        enviadas++;
      } catch (err) {
        console.error('[PUSH] Error enviando a dispositivo:', err.message || err);
      }
    }
