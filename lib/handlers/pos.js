...
async function abrirTurnoHandler(req, res, perfil) {
  ...
  if (!caja.deposito_id) {
    ...
    if (dep) {
      await asignarDepositoACaja(caja_id, dep.id);
...
async function productosHandler(req, res, perfil) {
  ...
  if (caja_id) {
    ...
    if (caja.deposito_id) {
      ...
      if (dep) {
        deposito_id = dep.id;
        asignarDepositoACaja(caja_id, dep.id);
      }
    }
  }
...
async function registrarVentaHandler(req, res, perfil) {
  ...
  const descGlobalPct = Math.max(0, Math.min(100, parseFloat(descuento_global_pct) || 0));

  // ── Validación server-side de descuentos por línea ──────────────────────
  // Clampear 0-100 y verificar PIN de supervisor si algún ítem supera el umbral.
  // El frontend ya lo hace en el cliente, pero nunca hay que confiar solo en eso.
  ...
  if (maxDescLinea > 0) {
    ...
    if (maxDescLinea > umbral) {
      if (!pin_supervisor || String(pin_supervisor).trim().length < 4) {
        return res.status(403).json({
          error: `Descuento del ${maxDescLinea}% supera el umbral autorizado (${umbral}%). Se requiere PIN de supervisor.`,
          requiere_pin: true,
        });
      }
  ...
  for (const pago of pagos) {
    if (!pago.medio || pago.monto === undefined || pago.monto === null) {
      return res.status(400).json({ error: 'Cada pago necesita medio y monto' });
    }
  }
...
async function transferirStockHandler(req, res, perfil) {
  ...
  if (!producto_id || !deposito_origen || !deposito_destino || !cantidad) {
    return res.status(400).json({
      error: 'producto_id, deposito_origen, deposito_destino y cantidad son requeridos',
    });
  }
...
// Cualquier rol de venta puede leerla: la necesita el navegador del cajero
// para inicializar PosPrinter / PosTerminal al abrir el POS.
async function getConfigHardwareHandler(req, res, perfil) {
  ...
  const hw = empresa?.config?.pos_hardware || {};
...
// Solo dueño/admin puede modificar la config de impresora/terminal.
async function postConfigHardwareHandler(req, res, perfil) {
  ...
  const { impresora, terminal } = req.body || {};
  if (!impresora && !terminal) {
    return res.status(400).json({ error: 'Nada para guardar: falta impresora o terminal' });
  }

  const actual = await obtenerConfigEmpresa(perfil.empresa_id);

  const configActual = actual?.config || {};
  const hwActual      = configActual.pos_hardware || {};
  ...
  const { error } = await actualizarConfigEmpresa(perfil.empresa_id, nuevoConfig);

  if (error) return errorSeguro(res, error, 500, 'No se pudo completar la operación.');

  await AuditRepo.registrarAuditoriaSilenciosa(
    perfil.empresa_id, perfil.id, 'empresas', 'UPDATE', perfil.empresa_id,
    { pos_hardware: hwActual }, { pos_hardware: nuevoConfig.pos_hardware }
  );
...
async function postPromocionesHandler(req, res, perfil) {
  ...
  if (accion === 'toggle') {
    ...
    const { error } = await togglePromocion(id, !promo.activa);
    if (error) return errorSeguro(res, error, 500, 'No se pudo completar la operación.');
