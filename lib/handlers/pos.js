...
//
// ── Punto 1 (mitad "agregar stock sin perder el comprobante") ────────────
// POST /api/pos/agregar-stock-rapido     → ingreso de stock sin salir del POS
//                                           (no navega a Stock, el carrito
//                                           en memoria queda intacto). Roles
//                                           sin permiso de ajustar stock
//                                           (ej. vendedor) requieren PIN de
//                                           supervisor, igual que el
//                                           descuento por encima del umbral.
...
import { listarDepositosIds, existeDepositoEnEmpresa, ajustarStockRpc } from '../repos/stock.js';
import {
...
import { calcularTotalesPos } from '../calc/pos-totales.js';
...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    // ── Punto 1 (mitad "agregar stock sin perder el comprobante") ────────
    if (accion === 'agregar-stock-rapido') return agregarStockRapidoHandler(req, res, perfil);
...
async function productosHandler(req, res, perfil) {
  ...
  const resultado = productos.map(p => {
    ...
    const base = {
      ...
      // v631: explícito (aunque ya venga en `...p`) porque es el dato del
      // que depende el POS para dejar agregar al carrito un producto sin
      // stock — si algún día cambia el select del repo, el contrato de
      // esta respuesta no se rompe en silencio.
      permite_negativo: p.permite_negativo === true,
    };
...
async function registrarVentaHandler(req, res, perfil) {
  ...
  // Fix auditoría 2026-09: el descuento global se prorratea por ítem (ver
  // lib/calc/pos-totales.js) en vez de restarse una sola vez del total ya
  // sumado — así `venta_pos_items.subtotal` + su IVA cierran exacto contra
  // `total` sin importar cuántas alícuotas distintas haya en el carrito,
  // que es justo lo que valida wsfev1.js al reconstruir el desglose para
  // Factura A/B (incluida la facturación automática de cta_cte más abajo).
  const precioMapConBase = Object.fromEntries(
    items.map(item => [item.producto_id, precioMap[item.producto_id] ?? productosMap[item.producto_id].precio_base])
  );
  const ivaMap = Object.fromEntries(
    items.map(item => [item.producto_id, productosMap[item.producto_id].iva ?? 21])
  );

  const { subtotal, iva_total, total, itemsParaRpc } = calcularTotalesPos(items, {
    precioMap: precioMapConBase,
    ivaMap,
  ...
  // Decisión 2.2: `total` ahora persiste con centavos (ver
  // lib/calc/pos-totales.js), pero los pagos que arma el cajero siguen
  // referidos al total en pesos enteros que muestra la pantalla (caja,
  // vuelto y arqueo se mantienen sin cambios, decisión v755 vigente). Por
  // eso este chequeo no puede exigir 0.01 de tolerancia — rechazaría en el
  // Node cualquier venta con centavos que el propio RPC
  // registrar_venta_pos igual aceptaría (ver migración
  // 496_fix_registrar_venta_pos_vuelto_efectivo.sql, que ya tolera hasta
  // $1 de diferencia por el mismo motivo de redondeo a peso entero). Se
  // usa la misma tolerancia acá para no rechazar en el Node lo que la base
  // de datos permitiría.
  const TOLERANCIA_REDONDEO_PESO = 1;
  ...
  if (sumaPagos < total - TOLERANCIA_REDONDEO_PESO) {
    return res.status(400).json({ error: 'La suma de los pagos no alcanza el total de la venta' });
  }
  if (sumaNoEfectivo > total + TOLERANCIA_REDONDEO_PESO) {
...
async function getFavoritosHandler(req, res, perfil) {
  ...
  const resultado = favs.map(f => ({
    ...
    // v631: el botón de favorito arma el producto a mano (no pasa por la
    // búsqueda), así que necesita el flag acá o el carrito lo ve undefined
    // y bloquea la venta aunque el producto esté autorizado.
    permite_negativo: f.productos?.permite_negativo === true,
  }));
...
async function getConfigHardwareHandler(req, res, perfil) {
  ...
  const empresaResp = {
    ...
    // 4b del audit — logo en el ticket térmico: reutiliza el mismo
    // empresas.logo_url que ya usan nav.js/topbar-widgets.js y el PDF de
    // comprobante ARCA (comprobante-pdf.js). Es una signed URL de larga
    // duración (ver obtenerLogoUrl en lib/repos/empresas.js) — no hace
    // falta ningún endpoint nuevo, solo exponerla acá para que el POS la
    // use igual que ya la usa el resto del admin.
...
// ── POST /api/pos/agregar-stock-rapido ───────────────────────────────────
// Ingreso de stock del producto en el depósito de la caja actual, sin salir
// del POS. Pensado para el momento en que el cajero está armando una venta,
// se encuentra con "sin stock" y quiere sumarlo ahí mismo — hoy Stock es
// otra página HTML y `carrito` es una variable en memoria sin persistencia:
// navegar la pierde por completo. Este endpoint no resuelve el "carrito en
// espera" (fuera de alcance) — solo evita que el cajero tenga que abandonar
// la pantalla del POS para dar de alta el stock que le falta a un producto.
//
// dueno/admin/depositero (mismos roles que ya pueden ajustar stock desde
// /api/stock) lo hacen directo. Cualquier otro rol con acceso al POS
// (vendedor) necesita PIN de supervisor — mismo mecanismo que ya existe
// para el descuento por encima del umbral en registrarVentaHandler.
const ROLES_AJUSTAN_STOCK_DIRECTO = ['dueno', 'admin', 'depositero'];

async function agregarStockRapidoHandler(req, res, perfil) {
  // No usa puede(perfil, 'vender', 'pos') — esa lista es ['dueno', 'admin',
  // 'vendedor'] y no incluye a depositero, que sí debe poder ajustar stock
  // directo acá (es uno de los roles que ya ajustan stock desde /api/stock).
  // El gate real de esta acción es "puede vender en el POS" (vendedor
  // incluido, para llegar al branch de PIN) o "ajusta stock directo".
  if (!ROLES_AJUSTAN_STOCK_DIRECTO.includes(perfil.rol) && perfil.rol !== 'vendedor') {
    return res.status(403).json({ error: 'Sin permisos para vender' });
  }

  const { caja_id, producto_id, cantidad, notas, pin_supervisor } = req.body || {};

  if (!caja_id || !producto_id) {
    return res.status(400).json({ error: 'caja_id y producto_id son requeridos' });
  }

  const cantidadNumerica = Number(cantidad);
  if (!Number.isInteger(cantidadNumerica) || cantidadNumerica <= 0) {
    return res.status(400).json({ error: 'cantidad debe ser un número entero positivo' });
  }
  ...
  if (!caja.deposito_id) {
    return res.status(400).json({ error: 'Esta caja no tiene un depósito asignado.' });
  }

  if (!ROLES_AJUSTAN_STOCK_DIRECTO.includes(perfil.rol)) {
    if (!pin_supervisor || String(pin_supervisor).trim().length < 4) {
      return res.status(403).json({
        error: 'Agregar stock requiere autorización de supervisor.',
        requiere_pin: true,
      });
    }
    const { data: empresaPin } = await obtenerPinSupervisor(perfil.empresa_id);
    if (!empresaPin?.supervisor_pin) {
      return res.status(403).json({ error: 'No hay PIN de supervisor configurado. Pedile al administrador que lo configure en Ajustes.' });
    }
    if (!(await verificarYMigrarPin(pin_supervisor, empresaPin.supervisor_pin, perfil.empresa_id))) {
  ...
  const motivo = notas
    ? `Alta rápida desde POS (venta en curso): ${notas}`
    : 'Alta rápida desde POS (venta en curso)';

  const { data: rpcResult, error: rpcError } = await ajustarStockRpc({
    producto_id,
    deposito_id: caja.deposito_id,
    delta:        cantidadNumerica,
    tipo:        'ingreso',
    motivo,
    notas,
    usuario_id:  perfil.id,
  ...
  if (!rpcResult?.ok) return res.status(400).json({ error: rpcResult?.error || 'No se pudo agregar el stock' });

  await AuditRepo.registrarAuditoriaSilenciosa(
    perfil.empresa_id, perfil.id, 'stock', 'UPDATE', producto_id,
    null, { deposito_id: caja.deposito_id, cantidad: cantidadNumerica, tipo: 'ingreso', origen: 'pos_rapido', notas: notas || null }
  );
