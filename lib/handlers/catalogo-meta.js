...
// tipo de token y tampoco hace falta (el único endpoint que se llama con
// este token es /me/owned_product_catalogs). config_id ya generado y
// pegado en frontend/env-config.js (META_CATALOG_LOGIN_CONFIG_ID).
...
async function getAuthorizedCatalogId(token) {
  const url = `${META_BASE_URL}/me/owned_product_catalogs?access_token=${encodeURIComponent(token)}`;
