...
async function verPortal(res, proveedor_id, empresa_id) {
  ...
  const facturas = await listarFacturasProveedorPortal(empresa_id, proveedor_id);
...
async function subirFactura(req, res, proveedor_id, empresa_id) {
  ...
  if (archivo_base64) {
    ...
    const mime = match[1];
    const ext = mime === 'application/pdf' ? 'pdf' : (match[2] === 'jpeg' ? 'jpg' : match[2]);
    ...
    const { data: pub } = supabase.storage.from('facturas-proveedor').getPublicUrl(path);
