...
// lib/permisos-service.js (recurso `portal_proveedor`). Sin blast radius:
// a diferencia de usuarios.js/chofer_invitacion.js, este array no lo
// reimporta ningún otro archivo, así que no hace falta reexportarlo
// aparte de este mismo módulo (que ya lo hace por su cuenta).
export const ROLES_ESCRITURA = rolesDe('portal_proveedor', 'escribir');
...
export async function handlePortalAdmin(req, res, perfil) {
  ...
  if (!puede(perfil, 'escribir', 'portal_proveedor'))
...
async function verPortal(res, proveedor_id, empresa_id) {
  ...
  // Bug de UX: antes de este chequeo, una empresa suspendida/dada de baja
  // mostraba el portal con 0 órdenes y 0 facturas, indistinguible de una
  // empresa al día sin movimientos pendientes. Se corta acá, antes de
  // pedir órdenes/facturas, y se le avisa al frontend con un flag explícito
  // para que muestre un mensaje claro en vez de listas vacías.
  const estadoEmpresa = await obtenerEstadoEmpresaPortal(empresa_id);
  if (estadoEmpresa && (estadoEmpresa.activa === false || estadoEmpresa.saas_suspendida === true)) {
    return res.json({
      ok: true,
      empresa_suspendida: true,
