...
export async function listarSesionesMigracion({ empresa_id }) {
  const { data, error } = await MigracionRepo.listarSesionesPorEmpresa(empresa_id);
...
async function listarSesiones(req, res, perfil) {
  const resultado = await listarSesionesMigracion({ empresa_id: perfil.empresa_id });
