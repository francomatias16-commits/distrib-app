...
export default async function handler(req, res) {
  ...
  // ── GET /api/empresa/arca-delegacion ─────────────────────────────────────
  // Datos para el wizard: qué CUIT hay que autorizar y el estado guardado
  // de la última verificación (sin volver a llamar a ARCA en cada carga
  // de pantalla — eso lo hace el POST de abajo, a demanda del usuario).
  if (req.method === 'GET' && _svc === 'arca-delegacion') {
    const perfil = await requerirPerfilAdmin(req, res);
    if (!perfil) return;

    try {
      const { obtenerDatosDelegacion } = await import('../arca/delegacion.js');
      const { data: config, error } = await db
        .from('facturacion_config')
        .select('modo_certificado, estado_delegacion, delegacion_verificada_en, delegacion_error')
        .eq('empresa_id', perfil.empresa_id)
        .maybeSingle();
      if (error) return errorSeguro(res, error, 500, 'No se pudo leer el estado de facturación.');

      const { cuitProveedor } = await obtenerDatosDelegacion();

      return res.status(200).json({
        ok: true,
        cuitProveedor,
        modoCertificado: config?.modo_certificado ?? 'delegado',
        estadoDelegacion: config?.estado_delegacion ?? 'pendiente',
        verificadaEn: config?.delegacion_verificada_en ?? null,
        error: config?.delegacion_error ?? null,
      });
    } catch (err) {
      return errorSeguro(res, err, 500, 'Error obteniendo el estado de la delegación ARCA.');
    }
  }

  // ── POST /api/empresa/arca-delegacion/verificar ──────────────────────────
  // Dispara la llamada de prueba contra ARCA (ver lib/arca/delegacion.js)
  // y devuelve el resultado actualizado. Es la acción del botón
  // "Verificar vinculación" del wizard.
  if (req.method === 'POST' && _svc === 'arca-delegacion-verificar') {
    const perfil = await requerirPerfilAdmin(req, res);
    if (!perfil) return;

    try {
      const { verificarDelegacionArca } = await import('../arca/delegacion.js');
      const resultado = await verificarDelegacionArca(perfil.empresa_id);
      return res.status(200).json({ ok: true, ...resultado });
    } catch (err) {
      return errorSeguro(res, err, 500, 'No se pudo verificar la delegación contra ARCA.');
    }
  }
