...
export async function handleCCProveedores(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    if (accion === 'factura') {
      ...
      // Etapa 1 auditoría (2026-09): facturas_proveedor_tipo_check solo
      // permite A/B/C/M/X y alta_factura_proveedor no valida `tipo` antes
      // del INSERT — su EXCEPTION WHEN OTHERS genérico devuelve el SQLERRM
      // crudo de Postgres como `data.error`, y el bloque de abajo lo
      // reenvía tal cual en el 400 (a diferencia del resto de los errores
      // de este handler, que sí pasan por errorSeguro). Un `tipo` fuera de
      // whitelist terminaría filtrando el nombre real de la constraint/tabla
      // al cliente. Mismo criterio que ya se aplica acá abajo para
      // `medio_pago` — cortar en JS antes de llegar a la RPC.
      const TIPOS_FACTURA_PROVEEDOR_VALIDOS = ['A', 'B', 'C', 'M', 'X'];
      if (tipo !== undefined && tipo !== null && !TIPOS_FACTURA_PROVEEDOR_VALIDOS.includes(tipo))
        return res.status(400).json({ error: 'tipo debe ser A, B, C, M o X' });
    ...
    if (accion === 'pago') {
      ...
      // Etapa 1 auditoría (2026-09): pagos_proveedor_medio_pago_check solo
      // permite efectivo/transferencia/cheque/otro y registrar_pago_proveedor
      // no lo valida (ni tiene EXCEPTION WHEN check_violation específico),
      // así que un valor fuera de rango tiraba un 500 crudo de Postgres en
      // vez de un 400 claro. Mismo patrón que TIPOS_NC_VALIDOS en facturas.js.
      const MEDIOS_PAGO_PROVEEDOR_VALIDOS = ['efectivo', 'transferencia', 'cheque', 'otro'];
      if (medio_pago !== undefined && medio_pago !== null && !MEDIOS_PAGO_PROVEEDOR_VALIDOS.includes(medio_pago))
        return res.status(400).json({ error: 'medio_pago debe ser efectivo, transferencia, cheque u otro' });
  ...
  if (req.method === 'PATCH') {
    if (accion === 'factura') {
      ...
      // Etapa 1 auditoría (2026-09): mismo gap que en el alta (ver
      // comentario en el bloque POST de arriba) — editar_factura_proveedor
      // tampoco valida `tipo` antes de la UPDATE, y su EXCEPTION WHEN
      // OTHERS también reenvía el SQLERRM crudo como `data.error` (acá
      // `estado` sí está cubierto: la RPC lo rechaza explícitamente con
      // ESTADO_NO_PERMITIDO antes de tocar la fila, sin pasar nunca por el
      // camino que expondría el nombre de la constraint).
      const TIPOS_FACTURA_PROVEEDOR_VALIDOS = ['A', 'B', 'C', 'M', 'X'];
      if (tipo !== undefined && tipo !== null && !TIPOS_FACTURA_PROVEEDOR_VALIDOS.includes(tipo))
        return res.status(400).json({ error: 'tipo debe ser A, B, C, M o X' });
