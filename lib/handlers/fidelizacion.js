...
import {
  ...
  obtenerEstadoProgramaFidelizacion,
...
async function listarCatalogoHandler(req, res, ctx) {
  ...
  // Empresas que no activaron (o desactivaron) el programa no deben
  // mostrarle puntos/recompensas a sus clientes — devolvemos un catálogo
  // vacío con `activo: false` en vez de listar recompensas huérfanas.
  const { data: programa, error: errPrograma } = await obtenerEstadoProgramaFidelizacion(empresa_id);
  if (errPrograma) return errorSeguro(res, errPrograma, 500, 'No se pudo verificar el programa de fidelización.');

  if (!programa?.activo) {
    return res.json({ ok: true, activo: false, puntos_disponibles: 0, puntos_totales: 0, recompensas: [] });
  }
...
async function canjearHandler(req, res, ctx) {
  ...
  // Mismo criterio que el catálogo: si la empresa desactivó el programa,
  // no se permite seguir canjeando (aunque queden puntos de antes).
  const { data: programa, error: errPrograma } = await obtenerEstadoProgramaFidelizacion(empresa_id);
  if (errPrograma) return errorSeguro(res, errPrograma, 500, 'No se pudo verificar el programa de fidelización.');
  if (!programa?.activo) {
    return res.status(400).json({ error: 'El programa de fidelización no está disponible.' });
  }
