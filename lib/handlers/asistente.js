...
import { responderConFallback, obtenerGeminiApiKeys } from '../asistente-providers.js';
...
//
// FIX (timeout largo del asistente): esta llamada NO tenía ningún timeout
// propio (a diferencia de fetchConTimeout en asistente-providers.js, que
// usa AbortController) — un fetch que Google no responde en un tiempo
// razonable podía quedar colgado sin límite, comiéndose sola buena parte
// (o la totalidad) del maxDuration de 60s de la función serverless ANTES
// de llegar siquiera a responderConFallback(). Se agrega un timeout de
// 8s por intento (generoso para un embedding, que es una llamada liviana
// comparada con una respuesta de texto), consistente con el resto del
// proyecto. Con withRetry({intentos:2}) el peor caso queda acotado a
// ~2 x 8s + backoff corto, en vez de indefinido.
//
// FIX (2026-09, mismo motivo que SEC-04 en asistente-providers.js): la
// key iba como query param (?key=...) — Google documenta el header
// x-goog-api-key como el método vigente, y las keys nuevas tipo "Auth
// key" (prefijo AQ.) fallan con 401 ACCESS_TOKEN_TYPE_UNSUPPORTED si se
// mandan por query string. Se pasa a header acá también, por las mismas
// razones (y para no loguear la key en texto plano si algún día esta URL
// terminara en un mensaje de error).
const EMBEDDING_TIMEOUT_MS = 8_000;

// FIX (rotación de keys): antes leía process.env.GEMINI_API_KEY directo y
// quedaba atada a una sola cuenta — si esa key se quedaba sin cuota (o era
// una key tipo "Auth key", prefijo AQ., rechazada por embedContent), toda
// la búsqueda semántica se caía aunque hubiera más keys configuradas en
// GEMINI_API_KEYS para el resto del asistente. Ahora reusa la misma lista
// que llamarGemini() (obtenerGeminiApiKeys(), asistente-providers.js) y
// prueba la siguiente key ante 401/403/429/5xx — mismo criterio que la
// rotación de la respuesta de texto, sin duplicar la lógica de
// cuotaAgotada/backoff completa (acá alcanza con "esta key no sirvió,
// probar la próxima").
const EMBEDDING_STATUS_ROTABLES = [401, 403, 429, 500, 502, 503];

async function generarEmbeddingConKey(pregunta, apiKey) {
...
async function generarEmbeddingPregunta(pregunta) {
  ...
  for (let i = 0; i < apiKeys.length; i++) {
    ...
    try {
      return await withRetry(() => generarEmbeddingConKey(pregunta, apiKey), { intentos: 2, baseDelayMs: 300 });
    } catch (error) {
      ultimoError = error;
      if (!EMBEDDING_STATUS_ROTABLES.includes(error.status)) throw error; // ej. 400 malformado: rotar no lo arregla
      if (i < apiKeys.length - 1) {
        console.warn(`[asistente] Embedding: key #${i + 1} falló (${error.status}), probando la siguiente.`);
      }
    }
  }
  throw ultimoError;
...
// conversacionId/toolsUsadas: Capa 3 de PLAN_QA_ASISTENTE.md (migración
// 600). Se guarda solo el NOMBRE de cada tool llamada, no args/resultado
// completos: es lo único que necesita la detección de fallas
// (scripts/detectar-fallas-asistente.js) y evita duplicar en asistente_uso
// datos de negocio que ya viven en las tablas que cada tool tocó.
//
...
async function registrarUso({
  ...
  const { error } = await insertarUsoAsistente({
    ...
    tools_usadas: (toolsUsadas || []).map((t) => t.nombre),
