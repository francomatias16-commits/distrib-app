...
export async function enviarEmailConfirmacionPedido(pedido, cliente, empresa, items = []) {
  ...
  const primerNombre = (cliente.razon_social || '').split(/[\s,]+/)[0];
    ...
    <table class="items-table">
      ...
      <tbody>
        ${items.map(i => {
          ...
          return `<tr>
            <td>${i.nombre || '—'}${i.descuento_pct > 0 ? ` <small style="color:#185FA5">(−${i.descuento_pct}%)</small>` : ''}</td>
  ...
  const notasHtml = pedido.notas_cliente
    ? `<div class="info-box"><strong>Notas del pedido:</strong> ${pedido.notas_cliente}</div>`
    : '';
  ...
  <div class="wrap">
    <div class="header">
      ...
      <p>${empresa?.nombre || 'Distribuidora'} · Pedido #${numeroLabel}</p>
    </div>
    <div class="body">
    ...
    <div class="footer">${empresa?.nombre || 'Distribuidora'} · Este es un email automático, no respondas directamente.</div>
  </div>
</body></html>`;

  return enviarEmail({
    to:      cliente.email,
    subject: `Pedido #${numeroLabel} confirmado — ${empresa?.nombre || 'Distribuidora'}`,
    html,
...
export async function enviarEmailDespacho(pedido, cliente, empresa) {
  ...
  return enviarEmail({
    to:      cliente.email,
    subject: `Tu pedido #${numeroLabel} está en camino — ${empresa?.nombre || 'Distribuidora'}`,
    html,
    replyTo: empresa?.email,
    empresa_id: empresa?.id,
  });
...
export async function enviarEmailRecuperacionPassword(email, linkRecuperacion, empresa) {
  ...
  <div class="wrap">
    <div class="header" style="background:#5B3DA8">
      ...
      <p>${empresa?.nombre || 'Distribuidora'}</p>
    </div>
    <div class="body">
      ...
      <a href="${linkRecuperacion}" class="btn">Restablecer mi contraseña</a>
      ...
      <p style="font-size:12px;color:#888780;margin:16px 0 0">Si el botón no funciona, copiá y pegá este enlace en tu navegador:<br>
        <a href="${linkRecuperacion}" style="color:#185FA5;word-break:break-all">${linkRecuperacion}</a>
      </p>
    </div>
    <div class="footer">${empresa?.nombre || 'Distribuidora'} · Este es un email automático, no respondas directamente.</div>
  </div>
  ...
  return enviarEmail({
    to:      email,
    subject: `Restablecé tu contraseña — ${empresa?.nombre || 'Distribuidora'}`,
    html,
    replyTo: empresa?.email,
    empresa_id: empresa?.id,
  });
...
export async function enviarEmailEstadoCuenta(
  ...
  const nombreEmpresa  = empresa?.nombre || 'Distribuidora';
  ...
  if (facturasPendientes.length > 0) {
    const filas = facturasPendientes.map(f => {
      ...
      const numero    = f.numero || f.id?.substring(0, 8).toUpperCase() || '—';
      return `
        <tr>
    ...
    const filasMov = movimientos.slice(0, 8).map(m => {
      ...
      return `
        <tr>
          ...
          <td style="padding:7px 10px;border-bottom:1px solid #EFEFED;color:#2C2C2A;font-size:13px">${m.descripcion || (esCobro ? 'Cobro recibido' : 'Factura emitida')}</td>
  ...
  const enviadoPorHtml = enviadoPor?.nombre
    ? `<p style="margin:8px 0 0;font-size:11px;color:#AEADA8">Enviado por ${enviadoPor.nombre} · ${fechaEmision}</p>`
...
<body style="margin:0;padding:0;background:#F1EFE8;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif">
  <div style="max-width:580px;margin:32px auto;background:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #D3D1C7">
    ...
    <div style="padding:28px 32px">
      ...
      <div style="background:#F1EFE8;border-radius:8px;padding:14px 16px;font-size:13px;color:#5F5E5A;margin:24px 0 0;line-height:1.6">
        ...
        ${empresa?.email ? `<br><a href="mailto:${empresa.email}" style="color:#185FA5">${empresa.email}</a>` : ''}
      </div>
  ...
  return enviarEmail({
    to:      cliente.email,
    subject: `Estado de cuenta — ${nombreEmpresa}`,
    html,
    replyTo: empresa?.email,
    empresa_id: empresa?.id,
  });
...
export async function enviarEmailRecepcionProveedor(
  ...
  const nombreEmpresa  = empresa?.nombre || 'Distribuidora';
  const nombreProv     = proveedor.razon_social || proveedor.contacto || 'Proveedor';
  const primerNombre   = nombreProv.split(/[\s,]+/)[0];
  const ordenLabel     = orden?.numero || orden?.id?.substring(0, 8).toUpperCase() || '—';
      ...
      <tbody>
        ${items.map(it => {
          ...
          return `<tr style="${rowBg}">
            <td>${it.nombre || '—'}${it.alerta ? ' <span style="font-size:10px;background:#FEE2E2;color:#DC2626;padding:1px 5px;border-radius:3px">⚠</span>' : ''}</td>
  ...
  const fotoHtml = recepcion?.foto_url ? `
    <div class="info-box" style="margin-top:16px">
      ...
      <a href="${recepcion.foto_url}" style="color:#185FA5;margin-left:6px">Ver imagen del remito</a>
    </div>` : '';
...
<body>
  <div class="wrap">
    ...
    <div class="footer">
      ${nombreEmpresa}${empresa?.email ? ` · <a href="mailto:${empresa.email}" style="color:#185FA5">${empresa.email}</a>` : ''}
  ...
  const asunto = hayDiscr
    ? `⚠ Recepción OC #${ordenLabel} — ${discrepancias.length} discrepancia(s) — ${nombreEmpresa}`
    : `Recepción OC #${ordenLabel} confirmada — ${nombreEmpresa}`;
