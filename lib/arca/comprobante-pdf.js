...
//
// FIX (auditoría Facturación/ARCA 2026-08-24, hallazgo crítico): el bucket
// era público y la ruta usa el número de comprobante (correlativo y
// predecible: .../facturas/000123.pdf). Cualquiera que tuviera la URL de
// SU propio comprobante —típicamente un cliente del portal— podía
// enumerar rutas vecinas dentro de la misma carpeta de empresa y
// descargar los comprobantes de OTROS clientes de esa empresa, sin
// autenticarse. El bucket ahora es privado (migración 538); el acceso
// se hace siempre a través de signed URLs de corta duración generadas
// acá, después de validar tenant + (si es cliente) que el comprobante
// le pertenece — mismo gate que ya usaba /api/facturas?accion=pdf.
const BUCKET = 'comprobantes';
const URL_FIRMADA_TTL_SEGUNDOS = 300; // 5 min: alcanza para abrir/descargar

// ── API pública ────────────────────────────────────────────────────────────

/** Ruta determinística del PDF de un comprobante dentro del bucket. */
export function rutaStorageComprobante(empresaId, numero) {
  const nombre = String(numero).replace(/[^a-zA-Z0-9_\-]/g, '_');
  return `empresas/${empresaId}/facturas/${nombre}.pdf`;
}

/**
 * Firma una URL de descarga de corta duración para un comprobante ya
 * generado. No valida permisos: el caller debe confirmar tenant/dueño
 * ANTES de invocar esto (ver lib/handlers/facturas.js).
 *
 * @returns {{ ok: boolean, url?: string, error?: string }}
 */
export async function obtenerUrlFirmadaComprobante(empresaId, numero, expiresIn = URL_FIRMADA_TTL_SEGUNDOS) {
  if (!empresaId || !numero) {
    return { ok: false, error: '[pdf] Faltan empresaId/numero para firmar la URL.' };
  }
  const storagePath = rutaStorageComprobante(empresaId, numero);
  const { data, error } = await supabase.storage.from(BUCKET).createSignedUrl(storagePath, expiresIn);

  if (error || !data?.signedUrl) {
    return { ok: false, error: `[pdf] No se pudo firmar la URL: ${error?.message || 'archivo no encontrado'}` };
  }
 ...
 * Actualiza `facturas.pdf_url` con la RUTA interna (no una URL pública — el
 * bucket es privado) y devuelve una signed URL lista para usar de una.
 *
...
export async function generarPDFComprobante(facturaId) {
  ...
  const storagePath = rutaStorageComprobante(contexto.factura.empresa_id, contexto.factura.numero);
  ...
  // 4. Guardar la RUTA (no URL — bucket privado) en facturas.pdf_url
  ...
  if (updateError) {
    ...
    console.error('[pdf] PDF subido pero no se pudo guardar la ruta en facturas:', updateError.message);
  }

  // 5. Firmar una URL de descarga para devolver ahora mismo
  const { ok, url, error } = await obtenerUrlFirmadaComprobante(
    contexto.factura.empresa_id, contexto.factura.numero
  );
...
async function leerContextoPDF(facturaId) {
  ...
  const { data: empresa, error: errEmp } = await supabase
    .from('empresas')
    .select('nombre, cuit, domicilio, email, logo_url')
  ...
  // FIX (bug real): el PDF nunca dibujaba el logo de la empresa aunque
  // estuviera configurado en empresa-config (empresas.logo_url) — el select
  // ni siquiera lo traía. logo_url es una signed URL de larga duración al
  // bucket privado 'logos' (ver lib/repos/empresas.js), así que se puede
  // descargar directo. El logo puede subirse como PNG/JPEG/WebP/SVG (ver
  // input#logo-input en empresa-config.html), pero pdfkit solo sabe dibujar
  // PNG/JPEG — se normaliza siempre a PNG con sharp (soporta los 4 formatos,
  // incluido rasterizado de SVG) antes de pasarlo a doc.image(). Si falla la
  // descarga o la conversión, no debe romper la emisión del comprobante: se
  // loguea y el PDF se genera igual, sin logo (mismo comportamiento que hoy).
  const logoBuffer = await obtenerLogoBufferPng(empresa.logo_url);

  // Datos adicionales de facturacion_config (razon_social oficial, condicion_iva, punto_venta)
  const { data: cfg } = await supabase
    .from('facturacion_config')
    .select('razon_social, condicion_iva, domicilio, punto_venta, homologacion')
  ...
  return {
    ...
    empresa: {
      ...
      // true si aún no se confirmó explícitamente que está en producción real.
      // Por defecto asumimos homologación (fail-safe): si por algún motivo
      // no se pudo leer facturacion_config, preferimos rotular de más un
      // comprobante real como "de prueba" antes que arriesgarnos a lo inverso.
      homologacion: cfg?.homologacion !== false,
    },
...
/**
 * Descarga la imagen de logo_url (signed URL) y la normaliza a un buffer
 * PNG apto para pdfkit. Devuelve null (nunca lanza) si no hay logo
 * configurado o si algo falla en la descarga/conversión — el comprobante
 * es válido igual sin logo, así que no es motivo para frenar la emisión.
 */
export async function obtenerLogoBufferPng(logoUrl) {
  ...
  try {
    ...
    console.error('[pdf] No se pudo obtener/convertir el logo de la empresa, se omite:', err.message);
    return null;
  }
}
...
async function construirPDF({ factura, empresa, cliente, items, logoBuffer }) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // Logo (si la empresa configuró uno): cuadro de 44x48 a la izquierda
      // del bloque emisor, el texto se corre para no superponerse. Sin
      // logo, el texto mantiene el mismo x de siempre (emisorX + 8).
      const textoX = logoBuffer ? emisorX + 52 : emisorX + 8;
      const textoW = logoBuffer ? emisorW - 60 : emisorW - 16;
      if (logoBuffer) {
        try {
          doc.image(logoBuffer, emisorX + 4, TOP + 4, { fit: [44, 48], align: 'center', valign: 'center' });
        } catch (err) {
          console.error('[pdf] No se pudo dibujar el logo en el PDF, se omite:', err.message);
        }
      }

      doc
        .font('Helvetica-Bold').fontSize(11)
        .text(empresa.nombre, textoX, TOP + 6, { width: textoW });
      doc
        .font('Helvetica').fontSize(8)
        .text(`CUIT: ${formatearCuit(empresa.cuit)}`, textoX, TOP + 21, { width: textoW })
        .text(empresa.domicilio || '', textoX, TOP + 31, { width: textoW })
        .text(condicionIvaLabel(empresa.condicion_iva), textoX, TOP + 41, { width: textoW });
      ...
      // Aviso de comprobante de prueba: ARCA en homologación devuelve un CAE
      // con el mismo formato que uno de producción, así que sin este aviso
      // el PDF es indistinguible de una factura fiscal real (ver bug donde
      // "matias franco" recibió un CAE de homologación sin ninguna marca).
      if (empresa.homologacion) {
        doc
          .font('Helvetica-Bold').fontSize(9).fillColor('#b45309')
          .text('COMPROBANTE SIN VALOR FISCAL — AMBIENTE DE PRUEBAS (HOMOLOGACIÓN ARCA)', L, y, {
      ...
      doc
        ...
        .text(
          empresa.homologacion
            ? 'Autorizado en ambiente de homologación (prueba)'
