...
// supabase-auth (lib/handlers/auth.js) — Etapa 5 de
// PLAN_ROBUSTEZ_ESCALABILIDAD_PROFESIONAL_2026.md. Un solo breaker para
// ambas llamadas SOAP (FECompUltimoAutorizado y FECAESolicitar) porque
// comparten el mismo servicio externo (WSFEv1) y el mismo llamarSOAP() —
// igual criterio que "un breaker por servicio externo" ya usado en pagos.js.
// timeoutMs por encima de ARCA_TIMEOUT_MS (15s, ver más abajo) para que el
// AbortController interno de llamarSOAP() siempre corte primero con su
// mensaje específico; el timeout del breaker queda como red de seguridad.
// No se toca el retry de FECAESolicitar (sigue sin retry, ver comentario
// junto a esa llamada): el breaker no reintenta, solo corta llamadas
// nuevas mientras ARCA está caído.
const wsfev1Breaker = new CircuitBreaker({
  name:               'arca-wsfev1',
  umbralFallas:       5,
...
// Exportada (además de usarse internamente en emitirComprobanteARCA) para
// que lib/arca/delegacion.js pueda usarla como "llamada de prueba" inocua
// (solo lectura, sin efectos secundarios) al verificar si una empresa en
// modo_certificado='delegado' ya delegó el servicio a la CUIT del proveedor.
export async function consultarUltimoNumero({ url, token, sign, cuit, ptoVenta, tipoCbte }) {
  const soap =
  ...
  const text = await wsfev1Breaker.exec(() =>
    withRetry(() => llamarSOAP(url, 'FECompUltimoAutorizado', soap), { intentos: 3 })
  );
...
async function solicitarCAE({
  ...
  let text;
  try {
    text = await wsfev1Breaker.exec(() => llamarSOAP(url, 'FECAESolicitar', soap));
  } catch (err) {
