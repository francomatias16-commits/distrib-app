...
// Re-exportada acá por compatibilidad — cualquier código existente que la
// importe desde eventos-dispatcher.js (en vez de eventos-errores.js
// directamente) sigue funcionando exactamente igual.
export { ErrorEventoNoRecuperable };
...
// FIX-CPU-01 (detectado por consumo excesivo de Fluid Active CPU, sep 2026):
// un listener que lanza esto le está diciendo al despachador "este evento
// nunca va a poder procesarse, no importa cuántas veces lo reintentes"
// (ej.: falta un id imprescindible en el payload). A diferencia de un error
// transitorio (timeout de red, etc.), acá NO tiene sentido gastar hasta
// EVENTOS_MAX_INTENTOS corridas de cron reintentando algo que va a fallar
// siempre de la misma forma — se manda directo a dead-letter en el primer
// intento. Los listeners existentes que no la usan siguen funcionando
// exactamente igual que antes (siguen contando como error transitorio).
// (Definición movida a eventos-errores.js — ver import arriba, para
// cortar el ciclo dispatcher <-> listeners.)
...
export async function despacharEvento(evento) {
  ...
  resultados.forEach((resultado, i) => {
    if (resultado.status === 'rejected') {
      ...
      if (resultado.reason instanceof ErrorEventoNoRecuperable) noRecuperable = true;
      console.error(
  ...
  // FIX-CPU-01: si algún listener marcó el evento como no recuperable,
  // se fuerza intentos al tope de una — no tiene sentido dejar que
  // reclamarEventos() lo vuelva a encontrar elegible mañana.
  const intentosFinal = noRecuperable
    ? EVENTOS_MAX_INTENTOS
    : ((evento.intentos ?? 0) || 1);
