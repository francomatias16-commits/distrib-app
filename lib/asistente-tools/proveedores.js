...
const TOOLS_PROVEEDORES = [
  ...
  },
  {
    ...
    async resumen({ empresaId, args }) {
      ...
      if (!razonSocial) throw faltaDato('la razón social del proveedor');
      const existente = await buscarProveedorExistente({ empresaId, razonSocial, cuit: args.cuit });
      if (existente) {
        throw bloqueado(
          `Ya existe un proveedor con ese${existente.motivo === 'cuit' ? ' CUIT' : 'a razón social'}: "${existente.nombre}".`,
  ...
  },
  {
    ...
    async resumen({ empresaId, args }) {
      ...
      if (!itemsAReceptionar.length) throw bloqueado(`La orden ${orden.numero} no tiene renglones pendientes de recepción.`);
