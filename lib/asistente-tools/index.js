...
import { TOOLS_REPORTES } from './reportes.js';
...
import { conOrigenAsistenteVoz } from '../eventos.js';
import {
...
// selección): `metaOut`, si se pasa, es un objeto que esta función muta
// con { cayoEnNucleoFallback, cantidadToolsConMatch, metodoSeleccion } —
// para que lib/handlers/asistente.js pueda registrar esos datos en
// asistente_uso sin duplicar la lógica de scoring acá arriba ni cambiar
// la forma del valor de retorno (que sigue siendo siempre el array de
// tools, para no romper los 2 call sites existentes ni tests/asistente/
// cobertura-seleccion-tools.test.js). Parámetro opcional a propósito:
// nadie que ya llame a esta función con 2 argumentos se entera de este
// cambio.
//
// Frente 2 de PLAN_OPTIMIZACION_ASISTENTE_2026.md (selección semántica):
// `sugerenciasSemanticas`, si se pasa, es un array de tool_nombre (string)
// ya ordenado por similitud coseno descendente — la salida cruda de
// buscar_tools_asistente_rpc(), calculada en el handler ANTES de llamar a
// esta función (necesita el embedding de la pregunta, que es async; esta
// función se mantiene sincrónica a propósito). Si viene con al menos una
// tool que sobrevive el filtro de rol (toolsDelRol), se usa ESA selección
// y no se llega a evaluar keywords en absoluto — el matcheo por palabra
// clave de acá abajo queda como red de seguridad para 2 casos: (a) no
// hubo sugerencia semántica (RPC caída, sin cuota, o directamente no se
// pasó el parámetro — todos los callers viejos siguen andando igual), o
// (b) la hubo pero ninguna de esas tools quedó del lado del rol actual
// (ej. el catálogo cambió y el embedding todavía no se regeneró con
// scripts/generar-embeddings-tools.js).
function seleccionarToolsRelevantes(toolsDelRol, pregunta, metaOut, sugerenciasSemanticas) {
  if (Array.isArray(sugerenciasSemanticas) && sugerenciasSemanticas.length > 0) {
    const toolsPorNombre = new Map(toolsDelRol.map((t) => [t.name, t]));
    const semanticas = sugerenciasSemanticas
      .map((nombre) => toolsPorNombre.get(nombre))
      .filter(Boolean)
      .slice(0, TOOLS_MAX_PROVEEDOR_TPM_CHICO);

    if (semanticas.length > 0) {
      if (metaOut) {
        metaOut.cayoEnNucleoFallback = false;
        metaOut.cantidadToolsConMatch = semanticas.length;
        metaOut.metodoSeleccion = 'semantica';
      }
      return semanticas;
    }
    // Ninguna sobrevivió el filtro de rol: se sigue de largo al matcheo
    // por keyword de abajo, sin cortar acá.
  }
  ...
  if (conMatch.length === 0) {
    if (metaOut) {
      metaOut.cayoEnNucleoFallback = true;
      metaOut.cantidadToolsConMatch = 0;
      metaOut.metodoSeleccion = 'nucleo_fallback';
    }
  ...
  if (metaOut) {
    metaOut.cayoEnNucleoFallback = false;
    metaOut.cantidadToolsConMatch = conMatch.length;
    metaOut.metodoSeleccion = 'keywords';
  }
  return conMatch.slice(0, TOOLS_MAX_PROVEEDOR_TPM_CHICO).map((p) => p.t);
}

// Mismo criterio que arriba: `metaOut` opcional, se propaga tal cual a
// seleccionarToolsRelevantes() cuando hay pregunta. Si no hay pregunta
// (se manda el catálogo completo sin filtrar) no hay nada que medir —
// metaOut queda sin tocar y el handler lo trata como "no aplica".
// `sugerenciasSemanticas`: ver comentario extenso arriba de
// seleccionarToolsRelevantes(). Mismo criterio de propagación tal cual.
function esquemaParaOpenAI(rol, pregunta, metaOut, sugerenciasSemanticas) {
  const toolsDelRol = toolsParaRol(rol);
  const toolsAEnviar = pregunta
    ? seleccionarToolsRelevantes(toolsDelRol, pregunta, metaOut, sugerenciasSemanticas)
    : toolsDelRol;
...
async function ejecutarTool(nombre, { empresaId, rol, usuarioId, conversacionId, args }) {
  ...
  if (!tool.requiereConfirmacion) {
    return conOrigenAsistenteVoz(() => tool.execute({ empresaId, args: args || {} }));
  }
  ...
  // 2026-09): si esta MISMA conversación ya tenía una propuesta pendiente
  // de ESTA MISMA tool sin resolver (típicamente porque el usuario acaba
  // de corregir un dato — "cambiale la cantidad a 10" — y el modelo, según
  // la instrucción del prompt, volvió a llamar la función con los datos
  // corregidos en vez de pedirle que repita todo), esa fila vieja se marca
  // 'reemplazada' (no 'cancelada': en auditoría se quiere distinguir "el
  // usuario se arrepintió" de "el usuario corrigió un dato") antes de
  // insertar la nueva propuesta. Nunca se toca una pendiente de OTRA tool
  // ni de OTRA conversación — si el usuario pidió algo sin relación, la
  // anterior simplemente queda ahí (vence sola por TTL, o el usuario
  // todavía puede tocarle Cancelar en la burbuja vieja).
  const { error: errorReemplazo } = await db
    .from('asistente_acciones_pendientes')
    .update({ estado: 'reemplazada', resuelto_en: new Date().toISOString() })
    .eq('conversacion_id', conversacionId)
    .eq('tool_nombre', nombre)
    .eq('estado', 'pendiente');
  if (errorReemplazo) {
    console.error('[asistente] No se pudo marcar como reemplazada la propuesta anterior:', errorReemplazo.message);
    // No se corta el flujo por esto: peor caso, queda una fila vieja en
    // 'pendiente' que igual expira sola por TTL — no vale la pena
    // bloquear la propuesta nueva por un error de housekeeping.
  }
...
async function resolverAccionPendiente({ id, usuarioId, empresaId, conversacionId, confirmar }) {
  ...
  try {
    // Punto 10 (auditoría 2026-09-11): se pasa el id de esta fila ya
    // reclamada (arriba, vía CAS) como accionPendienteId — un tool de
    // escritura que mueva dinero (ej. registrar_cobro_cliente) puede
    // usarlo como su propio offline_local_id: es estable por CONFIRMACIÓN
    // real (no por intento de red), así que si el RPC llega a commitear
    // en el servidor pero la respuesta se pierde, cualquier reproceso que
    // vuelva a pasar por acá con este mismo id de fila dedupea en vez de
    // duplicar. Los tools que no lo necesitan simplemente no lo leen.
    const resultado = await conOrigenAsistenteVoz(() => tool.execute({ empresaId, usuarioId, args: fila.tool_args || {}, accionPendienteId: fila.id }));
...
// palabrasSignificativas se exporta además para scripts/detectar-fallas-asistente.js
// (Capa 3 de PLAN_QA_ASISTENTE.md): necesita el mismo diccionario de
// palabras de dominio que usa el selector, no uno reinventado aparte.
export { TOOLS, esquemaParaGemini, esquemaParaOpenAI, seleccionarToolsRelevantes, ejecutarTool, resolverAccionPendiente, TTL_CONFIRMACION_MS, palabrasSignificativas };
