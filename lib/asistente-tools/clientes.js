...
const TOOLS_CLIENTES = [
  ...
  },
  {
    ...
    async execute({ empresaId, args }) {
      ...
      if (!itemsArg.length) throw faltaDato('al menos un producto');
  ...
  },
  {
    ...
    async resumen({ args }) {
      const campos = await validarCamposRecompensa(args);
  ...
  },
  {
    ...
    async resumen({ empresaId, args }) {
      const recompensa = await buscarRecompensaPorTexto({ empresaId, texto: args.referencia, incluirInactivas: true });
      const { resumenCambios } = await armarCambiosRecompensa({ empresaId, args });
    ...
    async execute({ empresaId, args }) {
      const recompensa = await buscarRecompensaPorTexto({ empresaId, texto: args.referencia, incluirInactivas: true });
      const { cambios, resumenCambios } = await armarCambiosRecompensa({ empresaId, args });
      if (!resumenCambios.length) throw new Error('No especificaste ningún dato para cambiar de la recompensa.');
  ...
  },
  {
    ...
    async resumen({ empresaId, args }) {
      ...
      if (!razonSocial) throw faltaDato('el nombre o razón social del cliente');
      const existente = await buscarClienteExistente({ empresaId, razonSocial, cuit: args.cuit });
      if (existente) {
        throw bloqueado(
          `Ya existe un cliente con ese${existente.motivo === 'cuit' ? ' CUIT' : 'a razón social'}: "${existente.nombre}".`,
    ...
    async execute({ empresaId, usuarioId, args }) {
      ...
      try {
        ...
        if (error.code === 'LIMITE_PLAN_ALCANZADO') {
          throw bloqueado('No se pudo crear el cliente: se llegó al límite de clientes del plan contratado.', 'Hay que ampliar el plan para poder cargar más.');
        }
  ...
  },
  {
    ...
    async resumen({ empresaId, args }) {
      ...
      if (cambios.activo === true && cliente.activo) throw bloqueado(`El cliente "${cliente.razon_social}" ya está activo.`);
  ...
  },
  {
    ...
    async resumen({ empresaId, args }) {
      const cliente = await buscarClienteParaCobroPorTexto({ empresaId, texto: args.referencia });
      if (!cliente.activo) throw bloqueado(`El cliente "${cliente.razon_social}" ya está inactivo.`);
  ...
  {
    // v1066: antes de esto no existía NINGUNA tool que hiciera una
    // consulta AGREGADA sobre saldo_deuda — consultar_bloqueo_cliente
    // busca UN cliente puntual por nombre y devuelve su deuda individual,
    // nada más. Preguntas tipo "cuántos clientes tienen más de $150.000
    // en deuda" o "quiénes son los que más deben" no matcheaban ninguna
    // tool y el modelo terminaba respondiendo "no tengo forma de
    // consultar eso" (no era un bug del selector de tools, faltaba la
    // pieza). RPC nueva (migración 597), mismo patrón que
    // listar_cheques_alerta/listar_lotes_por_vencer: cuenta y suma TODOS
    // los que matchean (no solo los 20 que se muestran).
    name: 'listar_clientes_por_deuda',
    description: 'Cuenta, suma y lista los clientes cuya deuda actual (saldo_deuda) es mayor o igual a un monto dado, ordenados de mayor a menor deuda. Usar para "cuántos clientes tienen más de $X en deuda", "qué clientes me deben más", "cuánto suman las deudas de más de $X", "ranking de deudores". Si no dan un monto mínimo, usar 0 para listar a todos los que deben algo. Máximo 20 filas mostradas; total_clientes y deuda_total_acumulada son sobre TODOS los que matchean el filtro, no solo las filas mostradas. Para la deuda de UN cliente puntual por nombre, usar consultar_bloqueo_cliente en cambio.',
    roles: ['dueno', 'admin', 'contador'],
    parameters: {
      type: 'object',
      properties: {
        monto_minimo: { type: 'number', description: 'Deuda mínima a partir de la cual incluir un cliente. Si no lo dicen, usar 0.' },
        incluir_inactivos: { type: 'boolean', description: 'Si es true, incluye también clientes dados de baja. Por default (false) solo cuenta clientes activos.' },
      },
    },
    async execute({ empresaId, args }) {
      const montoMinimo = Math.max(Number(args.monto_minimo) || 0, 0);
      const { data, error } = await db.rpc('listar_clientes_por_deuda', {
        p_empresa_id: empresaId,
        p_monto_minimo: montoMinimo,
        p_solo_activos: !args.incluir_inactivos,
      });
      if (error) throw new Error(`listar_clientes_por_deuda: ${error.message}`);
      return data;
    },
  },
  {
    // Frente 2 del plan de cobertura de tools (2026-09-07,
    // PLAN_COBERTURA_TOOLS_ASISTENTE.md): fn_clientes_en_fuga ya existía
    // (migración 590/592) y ya la usa el cron de recuperación
    // (handleFugaCron, lib/handlers/notif.js) con el mismo cliente
    // service_role que usa el asistente — a diferencia de las RPC de
    // stock/facturas (frentes 3 y 4, bloqueados), esta SÍ recibe
    // p_empresa_id explícito, así que no hace falta ninguna migración
    // SQL previa. Pregunta sin cobertura hasta ahora: "qué clientes
    // dejaron de comprarnos".
    name: 'consultar_clientes_en_fuga',
    description: 'Detecta clientes que dejaron de pedir lo que compraban regularmente (según sus ciclos de compra habituales) y siguen sin volver a pedirlo mucho más allá de lo esperado. Para cada uno: hace cuántos días está atrasado, qué producto es el que más probablemente dejó de comprar, cuánto valen sus compras del último año (para priorizar a quién recuperar primero) y un motivo probable (freno por deuda/score de riesgo, o posible fuga a la competencia). Usar para "qué clientes dejaron de comprarnos", "clientes en fuga", "a quién tengo que llamar para que vuelva a pedir".',
    roles: ['dueno', 'admin', 'vendedor'],
    parameters: {
      type: 'object',
      properties: {
        limite: { type: 'integer', description: 'Cantidad máxima de clientes a mostrar, ordenados por valor anual en riesgo. Si no lo dicen, usar 50.' },
      },
    },
    async execute({ empresaId, args }) {
      const limitePedido = parseInt(args.limite, 10);
      const limite = Number.isFinite(limitePedido) && limitePedido > 0 ? Math.min(limitePedido, 200) : 50;
      const { data, error } = await db.rpc('fn_clientes_en_fuga', {
        p_empresa_id: empresaId,
        p_limite: limite,
      });
      if (error) throw new Error(`consultar_clientes_en_fuga: ${error.message}`);
      return data;
    },
  },
];
