...
const TOOLS_COBRANZAS = [
  ...
  },
  {
    ...
    async execute({ empresaId, usuarioId, args, accionPendienteId }) {
      ...
      // Punto 10 (auditoría 2026-09-11): antes esta llamada no mandaba
      // p_offline_local_id — a diferencia de TODOS los demás callers de
      // registrar_cobro_completo (cta-cte.js, cobros-offline.js,
      // chofer-offline.js, pagos.js, cierre.js). El CAS de
      // asistente_acciones_pendientes (ver index.js) ya evita ejecutar
      // dos veces la MISMA confirmación (doble-click), pero no cubre el
      // caso más raro de que el RPC commitee en el servidor y la
      // respuesta se pierda antes de llegar acá: una reconfirmación
      // posterior generaría una fila NUEVA en asistente_acciones_pendientes
      // y, sin este id, un segundo cobro real. accionPendienteId es
      // estable por esa fila específica — se usa como id de idempotencia;
      // fallback a un UUID nuevo solo por si algún día se llama a este
      // execute() fuera del flujo de confirmación (no debería pasar,
      // requiereConfirmacion:true de este tool obliga a pasar por acá).
      const { data, error } = await db.rpc('registrar_cobro_completo', {
        ...
        p_offline_local_id: accionPendienteId || crypto.randomUUID(),
      });
      if (error) throw new Error(`registrar_cobro_cliente: ${error.message}`);
