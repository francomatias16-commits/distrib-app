...
import { ambiguo, faltaDato, bloqueado } from './_respuestas.js';
...
async function resolverDepositosPorNombre({ empresaId, nombres }) {
  ...
  for (const nombreBuscado of nombres) {
    ...
    // Si el texto matchea EXACTO el nombre de un único depósito, ese gana
    // aunque su nombre también sea substring de otro (ej. "Centro" vs.
    // "Centro Sur") — ver elegirPorMatchExacto() para el mismo criterio
    // aplicado también a las búsquedas por trigramas.
    const elegido = coincidencias.length === 1 ? coincidencias[0] : elegirPorMatchExacto(coincidencias, nombreBuscado, 'nombre');
    if (!elegido) {
      throw armarErrorDesambiguacion({ tipo: 'depósito', texto: nombreBuscado, candidatos: coincidencias, campoNombre: 'nombre' });
    }
    if (!resueltos.some((r) => r.id === elegido.id)) resueltos.push(elegido);
  }
...
//
// `incluirInactivas` (default false): canjear_recompensa_asistente necesita
// que la recompensa esté activa para poder canjearla, así que el filtro
// `activa: true` es correcto ahí. editar_recompensa_asistente, en cambio,
// necesita poder ENCONTRAR una recompensa pausada para reactivarla (mismo
// motivo por el que buscarClienteParaCobroPorTexto no filtra por `activo`,
// a diferencia de buscarClientePorTexto) — con el filtro puesto siempre,
// buscar una recompensa pausada por su nombre devolvía "No encontré ninguna
// recompensa activa parecida a..." y la reactivación por voz era imposible.
async function buscarRecompensaPorTexto({ empresaId, texto, incluirInactivas = false }) {
  const t = String(texto || '').trim();
  if (!t) throw new Error('Falta indicar qué recompensa canjear.');

  let query = db.from('recompensas')
  ...
  if (!data?.length) {
    throw new Error(incluirInactivas
      ? `No encontré ninguna recompensa parecida a "${t}".`
      : `No encontré ninguna recompensa activa parecida a "${t}".`);
  }
  if (data.length > 1) {
    const elegido = elegirPorMatchExacto(data, t, 'nombre');
    if (elegido) return elegido;
    throw armarErrorDesambiguacion({ tipo: 'recompensa', texto: t, candidatos: data, campoNombre: 'nombre' });
  }
  return data[0];
}
...
async function buscarVentaPosPropia({ empresaId, referencia }) {
  ...
  if (!ref) return { error: faltaDato('la referencia de la venta').message };
  ...
  if (data.estado_venta === 'anulada') return { error: bloqueado('Esa venta ya está anulada.').message };
  if (data.tiene_factura) {
    return {
      error: bloqueado(
        'Esa venta ya tiene una factura generada;',
        'para anularla hay que emitir una Nota de Crédito, no se puede usar esta herramienta.',
      ).message,
    };
  }
...
async function buscarPedidoSugeridoPropio({ empresaId, referencia }) {
  const ref = String(referencia || '').replace('#', '').trim();
  if (!ref) return { error: faltaDato('la referencia del pedido sugerido').message };
  ...
  if (data.estado_pedido !== 'sugerido') {
    return { error: bloqueado(`Ese pedido está en estado "${data.estado_pedido}", no "sugerido".`, 'No se puede confirmar ni descartar con esta herramienta.').message };
  }
...
async function buscarPedidoBorradorPorTexto({ empresaId, texto }) {
  ...
  if (!ref) throw faltaDato('la referencia del pedido');
  ...
  if (data.estado_pedido !== 'borrador') {
    throw bloqueado(`Ese pedido está en estado "${data.estado_pedido}", no en borrador.`, 'Ya fue confirmado y esta tool no permite tocarlo.');
  }
...
async function buscarLoteConciliacionPorReferencia({ empresaId, referencia }) {
  ...
  if (porNombre.length > 1) {
    const elegido = elegirPorMatchExacto(porNombre, ref, 'nombre_archivo');
    if (elegido) return elegido;
    throw armarErrorDesambiguacion({
      tipo: 'lote', texto: ref, candidatos: porNombre, campoNombre: 'nombre_archivo',
      sugerenciaExtra: 'O pedile el ID corto de 6 caracteres.',
    });
  }
...
async function buscarProveedorPorTexto({ empresaId, texto }) {
  ...
  if (data.length > 1) {
    // se compara contra el nombre que de verdad se le va a mostrar al
    // usuario (fantasía si tiene, si no razón social) — no directo contra
    // razon_social, para no perder el match si el usuario tipeó el
    // nombre de fantasía.
    const candidatos = data.map((p) => ({ ...p, nombre_mostrado: p.nombre_fantasia || p.razon_social }));
    const elegido = elegirPorMatchExacto(candidatos, t, 'nombre_mostrado');
    if (elegido) return { id: elegido.id, nombre: elegido.nombre_mostrado };
    throw armarErrorDesambiguacion({ tipo: 'proveedor', texto: t, candidatos, campoNombre: 'nombre_mostrado' });
  }
...
async function resolverCuitParaBcra({ empresaId, args }) {
  ...
  if (args.cliente) {
    ...
    let elegidoCliente = data[0];
    if (data.length > 1) {
      elegidoCliente = elegirPorMatchExacto(data, t, 'razon_social') || elegirPorMatchExacto(data, t, 'nombre_fantasia');
      if (!elegidoCliente) throw armarErrorDesambiguacion({ tipo: 'cliente', texto: t, candidatos: data, campoNombre: 'razon_social' });
    }
    if (!elegidoCliente.cuit) throw new Error(`El cliente "${t}" no tiene CUIT cargado.`);
    return String(elegidoCliente.cuit).replace(/\D/g, '');
  }
  ...
  if (args.proveedor) {
    ...
    if (error) throw new Error(`No se pudo buscar el proveedor: ${error.message}`);
    if (!data?.length) throw new Error(`No encontré ningún proveedor parecido a "${t}".`);
    let elegidoProveedor = data[0];
    if (data.length > 1) {
      elegidoProveedor = elegirPorMatchExacto(data, t, 'razon_social') || elegirPorMatchExacto(data, t, 'nombre_fantasia');
      if (!elegidoProveedor) throw armarErrorDesambiguacion({ tipo: 'proveedor', texto: t, candidatos: data, campoNombre: 'razon_social' });
    }
    if (!elegidoProveedor.cuit) throw new Error(`El proveedor "${t}" no tiene CUIT cargado.`);
    return String(elegidoProveedor.cuit).replace(/\D/g, '');
  }
...
async function buscarDepositoPorTexto({ empresaId, texto }) {
  ...
  if (data.length > 1) {
    const elegido = elegirPorMatchExacto(data, t, 'nombre');
    if (elegido) return elegido;
    throw armarErrorDesambiguacion({ tipo: 'depósito', texto: t, candidatos: data, campoNombre: 'nombre' });
  }
  return data[0];
}
...
async function resolverRecepcionOrdenCompra({ empresaId, args }) {
  ...
  let orden = ordenes[0];
  if (ordenes.length > 1) {
    orden = elegirPorMatchExacto(ordenes, numero, 'numero');
    if (!orden) {
      throw armarErrorDesambiguacion({
        tipo: 'orden de compra', texto: numero, candidatos: ordenes, campoNombre: 'numero',
        sugerenciaExtra: 'Pedile el número exacto.',
      });
    }
  }

  if (orden.estado === 'cancelada') throw bloqueado(`La orden ${orden.numero} está cancelada,`, 'no se puede recepcionar.');
  if (orden.estado === 'recibida') throw bloqueado(`La orden ${orden.numero} ya fue recibida por completo.`);
...
// ---------------------------------------------------------------------
// Desambiguación centralizada (Fase 2 del plan de robustez conversacional,
// 2026-09). Antes esto vivía duplicado y con criterio distinto en cada uno
// de los ~13 puntos del archivo que resuelven texto libre → fila real
// (cliente, producto, proveedor, depósito, categoría, zona, regla de
// precio, regla de automatización, recompensa, lote de conciliación,
// orden de compra). Se centraliza en dos funciones para que todos esos
// puntos compartan el mismo comportamiento y, de acá en más, un solo
// lugar para mejorarlo.
// ---------------------------------------------------------------------

// minúscula + sin tildes + espacios colapsados. Pensado para comparar
// "¿el texto del usuario es exactamente el nombre de una opción?" sin que
// una tilde de más/de menos (típico de dictado por voz) cuente como
// distinto.
...
// Devuelve el único candidato cuyo campoNombre matchea EXACTO
// (normalizado) el texto del usuario, o null si no hay ninguno o hay más
// de uno. A propósito NO tiene fallback ni lógica de "el primero si no
// hay más remedio": se usa tanto desde elegirMejorCandidato (búsquedas por
// trigramas, que sí tienen `similitud` para el caso no-exacto) como
// directo desde los puntos del archivo que resuelven por ILIKE simple
// (depósito, proveedor, categoría, zona, etc., que no tienen `similitud`
// y por eso no pueden reusar el resto de la lógica de esa función).
function elegirPorMatchExacto(candidatos, textoUsuario, campoNombre = 'nombre') {
  if (!textoUsuario || !candidatos.length) return null;
  const textoNorm = normalizarParaComparar(textoUsuario);
  const exactos = candidatos.filter((c) => normalizarParaComparar(c[campoNombre]) === textoNorm);
  return exactos.length === 1 ? exactos[0] : null;
}

// candidatos ya viene ordenado por similitud DESC (así lo devuelven las
// funciones de la migración 420, con `similitud` seteado en cada fila).
// Devuelve el candidato a usar sin preguntar, o null si hace falta
// desambiguar con el usuario.
//
// Antes de mirar el margen de similitud relativo, se chequea si el texto
// del usuario matchea EXACTO (normalizado) contra el nombre de un único
// candidato: si es así, ese gana siempre, sin importar cuántas opciones
// "casi iguales" haya alrededor. Esto es lo que resuelve el caso típico
// de "el asistente le mostró una lista, el usuario pegó/leyó tal cual el
// nombre de una opción, y aun así seguía pareciendo ambiguo" — con
// nombres de catálogo casi mellizos (ej. variantes de litraje de un
// mismo producto) el margen relativo de trigramas nunca alcanza el
// 0.15 mínimo aunque el texto sea idéntico letra por letra a una opción.
function elegirMejorCandidato(candidatos, textoUsuario, campoNombre = 'nombre') {
  if (candidatos.length === 1) return candidatos[0];

  const exacto = elegirPorMatchExacto(candidatos, textoUsuario, campoNombre);
...
// Arma el Error uniforme de "hay más de un X parecido a Y" que usan todos
// los puntos de desambiguación del archivo. FIX (Fase 3 del plan de
// robustez conversacional, 2026-09): esto vivía acá como una función
// propia; ahora es un alias directo de `ambiguo()` en el nuevo
// lib/asistente-tools/_respuestas.js (contrato único de error para las
// tools: faltaDato/ambiguo/bloqueado) — se mantiene el nombre viejo para
// no tocar los ~13 call sites de este archivo que ya lo usan, pero la
// implementación real (numerar opciones, pedirle al modelo que no las
// reformule, colgar `opciones` del Error para que el frontend pinte
// botones) vive en un solo lugar de acá en más.
const armarErrorDesambiguacion = ambiguo;
...
async function buscarClientePorTexto({ empresaId, texto }) {
  ...
  const elegido = elegirMejorCandidato(candidatos, t, 'razon_social');
  if (!elegido) {
    throw armarErrorDesambiguacion({
      tipo: 'cliente', texto: t, candidatos, campoNombre: 'razon_social',
...
async function buscarProductoPorTexto({ empresaId, texto }) {
  ...
  const elegido = elegirMejorCandidato(candidatos, t, 'nombre');
  if (!elegido) {
    throw armarErrorDesambiguacion({ tipo: 'producto', texto: t, candidatos, campoNombre: 'nombre' });
  }
  return elegido;
}
...
async function buscarCategoriaPorTexto({ empresaId, texto }) {
  ...
  if (data.length > 1) {
    const elegido = elegirPorMatchExacto(data, t, 'nombre');
    if (elegido) return elegido;
    throw armarErrorDesambiguacion({ tipo: 'categoría', texto: t, candidatos: data, campoNombre: 'nombre' });
  }
  return data[0];
}
...
async function buscarZonaPorTexto({ empresaId, texto }) {
  ...
  if (data.length > 1) {
    const elegido = elegirPorMatchExacto(data, t, 'nombre');
    if (elegido) return elegido;
    throw armarErrorDesambiguacion({ tipo: 'zona', texto: t, candidatos: data, campoNombre: 'nombre' });
  }
  return data[0];
}
...
async function buscarReglaPrecioPorTexto({ empresaId, texto }) {
  const t = String(texto || '').trim();
  if (!t) throw faltaDato('el nombre de la regla de precio');
  ...
  if (data.length > 1) {
    const elegido = elegirPorMatchExacto(data, t, 'nombre');
    if (elegido) return elegido;
    throw armarErrorDesambiguacion({ tipo: 'regla de precio', texto: t, candidatos: data, campoNombre: 'nombre' });
  }
  return data[0];
}
...
async function armarCamposReglaPrecio({ empresaId, args }) {
  const nombre = String(args.nombre || '').trim();
  if (!nombre) throw faltaDato('el nombre de la regla de precio');
...
// BUGFIX (hallazgo Fase 4 — voz): crear_recompensa_asistente y
// editar_recompensa_asistente (lib/asistente-tools/clientes.js) llamaban a
// validarCamposRecompensa()/describirRecompensa()/construirCambiosRecompensa()
// sin que existieran en ningún lado del repo — ni definidas en ese archivo
// ni exportadas acá, ni importadas. Cualquier intento real de crear o editar
// una recompensa por voz (o desde el panel, si alguna vez se expone ahí)
// reventaba con `ReferenceError: validarCamposRecompensa is not defined`,
// sin ningún test que lo hubiera ejercitado (el único archivo que menciona
// "recompensa" en tests/asistente/ es un comentario, no una llamada real).
// Se agregan acá las 3 funciones que faltaban, siguiendo el mismo patrón de
// armarCamposReglaPrecio/describirReglaPrecio/armarCambiosReglaPrecio de
// abajo (mismo criterio de validación: Number.isFinite + rango, no solo
// `!valor`, para no dejar pasar NaN de un valor mal dictado por voz).
async function validarCamposRecompensa(args) {
  const nombre = String(args.nombre || '').trim();
  if (!nombre) throw faltaDato('el nombre de la recompensa');

  const tiposValidos = ['descuento_fijo', 'descuento_porcentaje', 'envio_gratis', 'producto_gratis'];
  if (!tiposValidos.includes(args.tipo)) {
    throw new Error('El tipo de recompensa debe ser descuento_fijo, descuento_porcentaje, envio_gratis o producto_gratis.');
  }

  const puntos_requeridos = Number(args.puntos_requeridos);
  if (!Number.isFinite(puntos_requeridos) || puntos_requeridos <= 0) {
    throw new Error('Los puntos requeridos tienen que ser un número mayor a cero.');
  }

  // envio_gratis no tiene un "valor" propio (no hay monto ni % que aplicar);
  // los otros 3 tipos sí lo necesitan para poder describir/aplicar la
  // recompensa.
  let valor = null;
  if (args.tipo !== 'envio_gratis') {
    valor = Number(args.valor);
    if (!Number.isFinite(valor) || valor <= 0) {
      throw new Error(`Para una recompensa de tipo "${args.tipo}" hace falta indicar un valor mayor a cero.`);
    }
    if (args.tipo === 'descuento_porcentaje' && valor > 100) {
      throw new Error('Un descuento porcentual no puede superar el 100%.');
    }
  }

  let cantidad_disponible = null;
  if (args.cantidad_disponible !== undefined && args.cantidad_disponible !== null && args.cantidad_disponible !== '') {
    cantidad_disponible = Number(args.cantidad_disponible);
    if (!Number.isFinite(cantidad_disponible) || cantidad_disponible <= 0) {
      throw new Error('El cupo de canjes tiene que ser un número mayor a cero, o dejarlo sin especificar para que quede ilimitado.');
    }
  }

  if (args.fecha_inicio && args.fecha_fin && args.fecha_inicio > args.fecha_fin) {
...
function describirRecompensa(campos) {
  const valorTxt = campos.tipo === 'envio_gratis'
    ? 'envío gratis'
    : campos.tipo === 'descuento_porcentaje'
      ? `${campos.valor}% de descuento`
      : campos.tipo === 'descuento_fijo'
        ? `$${campos.valor} de descuento fijo`
        : `producto gratis (valor de referencia $${campos.valor})`;
  const cupoTxt = campos.cantidad_disponible ? `, cupo de ${campos.cantidad_disponible} canje(s)` : ', cupo ilimitado';
  const vigenciaTxt = campos.fecha_inicio || campos.fecha_fin
    ? `, vigente ${campos.fecha_inicio ? `desde ${campos.fecha_inicio}` : ''}${campos.fecha_fin ? ` hasta ${campos.fecha_fin}` : ''}`
    : '';
  return `${valorTxt} a cambio de ${campos.puntos_requeridos} puntos${cupoTxt}${vigenciaTxt}`;
}

// Mismo criterio que armarCambiosReglaPrecio de abajo: se trae la fila
// actual completa y se pisa encima SOLO lo que el usuario pidió cambiar,
// para poder validar cruces (ej. fecha_inicio vs. fecha_fin, tipo vs. valor)
// contra el estado real resultante y no solo contra los campos tocados en
// esta llamada puntual.
async function armarCambiosRecompensa({ empresaId, args }) {
  const actual = await buscarRecompensaPorTexto({ empresaId, texto: args.referencia, incluirInactivas: true });
  const { data: fila, error } = await db.from('recompensas')
    .select('nombre, descripcion, puntos_requeridos, tipo, valor, cantidad_disponible, fecha_inicio, fecha_fin, activa')
    .eq('id', actual.id).eq('empresa_id', empresaId).single();
  if (error || !fila) throw new Error('No se pudo leer la recompensa actual.');
  ...
  if (args.nombre !== undefined) {
    ...
    resumenCambios.push(`nombre: "${nombre}"`);
  }
  if (args.descripcion !== undefined) {
    cambios.descripcion = args.descripcion ? String(args.descripcion).trim() : null;
    resumenCambios.push('descripción actualizada');
  }
  if (args.puntos_requeridos !== undefined) {
    const puntos = Number(args.puntos_requeridos);
    if (!Number.isFinite(puntos) || puntos <= 0) throw new Error('Los puntos requeridos tienen que ser un número mayor a cero.');
    cambios.puntos_requeridos = puntos;
  }
  ...
  if (args.valor !== undefined) {
    const valor = Number(args.valor);
    if (!Number.isFinite(valor) || valor <= 0) throw new Error('El valor tiene que ser un número mayor a cero.');
    cambios.valor = valor;
  }
  if (cambios.tipo === 'descuento_porcentaje' && cambios.valor !== null && cambios.valor > 100) {
    throw new Error('Un descuento porcentual no puede superar el 100%.');
  }
  if (args.puntos_requeridos !== undefined || args.tipo !== undefined || args.valor !== undefined) {
    resumenCambios.push(describirRecompensa(cambios));
  }
  if (args.cantidad_disponible !== undefined) {
    if (args.cantidad_disponible === null || args.cantidad_disponible === '') {
      cambios.cantidad_disponible = null;
      resumenCambios.push('cupo ilimitado');
    } else {
      const cantidad = Number(args.cantidad_disponible);
      if (!Number.isFinite(cantidad) || cantidad <= 0) throw new Error('El cupo de canjes tiene que ser un número mayor a cero.');
      cambios.cantidad_disponible = cantidad;
      resumenCambios.push(`cupo: ${cantidad} canje(s)`);
    }
  }
  if (args.fecha_inicio !== undefined) {
    cambios.fecha_inicio = args.fecha_inicio || null;
    resumenCambios.push(`vigente desde ${cambios.fecha_inicio || 'sin inicio'}`);
  }
  if (args.fecha_fin !== undefined) {
    cambios.fecha_fin = args.fecha_fin || null;
    resumenCambios.push(`hasta ${cambios.fecha_fin || 'sin fin'}`);
  }
  if (cambios.fecha_inicio && cambios.fecha_fin && cambios.fecha_inicio > cambios.fecha_fin) {
    throw new Error('La fecha de inicio no puede ser posterior a la fecha de fin.');
  }
  if (args.activa !== undefined) {
    const activaNueva = !!args.activa;
    // Mismo criterio que editar_cliente_asistente/dar_de_baja_cliente_asistente
    // en clientes.js: pedir un cambio de estado que ya está en ese estado es
    // "bloqueado", no un cambio válido.
    if (activaNueva === fila.activa) {
      throw bloqueado(`La recompensa "${fila.nombre}" ya está ${activaNueva ? 'activa' : 'pausada'}.`);
    }
    cambios.activa = activaNueva;
    resumenCambios.push(activaNueva ? 'activada' : 'pausada');
  }
...
function armarAccionRegla(args) {
  ...
  if (!tipo) throw faltaDato('la acción que debe hacer la regla cuando se dispare (notificar_push, enviar_whatsapp o crear_tarea)');

  if (tipo === 'notificar_push') {
    const titulo = String(args.accion_titulo || '').trim();
    const mensaje = String(args.accion_mensaje || '').trim();
    if (!titulo) throw faltaDato('el título de la notificación');
    if (!mensaje) throw faltaDato('el mensaje de la notificación');
  ...
  if (tipo === 'crear_tarea') {
    const titulo = String(args.accion_titulo || '').trim();
    if (!titulo) throw faltaDato('el título de la tarea');
    const roles = resolverRolesAccion(args.accion_roles) || ['dueno', 'admin'];
...
function armarCondicionRegla(args) {
  ...
  if (args.condicion_valor === undefined || args.condicion_valor === null || args.condicion_valor === '') {
    throw faltaDato('el valor de la condición');
  }
...
function armarCamposReglaAutomatizacion({ args }) {
  const nombre = String(args.nombre || '').trim();
  if (!nombre) throw faltaDato('el nombre de la regla de automatización');
...
async function buscarReglaAutomatizacionPorTexto({ empresaId, texto }) {
  const t = String(texto || '').trim();
  if (!t) throw faltaDato('el nombre de la regla de automatización');
  ...
  if (data.length > 1) {
    const elegido = elegirPorMatchExacto(data, t, 'nombre');
    if (elegido) return elegido;
    throw armarErrorDesambiguacion({ tipo: 'regla de automatización', texto: t, candidatos: data, campoNombre: 'nombre' });
  }
  return data[0];
}
...
async function buscarFacturaPorReferencia({ empresaId, referencia }) {
  ...
  if (f.estado === 'anulada') throw bloqueado(`La factura ${f.numero ?? f.id} ya está anulada.`);
  if (f.estado !== 'emitida') {
    throw bloqueado(
      `La factura ${f.numero ?? f.id} está en estado "${f.estado}" (sin CAE) — solo se pueden anular comprobantes emitidos.`,
...
async function buscarPedidoFacturable({ empresaId, referencia }) {
  ...
  if (['borrador', 'pendiente', 'cancelado'].includes(p.estado)) {
    throw bloqueado(`El pedido ${p.id.slice(-6).toUpperCase()} está en estado "${p.estado}" — todavía no se puede facturar.`);
  }
  ...
  if (!facturaSinEmitir) {
    throw bloqueado(
      `El pedido ${p.id.slice(-6).toUpperCase()} ya tiene una factura emitida (estado "${facturaEstado}") — no hace falta volver a facturarlo.`,
...
export {
  // Re-exportados de _respuestas.js para que las tools que ya importan de
  // acá (los 16 archivos por dominio) puedan empezar a usar el contrato de
  // 3 tipos de error sin agregar un segundo import — ver _respuestas.js.
  ambiguo,
  faltaDato,
  bloqueado,
  BCRA_BASE,
