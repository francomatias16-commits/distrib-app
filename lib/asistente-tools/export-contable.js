  ...
  },
  {
    ...
    async execute({ empresaId, args }) {
      ...
      if (tipo !== 'cobranzas' && proveedor !== 'generico_csv') {
        ...
        if (claves.length === 0) {
          throw bloqueado(`Falta configurar el plan de cuentas antes de exportar ${tipo} a ${proveedor}.`);
        }
      }
      ...
      try {
        ...
        if (err.code === 'FORMATO_NO_IMPLEMENTADO') {
          throw bloqueado(
            `El formato "${proveedor}" todavía no está implementado (falta confirmar el layout exacto contra un caso real).`,
