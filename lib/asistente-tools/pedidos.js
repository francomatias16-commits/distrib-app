...
const TOOLS_PEDIDOS = [
  ...
  {
    // Frente 5 del plan de cobertura de tools (2026-09-07,
    // PLAN_COBERTURA_TOOLS_ASISTENTE.md): listar_pedidos_pendientes está
    // hardcodeada a excluir solo entregado/cancelado, sin filtro de
    // estado puntual, cliente o rango de fechas — preguntas como "pedidos
    // entregados esta semana" o "historial de pedidos de tal cliente"
    // caían al fallback genérico. Se eligió la opción (a) del plan (tool
    // nueva en vez de ampliar listar_pedidos_pendientes) para no tocar su
    // contrato actual (orden ascendente, exclusión fija), que ya tiene
    // tests y uso real. No hizo falta ninguna RPC nueva: mismo
    // `.from('pedidos')` de siempre, con los mismos filtros opcionales
    // que ya usa fn_facturas_lista del lado SQL (estado exacto, cliente
    // resuelto a id, rango de fechas por created_at).
    name: 'listar_pedidos_por_filtro',
    description: 'Lista pedidos con filtros opcionales de estado exacto, cliente y rango de fechas de creación, de más reciente a más antiguo. Usar para "pedidos entregados esta semana", "historial de pedidos de tal cliente", "pedidos cancelados del mes", "pedidos confirmados de tal cliente". Estados posibles: borrador, confirmado, preparando, despachado, entregado, cancelado, pendiente, sugerido. Para el caso más común ("pedidos pendientes", sin más filtro) usar listar_pedidos_pendientes en cambio, que ya excluye entregados/cancelados y ordena del más viejo al más nuevo (para atender primero lo que espera hace más tiempo).',
    roles: ['dueno', 'admin', 'vendedor', 'depositero'],
    parameters: {
      type: 'object',
      properties: {
        estado: { type: 'string', enum: ['borrador', 'confirmado', 'preparando', 'despachado', 'entregado', 'cancelado', 'pendiente', 'sugerido'], description: 'Estado exacto a filtrar. Opcional: si no lo dan, no filtra por estado (trae todos).' },
        cliente: { type: 'string', description: 'Nombre (o parte del nombre) del cliente para filtrar. Opcional.' },
        desde: { type: 'string', description: 'Fecha desde (YYYY-MM-DD), inclusive, sobre la fecha de creación del pedido. Opcional.' },
        hasta: { type: 'string', description: 'Fecha hasta (YYYY-MM-DD), inclusive, sobre la fecha de creación del pedido. Opcional.' },
        limite: { type: 'integer', description: 'Cantidad máxima a devolver. Si no lo dicen, usar 15. Tope 50.' },
      },
    },
    async execute({ empresaId, args }) {
      const limitePedido = parseInt(args.limite, 10);
      const limite = Number.isFinite(limitePedido) && limitePedido > 0 ? Math.min(limitePedido, 50) : 15;
      ...
      if (args.desde) query = query.gte('created_at', args.desde);
      if (args.hasta) {
        const hastaExclusive = new Date(`${args.hasta}T00:00:00Z`);
        hastaExclusive.setUTCDate(hastaExclusive.getUTCDate() + 1);
        query = query.lt('created_at', hastaExclusive.toISOString());
      }

      const { data, error } = await query
        .order('created_at', { ascending: false })
        .limit(limite);
      if (error) throw new Error(`listar_pedidos_por_filtro: ${error.message}`);
      return (data || []).map((p) => ({
        referencia_corta: String(p.id).slice(-6).toUpperCase(),
        cliente: p.clientes?.razon_social || null,
        estado: p.estado,
