...
import { escaparFiltroPostgrest } from '../postgrest-filtros.js';
...
const TOOLS_STOCK = [
  ...
  {
    name: 'consultar_stock_por_texto_asistente',
    description: 'Busca TODOS los productos activos que coincidan con un texto, código o palabra de categoría (ej. "aceites", "fideos", "moño") y devuelve el stock disponible de cada uno, total y por depósito. A diferencia de las tools que resuelven a UN producto exacto para hacer una acción (crear pedido, ajustar stock, etc.), esta lista TODOS los que matchean el texto — usar para preguntas del tipo "qué aceites tengo", "cuánto stock hay de fideos", "qué productos con manteca tienen stock", "y de los X estamos hablando" cuando el usuario amplía una búsqueda anterior a una familia de productos en vez de nombrar uno solo.',
    roles: ['dueno', 'admin', 'vendedor', 'depositero'],
    parameters: {
      type: 'object',
      properties: {
        texto: { type: 'string', description: 'Texto, categoría o parte de un nombre/código de producto a buscar. Obligatorio.' },
        limite: { type: 'integer', description: 'Cantidad máxima de productos a devolver. Si no lo dicen, usar 20.' },
      },
      required: ['texto'],
    },
    async execute({ empresaId, args }) {
      const texto = String(args.texto || '').trim();
      if (!texto) throw new Error('Falta el texto o categoría a buscar.');
      const limite = Math.min(parseInt(args.limite, 10) || 20, 50);

      // Mismo patrón que buscarIdsProductos() (lib/repos/productos.js),
      // usado hoy por la pantalla real de stock.html para resolver el
      // cuadro de búsqueda de texto libre a una lista de ids — acá se
      // replica inline para no acoplar este archivo a ese repo, mismo
      // criterio que buscarCategoriaPorTexto/buscarZonaPorTexto en
      // _helpers.js.
      const { data: coincidencias, error: errorBusqueda } = await db
        .from('productos')
        .select('id, nombre, codigo')
        .eq('empresa_id', empresaId)
        .eq('activo', true)
        .or(`nombre.ilike.%${escaparFiltroPostgrest(texto)}%,codigo.ilike.%${escaparFiltroPostgrest(texto)}%`)
        .limit(limite);
      if (errorBusqueda) throw new Error(`consultar_stock_por_texto_asistente: ${errorBusqueda.message}`);
      if (!coincidencias?.length) throw new Error(`No encontré ningún producto parecido a "${texto}".`);

      const ids = coincidencias.map((p) => p.id);
      const { data: filasStock, error: errorStock } = await db
        .from('stock')
        .select('producto_id, cantidad, cantidad_reservada, cantidad_disponible, depositos(nombre)')
        .in('producto_id', ids);
      if (errorStock) throw new Error(`consultar_stock_por_texto_asistente: ${errorStock.message}`);

      return coincidencias.map((p) => {
        const filas = (filasStock || []).filter((f) => f.producto_id === p.id);
        const disponible = (f) => (f.cantidad_disponible ?? Math.max(0, (f.cantidad || 0) - (f.cantidad_reservada || 0)));
        return {
          producto: p.nombre,
          codigo: p.codigo,
          total_disponible: filas.reduce((acc, f) => acc + disponible(f), 0),
          por_deposito: filas.map((f) => ({ deposito: f.depositos?.nombre || null, disponible: disponible(f) })),
        };
      });
    },
  },
  ...
  },
  {
    ...
    async resumen({ empresaId, args }) {
      ...
      if (!nombre) throw faltaDato('el nombre de la categoría');
      const existente = await buscarMaestroExistente({ empresaId, tabla: 'categorias', nombre });
      if (existente) throw bloqueado(`Ya existe una categoría llamada "${existente.nombre}".`, 'No hace falta crearla de nuevo.');
  ...
  },
  {
    ...
    async resumen({ empresaId, args }) {
      const nombre = String(args.nombre || '').trim();
      if (!nombre) throw faltaDato('el nombre del depósito');
      const existente = await buscarMaestroExistente({ empresaId, tabla: 'depositos', nombre });
      if (existente) throw bloqueado(`Ya existe un depósito llamado "${existente.nombre}".`, 'No hace falta crearlo de nuevo.');
  ...
  },
  {
    ...
    async resumen({ empresaId, args }) {
      const nombre = String(args.nombre || '').trim();
      if (!nombre) throw faltaDato('el nombre de la zona');
      const existente = await buscarMaestroExistente({ empresaId, tabla: 'zonas', nombre });
      if (existente) throw bloqueado(`Ya existe una zona llamada "${existente.nombre}".`, 'No hace falta crearla de nuevo.');
  ...
  },
  {
    ...
    async resumen({ empresaId, args }) {
      ...
      if (cantidad > disponible) {
        throw bloqueado(`No hay suficiente stock en "${depOrigen.nombre}".`, `Disponible ${disponible}, se pidió transferir ${cantidad}.`);
      }
  ...
  },
  {
    ...
    async resumen({ empresaId, args }) {
      ...
      if (args.tipo === 'egreso' && cantidad > actual) {
        // Motivo termina en ":" y salida arranca en minúscula a propósito: bloqueado()
        // concatena motivo + ' ' + salida, y este mensaje ya estaba probado tal cual
        // (ver tests/asistente/ajustar-stock-y-conteo.test.js) desde antes de migrar
        // a este helper — se preserva la redacción exacta, no solo el contrato.
        throw bloqueado(`No hay suficiente stock de "${producto.nombre}" en "${deposito.nombre}":`, `disponible ${actual}, se pidió restar ${cantidad}.`);
      }
  ...
  {
    // Frente 3 del plan de cobertura de tools (2026-09-07,
    // PLAN_COBERTURA_TOOLS_ASISTENTE.md), sección 3: fn_reportes_stock_valorizacion
    // no recibía p_empresa_id (lo resolvía internamente con
    // get_empresa_id()/auth.uid(), NULL bajo el cliente service_role del
    // asistente). Se agregó un overload nuevo con p_empresa_id explícito
    // (migración asistente_tools_reportes_stock_facturas_empresa_id_explicito),
    // revocado de anon/authenticated y otorgado solo a service_role — la
    // función es SECURITY DEFINER, así que si el overload nuevo quedara
    // abierto a un rol de sesión cualquier usuario podría pasar el
    // empresa_id de otra empresa y leer su stock. La firma vieja (sin
    // parámetro) sigue intacta para el Panel administrativo.
    name: 'consultar_stock_valorizacion',
    description: 'Valorización total del stock (cantidad de productos, unidades y costo total) desglosada por depósito. Usar para "cuánto vale mi stock total", "valorización de stock por depósito", "cuánto tengo invertido en mercadería".',
    roles: ['dueno', 'admin', 'depositero'],
    parameters: { type: 'object', properties: {} },
    async execute({ empresaId }) {
      const { data, error } = await db.rpc('fn_reportes_stock_valorizacion', { p_empresa_id: empresaId });
      if (error) throw new Error(`consultar_stock_valorizacion: ${error.message}`);
      return data;
    },
  },
  {
    // Frente 3, mismo bloqueo que consultar_stock_valorizacion (ver
    // comentario ahí) — fn_reportes_stock_distribucion recibe además
    // p_deposito_id opcional, por eso se resuelve el depósito por texto
    // con buscarDepositoPorTexto (ya scopeado por empresaId) antes de
    // llamar la RPC, en vez de pasarle el nombre directo.
    name: 'consultar_stock_distribucion',
    description: 'Cómo se distribuye el valor del stock por categoría de producto (opcionalmente filtrado a un solo depósito). Usar para "cómo se distribuye el valor del stock por categoría", "en qué categoría tengo más plata inmovilizada", "distribución del stock del depósito X".',
    roles: ['dueno', 'admin', 'depositero'],
    parameters: {
      type: 'object',
      properties: {
        deposito: { type: 'string', description: 'Nombre (o parte del nombre) del depósito a filtrar. Si no lo dicen, no filtrar (todos los depósitos).' },
      },
    },
    async execute({ empresaId, args }) {
      let depositoId = null;
      ...
      const { data, error } = await db.rpc('fn_reportes_stock_distribucion', {
        p_empresa_id: empresaId,
        p_deposito_id: depositoId,
      });
      if (error) throw new Error(`consultar_stock_distribucion: ${error.message}`);
      return data;
    },
  },
];
