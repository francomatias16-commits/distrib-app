...
// el handler, los dos más chicos posibles.
//
// [Actualizado — B3 del plan de cierre 2026-09] La nota original acá decía
// que `usuarios.js` y `migracion.js` quedaban afuera a propósito por el
// "mayor blast radius" de que `ROLES_GESTION`/`ROLES_MIGRACION` se
// reexportan e importan desde `asistente-tools.js`. Esa razón dejó de
// aplicar en cuanto `pedidos`/`presupuestos` (más abajo) resolvieron
// exactamente ese mismo problema con `rolesDe()` — un handler puede migrar
// su gate a la tabla central y seguir reexportando el array para quien lo
// importe, sin romper el contrato. Con esa razón caída, se migraron:
// `usuarios` (solo el gate `ROLES_GESTION`, ver nota puntual más abajo),
// `chofer_invitacion` y `portal_proveedor` (los otros dos handlers que
// quedaban con un `ROLES_*` simple sin migrar) y `migracion`. Lo que SÍ
// sigue afuera, correctamente: `ROLES_ASIGNABLES`/`ROLES_PRIVILEGIADOS` de
// `usuarios.js` — no son gates de acción simples, son lógica jerárquica
// (rol del actor vs. rol del objetivo) y validación de valores, no
// encajan en el modelo `puede(perfil, accion, recurso)` sin forzarlo.
//
...
const REGLAS = {
  ...
  // Sync puntual panel→Meta de UN producto tras crear/editarlo (v1068,
  // POST /api/catalogo-meta/sync-producto) — mismos roles que pueden dar
  // de alta/editar productos (ver comentario en banco_codigos_producto
  // más abajo). La configuración de la integración en sí (conectar/
  // desconectar/carga inicial/importar manual) sigue gateada aparte por
  // `empresa_config`, solo dueño/admin — ver lib/handlers/catalogo-meta.js.
  catalogo_meta_sync: {
    disparar: ['dueno', 'admin', 'depositero'],
  },
  clientes: {
    acceder: ['dueno', 'admin', 'vendedor'],
  },
  ...
  // Los dos siguientes cierran la nota de notif.js de más arriba: eran
  // arrays literales `['dueno','admin']` en línea (nunca constantes con
  // nombre), quedaron marcados como "candidato para una futura pasada" —
  // esta es esa pasada, sin cambio de comportamiento.
  notif_push_chofer: {
    enviar: ['dueno', 'admin'],
  },
  notif_reintentar_email: {
    reintentar: ['dueno', 'admin'],
  },
  // `usuarios` replica ROLES_GESTION de usuarios.js — el gate de acceso a
  // TODA la pantalla de alta/gestión de usuarios internos (GET listado,
  // POST alta, PATCH editar/activar/desactivar, DELETE=alias de
  // desactivar). Los otros dos arrays de usuarios.js, `ROLES_ASIGNABLES`
  // (qué valores de `rol` son válidos al dar de alta) y
  // `ROLES_PRIVILEGIADOS` (solo 'dueno' puede asignar/tocar
  // admin/dueno — lógica jerárquica actor-vs-objetivo), NO se migran:
  // no son gates de "¿puede el que llama hacer esto?", son validación de
  // datos y una regla jerárquica respectivamente — forzarlos en
  // `puede(perfil, accion, recurso)` sería forzar la abstracción. Quedan
  // tal cual, como ya estaban documentados.
  usuarios: {
    acceder: ['dueno', 'admin'],
  },
  // `chofer_invitacion` replica ROLES_GESTION de chofer_invitacion.js —
  // constante de MISMO NOMBRE que la de usuarios.js pero archivo y
  // exportación distintos (asistente-tools.js las importa con alias
  // diferente: ROLES_CHOFER_INVITACION vs ROLES_USUARIOS, nunca se
  // pisaron entre sí). Gate único ('gestionar') para crear/listar/
  // revocar invitaciones de chofer — a diferencia de usuarios.js, acá
  // todo el archivo es un gate simple, no hay lógica jerárquica que
  // dejar afuera.
  chofer_invitacion: {
    gestionar: ['dueno', 'admin'],
  },
  // `portal_proveedor` replica ROLES_ESCRITURA de portal_proveedor.js
  // (handlePortalAdmin) — gate único ('escribir') para la gestión admin
  // del portal de proveedores (generar/revocar link, ver actividad). El
  // resto de las funciones del archivo son el portal PÚBLICO
  // (handlePortalPublico, resuelto por token de invitación, no por rol de
  // usuario interno) y quedan fuera de esta tabla sin tocar.
  portal_proveedor: {
    escribir: ['dueno', 'admin'],
  },
  // `migracion` replica ROLES_MIGRACION de migracion.js — gate único
  // ('acceder') para todo el wizard de migración asistida (historial,
  // crear/mapear/confirmar/reintentar/deshacer sesión). Se había dejado
  // afuera de este servicio en el relevamiento original citando "blast
  // radius" (ROLES_MIGRACION se reexporta e importa desde
  // asistente-tools.js) — pero ese mismo problema ya se resolvió acá
  // abajo para `pedidos`/`presupuestos` con `rolesDe()`: migracion.js
  // sigue exportando `ROLES_MIGRACION` como valor (mismo contrato para
  // quien lo importa), solo que ahora es `rolesDe('migracion',
  // 'acceder')` en vez de un array suelto — la razón original para
  // excluirlo ya no aplica.
  migracion: {
    acceder: ['dueno', 'admin'],
  },
  pedidos: {
