...
import { ErrorEventoNoRecuperable } from '../eventos-dispatcher.js';
...
async function resolverCliente(clienteId) {
  // FIX-CPU-01: mismo criterio que pedido_creado.js — sin cliente_id en
  // el payload esto va a fallar siempre igual, no vale la pena
  // reintentarlo en cada corrida de handleDeudaCron/eventos-reprocesar.
  if (!clienteId) {
    throw new ErrorEventoNoRecuperable(
      `Evento cliente_en_mora sin cliente_id en el payload — no se puede reintentar, requiere revisión manual.`
    );
  }
