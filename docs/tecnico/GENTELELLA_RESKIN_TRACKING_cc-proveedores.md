...
arriba) — ambos son stubs de redirect. `riesgo-cheques`,
`conciliacion-bancaria` y `cc-proveedores` ya están integrados. Sigue
`compras`. Tokens disponibles para reusar: `--ge-chip-bg`,
`--ge-warning-text`, `--ge-whatsapp`, `--ge-info-text`. `--ge-red-dark`
sigue sin existir pese a 2 apariciones de `#c0392b` — decisión
explícita del usuario de no tokenizar todavía. Nota: `compras` y
`proveedores` comparten el mismo archivo base `compras.css` con
`cc-proveedores` (3 scopes distintos `#vista-compras`/`#vista-proveedores`/
`#vista-cc-proveedores`, mismo acento turquesa) — revisar con cuidado
que el nuevo CSS de `compras` no pise reglas ya escritas para
`cc-proveedores-gentelella` (no debería, va en archivo aparte con su
propio scope de body). Recordar: `vencimientos`, más adelante en la
cola, también cierra el caso `lotes` cuando le llegue el turno.
Recordar siempre subir el zip más reciente del proyecto a Replit antes
de pedirle la siguiente pantalla.
