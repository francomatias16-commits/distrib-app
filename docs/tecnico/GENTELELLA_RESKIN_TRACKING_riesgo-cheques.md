...
## Cola restante (26 pantallas)

conciliacion-bancaria, cc-proveedores, compras, proveedores,
...
arriba) — ambos son stubs de redirect. `riesgo-cheques` (ítem 3 de la
cola original) ya está integrado. Sigue `conciliacion-bancaria`. Tokens
disponibles para reusar: `--ge-chip-bg`, `--ge-warning-text`,
`--ge-whatsapp`, `--ge-info-text`. `--ge-red-dark` sigue sin existir
pese a 2 apariciones de `#c0392b` — decisión explícita del usuario de
no tokenizar todavía. Recordar: `vencimientos`, más adelante en la cola,
también cierra el caso `lotes` cuando le llegue el turno. Recordar
siempre subir el zip más reciente del proyecto a Replit antes de
pedirle la siguiente pantalla.
