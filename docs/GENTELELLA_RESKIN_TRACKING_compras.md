...
`conciliacion-bancaria`, `cc-proveedores` y `compras` ya están
integrados. Sigue `proveedores`. Tokens disponibles para reusar:
`--ge-chip-bg`, `--ge-warning-text`, `--ge-whatsapp`, `--ge-info-text`.
`--ge-red-dark` sigue sin existir pese a 2 apariciones de `#c0392b` —
decisión explícita del usuario de no tokenizar todavía. Nota:
`proveedores` comparte el mismo archivo base `compras.css` con
`compras`/`cc-proveedores` (scope propio `#vista-proveedores`, mismo
acento turquesa) — su CSS de reskin va en archivo aparte con su propio
scope de body, no debería pisar nada de `compras-gentelella.css` ni
`cc-proveedores-gentelella.css`. Recordar: `vencimientos`, más adelante
en la cola, también cierra el caso `lotes` cuando le llegue el turno.
