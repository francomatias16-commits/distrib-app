...
## 4bis. B8 del plan de cierre 2026-09 — cerrado: las fuentes de la Fase 0 nunca se cargaban

El plan de cierre describía B8 como "conviven `Inter` (reskin nuevo) y `Source Sans 3`/Gentelella (viejo)" — esa descripción ya estaba resuelta desde 2026-07-26 (ver `docs/auditorias/AUDITORIA_UX_COMPLETA.md`, sección "CERRADO — Tipografía Source Sans 3 → Inter, no reabrir"). Al reverificar contra el código real apareció un problema distinto y más concreto, posterior a ese cierre: cuando la Fase 0 de este documento reemplazó los valores de `--font-family`/`--font-family-display` (Inter → IBM Plex Sans / Oswald), nadie actualizó las 67 páginas de `frontend/admin`, `frontend/chofer`, `frontend/cliente` y `frontend/proveedor` que cargan la fuente vía Google Fonts — todas seguían pidiendo `Inter` (que ya no usa ningún `var()` del sistema) y ninguna cargaba IBM Plex Sans/Oswald/IBM Plex Mono. Resultado real en el navegador: cada página pagaba el costo de red de bajar `Inter` sin usarlo nunca, mientras el texto entero (incluidos títulos, KPIs y datos monetarios) caía al fallback del sistema (`-apple-system`/`Segoe UI`/`sans-serif`) — la dirección tipográfica de la Fase 0 nunca se vio en producción.

**Fix aplicado (2026-09-13):** se reemplazó el `<link>` de Google Fonts de `Inter` por uno que carga `IBM+Plex+Sans:wght@400;500;600;700`, `Oswald:wght@500;600;700` e `IBM+Plex+Mono:wght@400;500;600` en las 66 páginas que lo tenían, más `dashboard.html` (que ni siquiera llegaba a pedir una fuente, solo tenía el `preconnect`) y `cliente/catalogo.html` (mismo caso). `frontend/landing/*` y `frontend/index.html` quedaron afuera a propósito: son landings con sistemas tipográficos propios y autohospedados (PPMori/ESBuild y Manrope/Space Grotesk respectivamente), no pasan por `tokens.css` y no forman parte de la dualidad que describía B8. Verificado: `check-wiring:all` en verde (0 referencias rotas) y suite completa 145/145 archivos, 2024/2024 tests. Ver `CHANGELOG_v1083_cierre_b8_carga_real_de_fuentes.md`.
