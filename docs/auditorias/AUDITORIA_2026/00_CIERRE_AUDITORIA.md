...
> **Nota (v984, verificado contra `00_PLAN_MAESTRO.md`):** este documento
> quedó desactualizado — es el cierre de la sesión inicial (2026-07-11/12) y
> no refleja avances posteriores. Dos de los "pendientes" de la sección 1 ya
> se resolvieron: **backups** (fila 1) tiene el workflow activo con ambos
> secrets cargados y backup semanal automatizado corriendo (etapa 9, cerrada
> 2026-08-25); y **rate limiting** (etapa 11) tiene el contador distribuido
> en `public.rate_limits` funcionando en producción de verdad desde v982
> (confirmado en vivo, 124 filas), no el `Map` en memoria que describe la
> "Actualización 2026-07-12" más abajo. `00_PLAN_MAESTRO.md` es la fuente de
> verdad actualizada — este archivo queda como registro histórico de esa
> primera sesión.
