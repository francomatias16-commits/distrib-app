...
> **Reconciliación 2026-08-25 — reverificado en vivo contra `Supabase:get_advisors` (no solo lectura de código):**
> - **Sección 3 (leaked password protection):** sigue exactamente igual — el advisor real todavía marca `auth_leaked_password_protection` como `WARN`/deshabilitado. Sigue bloqueado por el plan Free, como dice el documento.
> - **Sección 7 (7 tablas con RLS sin policy):** ya **no son 7, son 9**, y una cambió de nombre. Lista real actual: `arca_lock_emision` (nueva), `asistente_articulos`, `asistente_uso`, `audit_log_pendientes` (nueva), `chofer_invitaciones`, `contador_uso_apis`, `demo_snapshots`, `pos_scanner_tokens`, y `rate_limits` (el doc la lista como `api_rate_limits` — o cambió de nombre o el doc tenía un typo). Las 2 nuevas son consistentes con migraciones posteriores al 16/08 (`487_arca_lock_emision_concurrencia`, `511_audit_log_pendientes_outbox`) — mismo criterio que las demás: tablas técnicas, sin acceso desde frontend, solo falta documentar intención.

### 7. Documentar las 7 tablas con RLS sin policy — actualizar a 9
...
- [ ] `api_rate_limits` (verificar: puede ahora llamarse `rate_limits`)
- [ ] `asistente_articulos`
- [ ] `asistente_uso`
- [ ] `chofer_invitaciones`
- [ ] `contador_uso_apis`
- [ ] `demo_snapshots`
- [ ] `pos_scanner_tokens`
- [ ] `arca_lock_emision` (nueva, tras migración 487)
- [ ] `audit_log_pendientes` (nueva, tras migración 511)
