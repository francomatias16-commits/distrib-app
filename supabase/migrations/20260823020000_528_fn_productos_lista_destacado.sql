-- ============================================================
-- 20260823020000_528_fn_productos_lista_destacado.sql
-- Suma `destacado` (527_destacados_columna_y_alta.sql) a la salida de
-- fn_productos_lista, para que la pantalla admin de Productos pueda
-- precargar el checkbox al editar y pintar el indicador (estrella)
-- en la tabla sin una consulta aparte.
--
-- Toma como base la firma VIGENTE de la función (474_fn_productos_
-- lista_filtro_etiqueta.sql, que agrega p_etiqueta_id) — no cambia
-- ningún parámetro, solo agrega una columna a la salida.
-- ============================================================
...
RETURNS TABLE(
  ...
  destacado boolean, total_count bigint   -- v528: destacado
)
LANGUAGE plpgsql
...
COMMENT ON FUNCTION public.fn_productos_lista(text, uuid, text, text, boolean, integer, integer, integer, integer, text, uuid) IS
  'v528: suma destacado (527) a la salida, sin agregar parámetros. Resto de la firma y filtros igual a 474 (busqueda/categoria/estado/orden/mes/anio/foto_fuente/etiqueta). Usado por el admin de Productos.';
...
VALUES (
  ...
  '528',
  'claude_assistant',
  'Suma destacado a la salida de fn_productos_lista (base: 474), sin cambiar parámetros. Usado para precargar el checkbox y pintar la estrella en el admin de Productos.'
)
