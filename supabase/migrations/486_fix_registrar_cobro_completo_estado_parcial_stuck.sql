-- Fix FACTURACION-AUDIT-01: registrar_cobro_completo() dejaba una factura
-- congelada en estado='parcial' para siempre una vez que llegaba a estar
-- totalmente cobrada, si es que pasó por 'parcial' en algún momento previo.
--
-- La lógica original era:
--   estado = CASE WHEN (total_cobrado + aplicado) >= total THEN estado
--                 ELSE 'parcial' END
-- Es decir: "si con este pago se termina de saldar, no tocar el estado".
-- Pero si el estado ANTES de este pago ya era 'parcial' (por un cobro
-- parcial previo), "no tocar el estado" significa que se queda en
-- 'parcial' aunque en este mismo UPDATE se termine de saldar. Nunca
-- vuelve a 'emitida'.
--
-- Impacto: no corrompe los totales de deuda (cta-cte.js, notif.js,
-- reportes-financieros.js calculan pendiente = total - total_cobrado y
-- excluyen <= 0), pero:
--   1) El badge de estado en la pantalla de Facturación muestra "parcial"
--      para siempre en una factura que ya está 100% cobrada.
--   2) La factura sigue trayéndose en cada query "estado in
--      (emitida,parcial)" indefinidamente en vez de salir del set de
--      facturas con saldo pendiente en cuanto se salda.
--
-- Confirmado con SELECT que no hay facturas ya afectadas en producción
-- (estado='parcial' AND total_cobrado >= total), por lo que no hace falta
-- backfill — el fix solo previene el problema hacia adelante.
--
-- Fix: cuando el pago deja la factura saldada Y el estado actual era
-- 'parcial', se la vuelve a 'emitida' (que es el estado que ya usan todas
-- las facturas emitidas y cobradas en un solo pago). El resto de los
-- casos (pendiente, error_afip llegando a saldarse — edge cases teóricos)
-- se comportan exactamente igual que antes.

CREATE OR REPLACE FUNCTION public.registrar_cobro_completo(p_empresa_id uuid, p_cliente_id uuid, p_monto numeric, p_medio text, p_referencia text DEFAULT NULL::text, p_notas text DEFAULT NULL::text, p_usuario_id uuid DEFAULT NULL::uuid, p_factura_id uuid DEFAULT NULL::uuid, p_facturas_aplicadas jsonb DEFAULT NULL::jsonb, p_offline_local_id text DEFAULT NULL::text)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF p_facturas_aplicadas IS NOT NULL THEN
    ...
    LOOP
      ...
      -- FIX FACTURACION-AUDIT-01: si esto la deja saldada y venía de
      -- 'parcial', se normaliza a 'emitida' en vez de quedarse pegada en
