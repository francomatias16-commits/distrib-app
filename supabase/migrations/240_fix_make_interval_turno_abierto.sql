-- ============================================================================
-- 240_fix_make_interval_turno_abierto.sql
--
-- Corrige el patrón (p) turno_caja_abierto_prolongado: make_interval(hours
-- => ...) no acepta 'numeric' (v_umbral_horas_turno_abierto es numeric),
-- el mismo problema que ya se había corregido en el patrón (h) vía la
-- migración 234. Se aplica la misma solución: multiplicar por
-- interval '1 hour' en vez de usar make_interval. También se renombra el
-- alias u3 -> u3b dentro del bloque (p) para evitar cualquier ambigüedad
-- con el u3 ya usado en el patrón (d). No se toca ningún otro patrón.
-- ============================================================================
...
BEGIN
  ...
  -- FIX 240: make_interval(hours => numeric) no existe; se usa
  -- multiplicación por interval, igual que el fix 234 del patrón (h).
  RETURN QUERY
  SELECT
  ...
  LEFT JOIN public.usuarios u3b ON u3b.id = tc.usuario_id
  WHERE cp.empresa_id = p_empresa_id
    AND tc.estado = 'abierto'
    AND tc.abierto_at <= now() - (v_umbral_horas_turno_abierto * interval '1 hour');
...
COMMENT ON FUNCTION public.detectar_anomalias_auditoria IS
  ...
  'restaurado en 239, fix 240). Devuelve una fila por patrón sospechoso agrupado '
