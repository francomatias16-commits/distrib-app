-- ============================================================
-- 517 — Fix: más FKs externas (NO ACTION) bloquean fn_reset_demo_v2
-- ============================================================
-- Tras el fix 516 (whatsapp_conversaciones.pedido_creado_id), el pipeline
-- volvió a fallar en el mismo patrón: canjes_recompensas.aplicado_en_pedido_id
-- también es NO ACTION y bloquea el DELETE de pedidos.
--
-- Una auditoría de pg_constraint contra las 56 tablas del reset encontró
-- el resto de FKs NO ACTION en la misma situación (ninguna estaba en el
-- ciclo de borrado/reinserción, todas son tablas externas al reset que
-- referencian sus ids): canjes_recompensas.aplicado_en_pedido_id,
-- alertas_stock.orden_compra_id, migracion_sesiones.lista_precio_id/
-- deposito_id, migracion_plantillas_mapeo.lista_precio_id/deposito_id.
-- Mismo patrón: guardar valor original, poner NULL antes del ciclo,
-- restaurar al final (los ids de las tablas reseteadas se preservan).
...
BEGIN
  ...
  -- FKs externas NO ACTION que bloquean el DELETE de las tablas del reset
  -- (ver CONTEXTO arriba). Todas se guardan, se anulan y se restauran.
