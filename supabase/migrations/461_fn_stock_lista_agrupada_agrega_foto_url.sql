-- 461_fn_stock_lista_agrupada_agrega_foto_url.sql
-- Fix: la página de Stock mostraba solo iniciales como avatar de producto,
-- nunca la foto real, aunque productos.js sí la muestra. Causa: la RPC
-- fn_stock_lista_agrupada (396) hace JOIN contra public.productos pero no
-- seleccionaba foto_url, así que el dato nunca llegaba a stock.js.
-- Este fix agrega foto_url al RETURNS TABLE y al SELECT, sin tocar el resto
-- de la lógica de la función (filtros, orden, paginación, permisos).
...
COMMENT ON FUNCTION public.fn_stock_lista_agrupada(text, uuid, uuid, text, uuid[], integer, integer) IS
  'v461: agrega foto_url (public.productos) al resultado, para que Stock.html pueda mostrar la foto real del producto igual que Productos.html en vez de solo iniciales. Resto de la lógica igual a v396 (lista de stock agrupada por producto, suma disponible/reservado/total entre depositos).';
