-- Etapa 8: auditoria robusta del reinicio automatico de la demo (corre cada
-- 6 horas via cron: fn_reset_demo_cron -> fn_reset_demo_v2).
--
-- Hallazgo: dos tablas hijas quedaban FUERA de fn_snapshot_demo_v2 /
-- fn_reset_demo_v2 pero SI tienen FK ON DELETE CASCADE hacia una tabla que
-- el reset SI borra y vuelve a insertar. Resultado: en cuanto esas tablas
-- tuvieran datos, el proximo reset las borraria por cascada y NUNCA las
-- restauraria (perdida de datos silenciosa, sin error visible):
...
-- Al momento de esta auditoria ambas estaban vacias para la empresa demo,
-- por eso el bug no se habia notado todavia.
--
-- Hallazgo adicional (no es perdida de datos, pero si inconsistencia):
--   - contadores_empresa (numeracion correlativa de facturas, remitos,
--     ordenes de compra, etc.) tampoco estaba dentro del ciclo de
--     snapshot/reset. Como los comprobantes SI vuelven al numero del
--     snapshot pero el contador seguia avanzando para siempre, cada reset
--     iba abriendo una brecha creciente entre "ultima factura visible" y
--     "proximo numero a emitir".
--
-- Fix: se agregaron las 3 tablas al mismo mecanismo generico de
-- snapshot/reset (mismo patron que ya usan el resto de las tablas hijas),
-- respetando el orden de dependencias (se listan con ord > su tabla padre
-- para que se borren antes que ella y se inserten despues).
--
-- Ya validado en producción: se corrió fn_snapshot_demo_v2 y luego
-- fn_reset_demo_v2 sobre la empresa demo sin errores, y los datos
-- (facturas, pedidos, stock, contadores_empresa) quedaron consistentes.
...
BEGIN
  ...
  FOR v_cfg IN
    SELECT * FROM (VALUES
      ...
      (71, 'combos',                  false, NULL,                 NULL),
      (72, 'combo_items',             true,  'combos',             'combo_id'),
      (73, 'etiquetas',               false, NULL,                 NULL),
      (74, 'entidad_etiquetas',       false, NULL,                 NULL),
      (75, 'reglas_score',            false, NULL,                 NULL),
      (76, 'config_etiquetas',        false, NULL,                 NULL),
      (77, 'asistente_conversaciones', false, NULL,                NULL),
      (78, 'asistente_mensajes',       true,  'asistente_conversaciones', 'conversacion_id'),
      (79, 'conciliacion_bancaria_lotes',      false, NULL,        NULL),
      (80, 'conciliacion_bancaria_movimientos', false, NULL,       NULL),
      (81, 'captura_competencia',              false, NULL,        NULL),
      (82, 'prospectos_competencia',           false, NULL,        NULL),
      (83, 'ahorro_competencia_movimientos',   false, NULL,        NULL),
      (84, 'cliente_ahorro_acumulado',         false, NULL,        NULL),
      (85, 'gastos_generales',                 false, NULL,        NULL),
      (86, 'eventos_negocio',                  false, NULL,        NULL),
      (87, 'notif_log',                        false, NULL,        NULL),
      (88, 'captura_competencia_items',        true,  'captura_competencia',      'captura_id'),
      (89, 'asistente_acciones_pendientes',    true,  'asistente_conversaciones', 'conversacion_id'),
      (90, 'contadores_empresa',                false, NULL,        NULL)
...
BEGIN
  ...
  ALTER TABLE cobros DISABLE TRIGGER tg_score_cobro;
  ...
  FOR v_cfg IN
    SELECT * FROM (VALUES
      ...
      (69,'whatsapp_mensajes',true,'whatsapp_conversaciones','conversacion_id'),
      (70,'devoluciones_pos_items',true,'devoluciones_pos','devolucion_id'),
      (71,'combos',false,NULL,NULL),(72,'combo_items',true,'combos','combo_id'),
      (73,'etiquetas',false,NULL,NULL),(74,'entidad_etiquetas',false,NULL,NULL),(75,'reglas_score',false,NULL,NULL),
      (76,'config_etiquetas',false,NULL,NULL),
      (77,'asistente_conversaciones',false,NULL,NULL),
      (78,'asistente_mensajes',true,'asistente_conversaciones','conversacion_id'),
      (79,'conciliacion_bancaria_lotes',false,NULL,NULL),
  ...
  FOR v_cfg IN
    SELECT * FROM (VALUES
      ...
      (71,'combos'),(72,'combo_items'),(73,'etiquetas'),(74,'entidad_etiquetas'),(75,'reglas_score'),
      (76,'config_etiquetas'),
      (77,'asistente_conversaciones'),(78,'asistente_mensajes'),
      (79,'conciliacion_bancaria_lotes'),(80,'conciliacion_bancaria_movimientos'),
      (81,'captura_competencia'),(82,'prospectos_competencia'),(83,'ahorro_competencia_movimientos'),
      (84,'cliente_ahorro_acumulado'),(85,'gastos_generales'),(86,'eventos_negocio'),(87,'notif_log'),
      (88,'captura_competencia_items'),(89,'asistente_acciones_pendientes'),(90,'contadores_empresa')
    ) AS t(ord, tabla)
    ORDER BY ord
  LOOP
    SELECT string_agg(quote_ident(column_name), ', ' ORDER BY ordinal_position)
  ...
  PERFORM public.fn_relink_portal_clientes_demo(v_empresa_id);

  ALTER TABLE cobros ENABLE TRIGGER tg_score_cobro;
