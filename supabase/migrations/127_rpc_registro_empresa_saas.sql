-- 127_rpc_registro_empresa_saas.sql
-- RPC de registro público multi-tenant.
-- Equivalente a setup_inicial_empresa pero SIN la restricción de empresa única.
-- Crea empresa en modo trial + usuario admin + datos iniciales.
...
BEGIN
  ...
  -- Crear empresa en modo trial (30 días)
  -- El trigger trg_saas_nuevo_empresa setea saas_plan y saas_trial_fin automáticamente
  INSERT INTO public.empresas (
    nombre, cuit, domicilio, telefono, email, activa
  )
  VALUES (
  ...
  -- Leer el trial_fin que puso el trigger
...
COMMENT ON FUNCTION public.registrar_empresa_saas IS
  'Registro público SaaS: crea empresa en trial + usuario admin. Sin restricción de empresa única (multi-tenant).';
