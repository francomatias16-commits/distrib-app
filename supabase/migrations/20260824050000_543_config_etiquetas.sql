-- ============================================================
-- 543 — Generador de etiquetas de precio/código de barras — Etapa 1:
--        tabla config_etiquetas (config por empresa, singleton) + alta
--        en el ciclo de snapshot/reset del demo.
-- ============================================================
--
-- Ver PLAN_ETIQUETAS_PRECIO_CODIGO_BARRAS.md, sección 3.1.
--
-- Mismo patrón que facturacion_config (102_facturacion_arca.sql):
-- singleton por empresa_id, RLS solo service_role (el frontend nunca la
-- toca directo — pasa por lib/handlers/etiquetas.js con el mismo gate
-- de permiso `empresa_config` que usa Admin → Hardware para el resto de
-- la config de impresión).
--
-- No se toca el esquema de `productos`: ya tiene codigo, codigo_es_barras,
-- precio_base, iva, vendido_por_peso (ver sección 3.2 del plan).

-- ------------------------------------------------------------
-- 1. Tabla
-- ------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.config_etiquetas (
  empresa_id              uuid PRIMARY KEY REFERENCES public.empresas(id) ON DELETE CASCADE,
  ancho_mm                numeric NOT NULL DEFAULT 50,
  alto_mm                 numeric NOT NULL DEFAULT 25,
  columnas                integer NOT NULL DEFAULT 3,
  margen_mm               numeric NOT NULL DEFAULT 2,
  formato_simbologia      text NOT NULL DEFAULT 'auto'
                             CHECK (formato_simbologia IN ('auto', 'ean13', 'code128')),
  lista_precio_default_id uuid REFERENCES public.listas_precios(id) ON DELETE SET NULL,
  incluir_iva             boolean NOT NULL DEFAULT true,
  mostrar_codigo_interno  boolean NOT NULL DEFAULT true,
  updated_at              timestamptz NOT NULL DEFAULT now()
);

COMMENT ON TABLE public.config_etiquetas IS
  'Config por empresa (singleton) del generador de etiquetas de precio/código de barras. Ver PLAN_ETIQUETAS_PRECIO_CODIGO_BARRAS.md.';

-- Sin trigger de updated_at: mismo criterio que facturacion_config (102),
-- no hay una función trigger genérica en el proyecto para esto — el repo
-- (lib/repos/etiquetas.js) setea updated_at = now() a mano en cada UPDATE.

-- ------------------------------------------------------------
-- 2. RLS — solo service_role (mismo criterio que facturacion_config)
-- ------------------------------------------------------------
ALTER TABLE public.config_etiquetas ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS service_role_all_config_etiquetas ON public.config_etiquetas;
CREATE POLICY service_role_all_config_etiquetas ON public.config_etiquetas
  FOR ALL
  TO service_role
  USING (true)
  WITH CHECK (true);
...
-- ------------------------------------------------------------
-- 3. Alta en el ciclo de snapshot/reset del demo (ord 56, singleton
--    no-hija — mismo patrón que facturacion_config en ord 1). Gotcha
--    documentado en la sección 3.1 del plan: si se posterga, la config
--    de etiquetas desaparece en el próximo reset de las 6hs.
-- ------------------------------------------------------------
...
BEGIN
  ...
  FOR v_cfg IN
    SELECT * FROM (VALUES
      ...
      (56, 'config_etiquetas',        false, NULL,                 NULL)
...
BEGIN
  ...
  FOR v_cfg IN
    SELECT * FROM (VALUES
      ...
      (54,'entregas',true,'rutas','ruta_id'),(55,'audit_log',false,NULL,NULL),(56,'config_etiquetas',false,NULL,NULL)
    ) AS t(ord, tabla, es_hija, tabla_padre, fk_col)
    ORDER BY ord DESC
  LOOP
    IF v_cfg.es_hija THEN
  ...
  FOR v_cfg IN
    SELECT * FROM (VALUES
      ...
      (52,'movimientos_puntos'),(53,'movimientos_stock'),(54,'entregas'),(55,'audit_log'),(56,'config_etiquetas')
    ) AS t(ord, tabla)
    ORDER BY ord
  LOOP
    SELECT string_agg(quote_ident(column_name), ', ' ORDER BY ordinal_position)
