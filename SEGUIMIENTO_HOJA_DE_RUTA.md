...
| JS que genera markup con colores hardcodeados | v728-v730 (ver arriba) + v731: `remito.js` evaluado (excepción, no aplica); `vincular-celular.js` migrado completo (21 colores → tokens, incluye reemplazo del azul #2563EB heredado de Stripe/Linear) + v823: `rutas.js` migrado completo (26 casos + 1 fallback desincronizado corregido) + v824: `busqueda-global.js` migrado completo (6 fallbacks desincronizados corregidos + 1 shadow → tinta ink, sin hex crudo real) + v825: 5 casos sueltos confirmados (`topbar-widgets.js`, `productos-scanner-remoto.js`) + `offline-core.js` migrado completo (1 token falso, 2 fallbacks, 2 overlays) + v826: `pedidos.js` migrado completo (5 fallbacks + 1 hex crudo real) + v827: `cc-proveedores.js` migrado completo (1 fallback) + v828: `etiquetas.js` migrado completo (5 fallbacks + 1 shadow) | 23/34 archivos del frente de hex crudo real | En curso |
...
- [ ] Resto del frente de hex/rgba crudos reales en JS — 23 archivos, pendiente (siguiente: `reportes-stock.js`, `pos.js` — ver `audit_table.md`)
